...
- `bam.kinship`: the admitted `king` benchmark row must preserve `kinship.json`, `kinship.summary.json`, `kinship.segments.tsv`, and `stage.metrics.json` while reporting observed overlap, pair counts, and governed relationship labels through the pairwise kinship surface.
...
- `bam.authenticity`: the admitted `authenticct`, `damageprofiler`, and `pmdtools` benchmark rows must preserve `authenticity.json`, `authenticity.summary.json`, and `stage.metrics.json` while retaining the governed composed evidence artifacts consumed from damage, contamination, complexity, coverage, and mapping summaries.
...
- `bam.contamination`: the admitted `schmutzi`, `verifybamid2`, and `contammix` benchmark rows must preserve `contamination.json`, `contamination.summary.json`, and `stage.metrics.json` while retaining the governed panel-backed evidence contract.
