...
pub fn write_adapter_bank_ref(
    ...
    bijux_infra::atomic_write_json(&path, &payload).context("write adapter_bank_ref.json")?;
    Ok(path)
}
...
pub fn write_adapter_trimming_report(
    ...
    bijux_infra::atomic_write_json(&path, &report).context("write adapter_trimming_report.json")?;
    Ok(path)
}
