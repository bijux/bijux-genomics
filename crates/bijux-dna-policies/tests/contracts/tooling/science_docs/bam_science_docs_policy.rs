...
#[test]
fn policy__contracts__bam_science_docs_policy__operational_contract_links_governed_bam_artifact_surfaces_exactly(
) {
    let expected = BTreeSet::from([
    ...
    let documented = markdown_link_targets("docs/20-science/bam/OPERATIONAL_CONTRACT.md");
    assert_eq!(
        documented, expected,
        "docs/20-science/bam/OPERATIONAL_CONTRACT.md must link the governed BAM artifact surfaces exactly"
    );
}

#[test]
fn policy__contracts__bam_science_docs_policy__stage_assumptions_link_governed_bam_stage_surfaces_exactly(
) {
    let expected = BTreeSet::from([
    ...
    let documented = markdown_link_targets("docs/20-science/bam/STAGE_ASSUMPTIONS.md");
    assert_eq!(
        documented, expected,
        "docs/20-science/bam/STAGE_ASSUMPTIONS.md must link the governed BAM stage surfaces exactly"
    );
}

#[test]
fn policy__contracts__bam_science_docs_policy__stage_catalog_links_governed_bam_catalog_surfaces_exactly(
) {
    let expected = BTreeSet::from([
    ...
    let documented = markdown_link_targets("docs/20-science/bam/STAGE_CATALOG.md");
    assert_eq!(
        documented, expected,
        "docs/20-science/bam/STAGE_CATALOG.md must link the governed BAM catalog surfaces exactly"
    );
}
...
#[test]
fn policy__contracts__bam_science_docs_policy__reference_governance_links_governed_bam_bank_surfaces_exactly(
) {
    let expected = BTreeSet::from([
        "../../../domain/bam/docs/DEFAULT_SETTINGS.md".to_string(),
        "../../../domain/bam/stages/".to_string(),
    ]);
    let documented = markdown_link_targets("docs/20-science/bam/REFERENCE_GOVERNANCE.md");
    assert_eq!(
        documented, expected,
        "docs/20-science/bam/REFERENCE_GOVERNANCE.md must link the governed BAM bank surfaces exactly"
    );
}
