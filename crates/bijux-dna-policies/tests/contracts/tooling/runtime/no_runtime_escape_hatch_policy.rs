...
fn policy__contracts__no_runtime_escape_hatch_policy__forbidden_terms_blocked_in_source() {
    ...
    let forbidden =
        ["over".to_string() + "ride", "by".to_string() + "pass", "ALLOW_OVERRIDE".to_string()];
    let mut offenders = Vec::new();

    let source_roots = [root.join("crates/bijux-dna/src"), root.join("crates/bijux-dna/src/bin")];
    for source_root in source_roots {
        ...
        for entry in WalkDir::new(&source_root).into_iter().filter_map(Result::ok) {
            ...
            if forbidden.iter().any(|needle| content.contains(&needle.to_lowercase())) {
