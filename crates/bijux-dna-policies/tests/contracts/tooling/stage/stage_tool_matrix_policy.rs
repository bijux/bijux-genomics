...
fn policy__contracts__stage_tool_matrix_policy__stages_have_primary_validation_and_reporting_contracts(
    ...
    if stages.is_empty() {
        offenders.push(
            "missing [[stages]] matrix in configs/ci/registry/tool_registry.toml".to_string(),
        );
    }
    ...
    for stage in &stages {
        ...
        let requires_validation =
            stage.get("requires_validation").and_then(toml::Value::as_bool).unwrap_or(false);
        ...
        let requires_reporting =
            stage.get("requires_reporting").and_then(toml::Value::as_bool).unwrap_or(false);
        if requires_reporting && reporting.is_empty() {
            offenders
                .push(format!("stage={id}: requires_reporting=true but reporting_tools is empty"));
        }

        if id.contains("qc") && !reporting.iter().any(|t| t == "multiqc") {
            offenders
                .push(format!("stage={id}: qc stages must include multiqc in reporting_tools"));
        }
    ...
    assert!(offenders.is_empty(), "stage tool matrix policy violations:\n{}", offenders.join("\n"));
}
