...
fn bench_contract_snapshot() -> anyhow::Result<()> {
    ...
    insta::assert_json_snapshot!(bijux_dna_testkit::snapshot_normalize_json(&payload), @r###"
    {
      "spec": {
...
fn suite_requires_stratification_metadata() {
    ...
    let result = bijux_dna_benchmark::summarize(
        &suite,
        &[],
        &bijux_dna_benchmark::BenchRunOptions::default(),
    );
    assert!(
        result.is_err(),
