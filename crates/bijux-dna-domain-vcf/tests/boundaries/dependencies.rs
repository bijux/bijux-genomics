...
fn dependency_graph_matches_domain_vcf_boundary() {
    ...
    let expected_dependencies: BTreeSet<_> =
        ["anyhow", "bijux-dna-core", "flate2", "schemars", "serde", "serde_json"]
            .into_iter()
            .map(str::to_string)
            .collect();
    ...
    assert!(
        manifest.contains("bijux-dna-core.workspace = true"),
        "bijux-dna-core must use the workspace dependency declaration"
    );
