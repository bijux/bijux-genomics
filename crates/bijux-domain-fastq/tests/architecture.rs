...
fn domain_fastq_does_not_depend_on_engine() {
    ...
    assert!(
        !contents.contains("bijux-engine"),
        "bijux-domain-fastq must not depend on bijux-engine directly"
    );

    let deps = cargo_lock_dependencies("bijux-domain-fastq");
    assert!(
        !deps.iter().any(|dep| dep.starts_with("bijux-engine")),
        "bijux-domain-fastq must not depend on bijux-engine indirectly via Cargo.lock"
    );
}
