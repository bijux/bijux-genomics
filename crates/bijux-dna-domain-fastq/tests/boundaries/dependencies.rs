...
#[test]
fn dependency_graph_matches_domain_fastq_boundary() {
    ...
    for dependency in ["bijux-dna-core", "bijux-dna-infra"] {
        assert!(
    ...
    assert!(
        ...
        "domain FASTQ must not declare ad hoc internal path dependencies"
    );
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "reverse-layer dependency `{forbidden}` must not enter bijux-dna-domain-fastq"
        );
    }
}
