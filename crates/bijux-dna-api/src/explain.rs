...
pub struct PlanExplainStageV1 {
    ...
    pub inputs: Vec<bijux_dna_stage_contract::ArtifactRef>,
    pub outputs: Vec<bijux_dna_stage_contract::ArtifactRef>,
...
pub struct PlanExplainV1 {
    ...
    pub policy: bijux_dna_core::contract::PlanPolicy,
...
impl PlanExplainV1 {
    #[must_use]
    pub fn from_plan(plan: &bijux_dna_core::contract::ExecutionGraph) -> Self {
        let stages = plan
            .steps()
            .iter()
...
pub fn explain_bundle(
    ...
    let stage_contracts = plan
        ...
        .filter_map(|step| {
            ...
            {
                bijux_dna_domain_fastq::stage_contract_hash(&stage_id)
                    .and_then(std::result::Result::ok)
            } else if stage_id.starts_with(id_catalog::BAM_PREFIX) {
                bijux_dna_domain_bam::stage_contract_hash(&stage_id)
                    .and_then(std::result::Result::ok)
            } else {
                None
            };
