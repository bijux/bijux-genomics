...
pub fn bench_fastq_screen<S: ::std::hash::BuildHasher>(
    ...
    if args.explain {
        write_explain_md(&bench_dir, STAGE_SCREEN_TAXONOMY.as_str(), &tools, &[], None)?;
        write_explain_plan_json(&bench_dir, STAGE_SCREEN_TAXONOMY.as_str(), &tools, &registry, None)?;
    }
    ...
    for tool in &tools {
        ...
        let tool_spec =
            build_tool_execution_spec(STAGE_SCREEN_TAXONOMY.as_str(), tool, &registry, catalog, platform)?;
