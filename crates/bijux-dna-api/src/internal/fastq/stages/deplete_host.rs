...
pub fn bench_fastq_deplete_host<S: ::std::hash::BuildHasher>(
    ...
    preflight_stage(STAGE_DEPLETE_HOST.as_str(), artifact.kind)?;
    ...
    for tool in tools {
        ...
        let plan = plan_host_depletion(&tool_spec, &bench_inputs.r1, &out_dir)?;
        ...
        let host_fraction_removed = if bench_inputs.input_stats.reads == 0 {
            0.0
        } else {
