...
pub fn bench_fastq_deplete_host<S: ::std::hash::BuildHasher>(
    ...
    for tool in setup.tools.clone() {
        ...
        validate_host_report_metrics(&report, &metric_set.metrics)?;
...
fn build_deplete_host_report<S: ::std::hash::BuildHasher>(
    ...
    let host_fraction_removed = ratio_u64(reads_removed, reads_in);
...
fn validate_host_report_counts(
    setup: &DepleteHostBenchmarkSetup,
    report: &DepleteHostReportV1,
) -> Result<()> {
    ...
    if report.reads_in != expected_reads_in {
        return Err(anyhow!(
            "host depletion report reads_in mismatch: expected {}, observed {}",
            expected_reads_in,
            report.reads_in
        ));
    }
    ...
    if report.bases_in != expected_bases_in {
        return Err(anyhow!(
            "host depletion report bases_in mismatch: expected {}, observed {}",
            expected_bases_in,
            report.bases_in
        ));
    }
    validate_host_removed_count("reads", report.reads_in, report.reads_out, report.reads_removed)?;
    validate_host_removed_count("bases", report.bases_in, report.bases_out, report.bases_removed)?;
    ...
    if report.pairs_in != expected_pairs_in {
        return Err(anyhow!(
            "host depletion report pairs_in mismatch: expected {:?}, observed {:?}",
            expected_pairs_in,
            report.pairs_in
        ));
    }
    if report.pairs_out.zip(report.pairs_in).is_some_and(|(out, input)| out > input) {
        return Err(anyhow!(
            "host depletion report pairs_out exceeds pairs_in: pairs_out={:?} pairs_in={:?}",
            report.pairs_out,
            report.pairs_in
        ));
    }
...
fn validate_host_removed_count(label: &str, input: u64, output: u64, removed: u64) -> Result<()> {
...
fn validate_host_report_metrics(
    report: &DepleteHostReportV1,
    metrics: &FastqDepleteHostMetrics,
) -> Result<()> {
    if metrics.reads_in != report.reads_in {
        return Err(anyhow!(
            "host depletion metrics reads_in mismatch: expected {}, observed {}",
            report.reads_in,
            metrics.reads_in
        ));
    }
    if metrics.reads_out != report.reads_out {
        return Err(anyhow!(
            "host depletion metrics reads_out mismatch: expected {}, observed {}",
            report.reads_out,
            metrics.reads_out
        ));
    }
    if metrics.bases_in != report.bases_in {
        return Err(anyhow!(
            "host depletion metrics bases_in mismatch: expected {}, observed {}",
            report.bases_in,
            metrics.bases_in
        ));
    }
    if metrics.bases_out != report.bases_out {
        return Err(anyhow!(
            "host depletion metrics bases_out mismatch: expected {}, observed {}",
            report.bases_out,
            metrics.bases_out
        ));
    }
    validate_host_metrics_pair_count("pairs_in", report.pairs_in, metrics.pairs_in)?;
    validate_host_metrics_pair_count("pairs_out", report.pairs_out, metrics.pairs_out)?;
    let expected_fraction = report.host_fraction_removed.clamp(0.0, 1.0);
    if (metrics.host_fraction_removed - expected_fraction).abs() > f64::EPSILON {
        return Err(anyhow!(
            "host depletion metrics fraction mismatch: expected {}, observed {}",
            expected_fraction,
            metrics.host_fraction_removed
        ));
    }
    Ok(())
}

fn validate_host_metrics_pair_count(
    name: &str,
    expected: Option<u64>,
    observed: u64,
) -> Result<()> {
    ...
    if observed != expected {
        return Err(anyhow!(
            "host depletion metrics {name} mismatch: expected {expected}, observed {observed}"
        ));
    }
    Ok(())
}
