...
pub fn bench_fastq_deplete_host<S: ::std::hash::BuildHasher>(
    ...
    ensure_image_qa_passed(STAGE_DEPLETE_HOST.as_str(), &setup.tools, platform, catalog)?;
    ensure_tool_qa_passed(STAGE_DEPLETE_HOST.as_str(), &setup.tools, platform, catalog)?;
