...
pub fn bench_fastq_trim<S: ::std::hash::BuildHasher>(
...
) -> Result<BenchOutcome<bijux_dna_analyze::FastqTrimMetrics>> {
    ...
    preflight_stage(STAGE_TRIM_READS.as_str(), artifact.kind)?;
    ...
    let bench_dir_name = bench_dir_name(&STAGE_TRIM_READS)
        .ok_or_else(|| anyhow!("bench dir missing for {}", STAGE_TRIM_READS.as_str()))?;
    ...
    if args.explain {
        write_explain_md(&bench_dir, STAGE_TRIM_READS.as_str(), &tools, &[], None)?;
        write_explain_plan_json(&bench_dir, STAGE_TRIM_READS.as_str(), &tools, &registry, None)?;
    }
    ...
    for tool in &tools {
        ...
        let tool_spec =
            build_tool_execution_spec(STAGE_TRIM_READS.as_str(), tool, &registry, catalog, platform)?;
