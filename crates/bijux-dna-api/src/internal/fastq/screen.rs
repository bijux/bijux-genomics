...
use bijux_dna_planner_fastq::stage_api::fastq::screen::plan_screen;
...
pub fn bench_fastq_screen<S: ::std::hash::BuildHasher>(
...
) -> Result<BenchOutcome<bijux_dna_analyze::FastqScreenMetrics>> {
    let tools = select_screen_tools(&args.tools)?;
    let artifact = FastqArtifact::single_end(&args.r1);
    preflight_stage(STAGE_SCREEN.as_str(), artifact.kind)?;
    let header = inspect_headers(&args.r1, None, false)?;
    log_header_warnings(STAGE_SCREEN.as_str(), &header);

    let registry = load_registry(&std::env::current_dir()?.join("domain"))
        .map_err(|err| anyhow!("manifest validation failed: {err}"))?;
    let tools = filter_tools_by_role(STAGE_SCREEN.as_str(), &tools, &registry, false)?;

    let bench_dir_name = bench_dir_name(&STAGE_SCREEN)
        .ok_or_else(|| anyhow!("bench dir missing for {}", STAGE_SCREEN.as_str()))?;
    ...
    if args.explain {
        write_explain_md(&bench_dir, STAGE_SCREEN.as_str(), &tools, &[], None)?;
        write_explain_plan_json(&bench_dir, STAGE_SCREEN.as_str(), &tools, &registry, None)?;
    }

    ensure_bench_runner(platform, runner_override)?;
    ensure_image_qa_passed(STAGE_SCREEN.as_str(), &tools, platform, catalog)?;
    ensure_tool_qa_passed(STAGE_SCREEN.as_str(), &tools, platform, catalog)?;
    ...
    for tool in &tools {
        ...
        let tool_spec =
            build_tool_execution_spec(STAGE_SCREEN.as_str(), tool, &registry, catalog, platform)?;
    ...
    for (tool, execution) in tool_order.into_iter().zip(executions.into_iter()) {
        if execution.exit_code != 0 {
            ...
            failures.push(RawFailure {
                stage: STAGE_SCREEN.as_str().to_string(),
                tool,
                reason: format!(
