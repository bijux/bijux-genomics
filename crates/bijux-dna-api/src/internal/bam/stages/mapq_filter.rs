...
///
/// The written report lives at `target/local-smoke/bam.mapq_filter/mapq_filter.json`
...
pub fn write_local_mapq_filter_smoke_report() -> Result<PathBuf> {
    ...
    let output_root = repo_root.join("target/local-smoke/bam.mapq_filter");
...
fn materialize_local_mapq_filter_smoke_case(
    ...
    let top_level_filtered_bai = output_root.join("mapq_filtered.bam.bai");
    bijux_dna_infra::atomic_write_bytes(
