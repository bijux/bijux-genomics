...
/// Returns an error if the repository root cannot be resolved, the governed planner config is
/// invalid, or the plan artifact cannot be written.
pub fn write_local_contamination_plan() -> Result<PathBuf> {
    let repo_root = crate::support::workspace::resolve_repo_root()?;
...
/// Materialize the governed local-smoke `bam.contamination` report bundle.
///
/// The written report artifact lives at
/// `runs/bench/local-smoke/bam.contamination/local_smoke.json`
...
pub fn write_local_contamination_smoke_report() -> Result<PathBuf> {
    crate::internal::bam::stages::contamination::write_local_contamination_smoke_report()
}
...
pub fn write_local_haplogroups_plan() -> Result<PathBuf> {
    let repo_root = crate::support::workspace::resolve_repo_root()?;
...
pub fn write_local_damage_smoke_report() -> Result<PathBuf> {
    crate::internal::bam::stages::damage::write_local_damage_smoke_report()
}
...
pub fn write_local_authenticity_smoke_report() -> Result<PathBuf> {
    crate::internal::bam::stages::authenticity::write_local_authenticity_smoke_report()
}
...
pub fn write_local_recalibration_smoke_report() -> Result<PathBuf> {
    crate::internal::bam::stages::recalibration::write_local_recalibration_smoke_report()
}
