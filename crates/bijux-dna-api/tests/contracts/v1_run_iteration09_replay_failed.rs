...
use bijux_dna_api::v1::api::run::{
    assess_failed_replay_eligibility, execute, replay_failed_run, ExecuteRequest, RuntimeKind,
};
...
fn failed_run_eligibility_marks_pending_stage_as_replayable() -> Result<()> {
    ...
    assert_eq!(eligibility.get("failed").and_then(serde_json::Value::as_bool), Some(true));
    assert_eq!(eligibility.get("unsafe_resume").and_then(serde_json::Value::as_bool), Some(false));
    assert!(eligibility
...
fn failed_run_replay_replays_in_new_directory_without_overwriting_source() -> Result<()> {
    ...
    graph_json["steps"][0]["command"]["template"] =
        serde_json::json!(["sh", "-c", format!("printf 'ACGT\\n' > {}", replay_output.display())]);
