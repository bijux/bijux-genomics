...
#[test]
fn write_local_length_filter_smoke_report_materializes_governed_outputs() -> Result<()> {
    let repo_root = repo_root()?;
    let _guard = RepoRootOverrideGuard::install(&repo_root);
    let output_dir = repo_root.join("runs/bench/local-smoke/bam.length_filter");
    if output_dir.exists() {
        std::fs::remove_dir_all(&output_dir)?;
    }

    let report_path = bijux_dna_api::v1::api::bam::write_local_length_filter_smoke_report()?;
    assert_eq!(
        report_path,
        repo_root.join("runs/bench/local-smoke/bam.length_filter/length_filter.json")
    );
    assert!(report_path.is_file(), "local-smoke BAM length filter report must exist");

    let payload: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(&report_path)?)?;
    assert_eq!(payload["stage_id"], serde_json::json!("bam.length_filter"));
    assert_eq!(
        payload["schema_version"],
        serde_json::json!("bijux.bam.length_filter.local_smoke.report.v1")
    );
    assert_eq!(payload["sample_id"], serde_json::json!("human_like_length_threshold_ladder"));
    assert_eq!(payload["expectation_matched"], serde_json::json!(true));
    assert_eq!(payload["min_length_threshold"], serde_json::json!(8));
    ...
    assert_eq!(payload["observed_min_length"], serde_json::json!(8));
    assert_eq!(payload["observed_max_length"], serde_json::json!(12));
    ...
    let length_filter_summary = repo_root.join(
        payload["length_filter_summary"]
            .as_str()
            .unwrap_or_else(|| panic!("length_filter_summary path missing")),
    );
    ...
    for path in [
        ...
        assert!(
            path.is_file(),
            "governed BAM length filter artifact must exist: {}",
            path.display()
        );
    }
    ...
    let summary_json: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&length_filter_summary)?)?;
    assert_eq!(summary_json["schema_version"], serde_json::json!("bijux.bam.length_filter.v1"));
    assert_eq!(summary_json["min_length_threshold"], serde_json::json!(8));
