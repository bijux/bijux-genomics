...
fn fastq_small_pipeline_emits_multi_stage_manifest() -> Result<()> {
    ...
    assert_eq!(
        manifest.get("profile_id").and_then(serde_json::Value::as_str).unwrap_or_else(|| panic!(
    ...
    for key in ["reads_total", "bases_total", "retention_ratio"] {
        assert!(metrics.get(key).is_some(), "metrics.json missing required key `{key}`");
    }
    Ok(())
}
...
fn fastq_small_golden_checksums_match_materialized_files() -> Result<()> {
    let run_dir = golden_fastq_toy_dir()?;
    let checksums: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(run_dir.join("artifact_checksums.json"))?)?;
    let artifacts = checksums
        .get("artifacts")
        .and_then(serde_json::Value::as_object)
        .ok_or_else(|| anyhow!("artifact_checksums missing artifacts object"))?;
    ...
    for (name, expected) in artifacts {
        ...
        assert_eq!(actual, expected, "golden toy checksum drift detected for `{name}`");
    }
    Ok(())
}
...
fn stable_toy_digest(path: &Path) -> Result<String> {
    match path.extension().and_then(|ext| ext.to_str()) {
        ...
        Some("html") => {
            Ok(sha256_hex_bytes(normalize_toy_html(&std::fs::read_to_string(path)?)?.as_bytes()))
        }
        _ => {
