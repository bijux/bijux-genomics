...
use bijux_dna_api::v1::api::run::{
    execute_local_fastq_workflow, explain_successful_replay, replay_explain, ReplayExplainRequestV1,
};
...
fn replay_success_explain_reports_reuse_and_drift_sets() -> Result<()> {
    ...
    assert_eq!(
        explain.get("schema_version").and_then(serde_json::Value::as_str),
    ...
    assert!(explain.get("reused_outputs").and_then(serde_json::Value::as_array).is_some());
    assert!(explain.get("unchanged_outputs").and_then(serde_json::Value::as_array).is_some());
    Ok(())
}
