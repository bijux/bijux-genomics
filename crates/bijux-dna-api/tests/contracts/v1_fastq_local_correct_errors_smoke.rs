...
fn write_local_correct_errors_smoke_plan_materializes_governed_target_output() -> Result<()> {
    ...
    let output_dir = repo_root.join("runs/bench/local-smoke/fastq.correct_errors");
    if output_dir.exists() {
        std::fs::remove_dir_all(&output_dir)?;
    }

    let plan_path = bijux_dna_api::v1::api::fastq::write_local_correct_errors_smoke_plan()?;
    assert_eq!(plan_path, repo_root.join("runs/bench/local-smoke/fastq.correct_errors/plan.json"));
    ...
    assert_eq!(
        payload["out_dir"],
        serde_json::json!("runs/bench/local-smoke/fastq.correct_errors/paired-dry-run/rcorrector")
    );
    assert_eq!(
