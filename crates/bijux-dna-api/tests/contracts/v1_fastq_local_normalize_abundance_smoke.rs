...
fn write_local_normalize_abundance_smoke_report_materializes_governed_outputs() -> Result<()> {
    ...
    assert!(table.contains("sample_a\totu_001\t0.25000000"));
    assert!(table.contains("sample_a\totu_002\t0.75000000"));
    assert!(table.contains("sample_b\totu_001\t0.50000000"));
    assert!(table.contains("sample_b\totu_003\t0.50000000"));
    ...
    assert_eq!(payload["sample_id"], serde_json::json!("core-v1-otu-table"));
    ...
    assert_eq!(
        ...
        serde_json::json!(
            "target/local-smoke/fastq.normalize_abundance/core-v1-otu-table/seqkit/abundance_normalized.tsv"
        )
    );
