...
use bijux_dna_pipelines::DefaultsLedgerV1;
use bijux_dna_runtime::recording::write_plan_provenance;
...
impl Runner for FakeRunner {
    fn run(&self, invocation: &Invocation) -> anyhow::Result<RunnerResult> {
        ...
        for output in &invocation.step.io.outputs {
            ...
            bijux_dna_infra::write_bytes(&output.path, "output")?;
        }
        Ok(RunnerResult {
            exit_code: 0,
            stdout: String::new(),
...
fn golden_spine_contract() -> Result<()> {
    ...
    bijux_dna_infra::write_bytes(&plan_hash_path, plan_hash.as_bytes())?;
    ...
    bijux_dna_infra::write_bytes(&facts_path, facts_line.as_bytes())?;
    ...
    bijux_dna_infra::write_bytes(&defaults_path, serde_json::to_vec(&defaults)?)?;

    let report_path = bijux_dna_analyze::write_run_report_from_facts(base_dir, &[facts_row])?;
    assert!(report_path.exists());

    let report_json: serde_json::Value = serde_json::from_slice(&std::fs::read(&report_path)?)?;
    let index_html = bijux_dna_api::v1::api::render_report_bundle_html(&report_json);
    let bundle_dir = base_dir.join("report_bundle");
    bijux_dna_infra::ensure_dir(&bundle_dir)?;
    let index_path = bundle_dir.join("index.html");
    bijux_dna_infra::write_bytes(&index_path, index_html.as_bytes())?;
