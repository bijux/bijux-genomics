...
use bijux_dna_core::contract::{RunRecordV1, StageExecutionRecordV1};
...
impl<'a> ContractEnforcer<'a> {
    ...
    fn verify_outputs(&self) -> Result<()> {
        for output in &self.step.io.outputs {
            ...
            if matches!(
                output.role,
                bijux_dna_core::contract::ArtifactRole::MetricsJson
                    | bijux_dna_core::contract::ArtifactRole::MetricsEnvelope
            ) {
