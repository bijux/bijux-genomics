...
pub fn plan(
    ...
    let outputs =
        crate::stages_support::audit_outputs(bijux_domain_bam::BamStage::Contamination, out_dir);
    let plan = StagePlanV1 {
        stage_id: StageId(STAGE_ID.to_string()),
        stage_version: STAGE_VERSION,
        tool_id: tool.tool_id.clone(),
...
pub fn plan(
    ...
    let outputs = crate::stages_support::audit_outputs(bijux_domain_bam::BamStage::Sex, out_dir);
    let plan = StagePlanV1 {
        stage_id: StageId(STAGE_ID.to_string()),
        stage_version: STAGE_VERSION,
        tool_id: tool.tool_id.clone(),
