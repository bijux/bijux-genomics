...
fn production_source_does_not_spawn_processes_or_open_networks() {
    ...
    assert!(
        findings.is_empty(),
        "planner production source must not spawn processes or open network connections:\n{}",
        findings.join("\n")
    );
}
...
fn production_source_does_not_mutate_filesystem_outputs() {
    ...
    assert!(
        findings.is_empty(),
        "planner production source must return planned outputs instead of mutating the filesystem:\n{}",
        findings.join("\n")
    );
}
