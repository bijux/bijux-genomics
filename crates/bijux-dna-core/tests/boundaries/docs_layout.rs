...
fn markdown_docs_stay_in_root_readme_or_docs_dir() {
    ...
    assert_eq!(
        ...
        "bijux-dna-core docs must stay at root README.md plus the governed docs directory files"
    );
}

#[test]
fn commands_doc_remains_part_of_the_docs_allowance() {
    let root = crate_root();
    let commands = std::fs::read_to_string(root.join("docs/COMMANDS.md"))
        .unwrap_or_else(|err| panic!("read docs/COMMANDS.md: {err}"));

    assert!(
        commands.contains("SSOT") && commands.contains("Managed Core Operations"),
        "docs/COMMANDS.md must stay the core operation inventory"
    );
}
