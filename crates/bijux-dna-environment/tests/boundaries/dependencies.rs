...
#[test]
fn dependency_graph_matches_environment_boundary() {
    ...
    let expected_dev_dependencies: BTreeSet<_> =
        ["bijux-dna-core", "bijux-dna-policies", "bijux-dna-testkit", "walkdir"]
            .into_iter()
            .map(str::to_string)
            .collect();
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "reverse-layer dependency `{forbidden}` must not enter bijux-dna-environment"
        );
    }
}
