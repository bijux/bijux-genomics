...
#[test]
fn vcf_demography_missing_time_bins_fails_loudly() {
    let row = FactsRowV1 {
        schema_version: "bijux.facts.v1".to_string(),
        run_id: "run-vcf-demography-contract".to_string(),
        stage_id: "vcf.demography".to_string(),
        tool_id: "ibdne".to_string(),
        tool_version: "1.0".to_string(),
        image_digest: Some(
    ...
    let err = build_run_report_model(std::path::Path::new("."), &[row])
        .expect_err("expected VCF demography contract violation");
    let msg = err.to_string();
    assert!(
        msg.contains("vcf downstream report contract violation"),
        "unexpected error message: {msg}"
    );
    assert!(msg.contains("vcf.demography:time_bins"));
}
