...
use bijux_dna_analyze::aggregate::{
    metric_kind_for_stage, metric_spec, stage_metric_spec, BenchmarkContext, BenchmarkRecord,
    FastqTrimMetrics,
};
use bijux_dna_analyze::report::{bench_schema_json, rank_trim_tools, write_run_summary_from_facts};
...
fn ranking_explanation_generation_has_modes() -> Result<()> {
    let metrics = FastqTrimMetrics {
        ...
        mean_q_before: 30.0,
        mean_q_after: 31.0,
        delta_metrics: bijux_dna_analyze::FastqDeltaMetrics {
            read_retention: 0.9,
            base_retention: 0.9,
            mean_q_delta: 1.0,
            gc_delta: 0.1,
