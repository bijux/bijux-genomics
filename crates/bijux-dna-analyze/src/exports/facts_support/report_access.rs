...
pub(super) fn stage_report_path(reports: &JsonBlob) -> Option<String> {
    reports.as_value().get("stage_report").and_then(|value| value.as_str()).map(str::to_string)
}
...
pub(crate) fn tool_invocation_for_stage(report: &StageReportV1) -> Option<ToolInvocationV1> {
    let raw = std::fs::read_to_string(&report.tool_invocation_path).ok()?;
    serde_json::from_str(&raw).ok()
}

pub(crate) fn stage_outputs_for_row(row: &FactsRowV1) -> Vec<String> {
