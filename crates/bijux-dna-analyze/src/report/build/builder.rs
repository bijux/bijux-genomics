            ...
            .map(|row| {
                serde_json::json!({
                    ...
                    "metrics": {
                        ...
                        "pair_count": row.metrics.get("pair_count").cloned().unwrap_or(serde_json::Value::Null),
                        "ibd_status": row.metrics.get("status").cloned().unwrap_or(serde_json::Value::Null),
                        "demography_method": row.metrics.get("method").cloned().unwrap_or(serde_json::Value::Null),
                        "demography_inference_status": row.metrics.get("inference_status").cloned().unwrap_or(serde_json::Value::Null),
                        "demography_status": row.metrics.get("status").cloned().unwrap_or(serde_json::Value::Null),
                        "insufficient_reason": row.metrics.get("insufficient_reason").cloned().unwrap_or(serde_json::Value::Null),
                        "time_bins": row.metrics.get("time_bins").cloned().unwrap_or(serde_json::Value::Null),
                        "ne_estimates": row.metrics.get("ne_estimates").cloned().unwrap_or(serde_json::Value::Null),
