...
pub fn write_report_bundle(dir: &Path, model: &ReportModel) -> Result<()> {
    ...
    bijux_dna_infra::atomic_write_json(&report_path, &model.report)
        .map_err(anyhow::Error::from)
        .context("write report bundle report.json")?;
    let assets_dir = dir.join("assets");
    bijux_dna_infra::ensure_dir(&assets_dir).context("create report assets dir")?;
...
mod tests {
    ...
    fn report_bundle_snapshot() -> anyhow::Result<()> {
        ...
        if update {
            bijux_dna_infra::write_bytes(&snapshot_path, actual.as_bytes())?;
        } else {
