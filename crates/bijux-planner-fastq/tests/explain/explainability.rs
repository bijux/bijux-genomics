...
use bijux_planner_fastq::compose_fastq_pipeline_steps;
use bijux_stage_contract::{PlanDecisionReason, PlanReasonKind};

#[test]
fn tool_reasons_carry_defaults_and_contract_hash() -> anyhow::Result<()> {
    let temp = bijux_infra::temp_dir("fastq-reasons")?;
    ...
    let stage = bijux_domain_fastq::STAGE_VALIDATE_PRE.as_str().to_string();
    let contract_hash = bijux_domain_fastq::stage_contract_hash(&stage)
