    ...
    #[test]
    fn impute_chunk_regions_respect_include_exclude_knobs() {
        let old_inc = std::env::var("BIJUX_IMPUTE_INCLUDE_CHR").ok();
        let old_exc = std::env::var("BIJUX_IMPUTE_EXCLUDE_CHR").ok();
        std::env::set_var("BIJUX_IMPUTE_INCLUDE_CHR", "chr1,chr2");
        std::env::set_var("BIJUX_IMPUTE_EXCLUDE_CHR", "chr2");
        let dir = tempfile::tempdir().unwrap_or_else(|err| panic!("tempdir: {err}"));
        let input = Path::new("tests/fixtures/vcf/default/input.vcf");
        let species = SpeciesContext {
            species_id: "Homo sapiens".to_string(),
        ...
        let out = run_impute_stage(
            ...
            &ImputeStageParams {
                ...
                chunk_overlap_bp: 100,
            },
        )
        .unwrap_or_else(|err| panic!("run include/exclude chunk test: {err}"));
        let manifest_raw = std::fs::read_to_string(&out.imputation_manifest_json)
            .unwrap_or_else(|err| panic!("read imputation manifest: {err}"));
        let manifest: serde_json::Value = serde_json::from_str(&manifest_raw)
            .unwrap_or_else(|err| panic!("parse imputation manifest: {err}"));
        let chunk_regions_path = manifest
            .get("chunk_regions_artifact")
            .and_then(serde_json::Value::as_str)
            .unwrap_or_else(|| panic!("chunk_regions_artifact missing from manifest"));
        let regions_raw = std::fs::read_to_string(chunk_regions_path)
            .unwrap_or_else(|err| panic!("read chunk regions json: {err}"));
        let regions: serde_json::Value = serde_json::from_str(&regions_raw)
            .unwrap_or_else(|err| panic!("parse chunk regions json: {err}"));
        let listed = regions
            .get("regions")
            .and_then(serde_json::Value::as_array)
            .unwrap_or_else(|| panic!("regions array missing"));
        assert!(
            listed
                .iter()
                .all(|row| row.as_str().unwrap_or_default().starts_with("chr1:")),
            "include/exclude filters must keep only chr1 chunks"
        );
        if let Some(v) = old_inc {
            std::env::set_var("BIJUX_IMPUTE_INCLUDE_CHR", v);
        } else {
            std::env::remove_var("BIJUX_IMPUTE_INCLUDE_CHR");
        }
        if let Some(v) = old_exc {
            std::env::set_var("BIJUX_IMPUTE_EXCLUDE_CHR", v);
        } else {
            std::env::remove_var("BIJUX_IMPUTE_EXCLUDE_CHR");
        }
    }
