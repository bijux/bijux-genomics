...
use super::helpers::{
    normalize_umi_tool_list, ratio_u64, resolve_image_for_run, ExecutionManifest,
};
...
pub fn bench_fastq_umi(
    ...
    for tool in tools {
        ...
        let record = run_umi_tool(catalog, platform, args, &bench_inputs, &tool)?;
    ...
    write_umi_report(&bench_inputs.bench_dir, &records, args.explain)?;
    Ok(())
}
