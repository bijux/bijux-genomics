...
use super::helpers::{delta_metrics, find_first_fastq, DeltaMetrics, ExecutionManifest};
...
pub fn bench_fastq_preprocess(
    ...
    let trim_out_dir = bench_base_dir(&args.out, "trim", &args.sample_id)
        .join("tools")
        .join("fastp")
        .join("out");
    ...
    let correct_out_dir = bench_base_dir(&args.out, "correct", &args.sample_id)
        .join("tools")
        .join("rcorrector")
        .join("out");
    ...
    let filter_out_dir = bench_base_dir(&args.out, "filter", &args.sample_id)
        .join("tools")
        .join("fastp")
        .join("out");
    ...
    let delta_path = out_dir.join("delta_metrics.json");
    fs::write(&delta_path, serde_json::to_vec_pretty(&deltas)?).context("write delta metrics")?;

    let manifest = ExecutionManifest {
        tool: "fastq.preprocess".to_string(),
    ...
    let manifest_path = out_dir.join("execution_manifest.json");
