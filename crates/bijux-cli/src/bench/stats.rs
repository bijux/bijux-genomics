...
use bijux_bench::{
    ...
    ExecutionMetrics, FastqStatsMetrics, LengthHistogramBin, StageMetricSchema,
};
...
use crate::utils::{
    ...
    input_fastq_stats, length_histogram, run_validate_container, SeqkitMetrics,
};

use super::helpers::{normalize_stats_tool_list, resolve_image_for_run, ExecutionManifest};
...
pub fn bench_fastq_stats(
    ...
    args: &crate::cli::BenchFastqStatsArgs,
) -> Result<()> {
    ...
    for tool in tools {
        ...
        let record = run_stats_tool(catalog, platform, args, &bench_inputs, &tool)?;
    ...
    write_stats_report(&bench_inputs.bench_dir, &records, args.explain)?;
    Ok(())
}
