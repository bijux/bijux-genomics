...
#[test]
fn runner_docs_stay_in_root_readme_and_ten_docs_files() {
    ...
    assert_eq!(
        ...
        "runner docs must be one root README plus exactly ten docs/ files"
    );

    let docs_entries = fs::read_dir(root.join("docs"))
        .unwrap_or_else(|err| panic!("read runner docs dir: {err}"))
        .count();
    assert_eq!(docs_entries, 10, "runner docs allowance is ten files");
}
