...
use bijux_engine::primitives::replay;

#[test]
fn replay_regenerates_outputs_from_manifest() -> Result<()> {
    let temp = bijux_infra::temp_dir("bijux-replay")?;
    let root = temp.path();
    ...
    replay::replay_run("run-1", root)?;
    let hash_a = bijux_infra::hash_file_sha256(&output)?;
    std::fs::remove_file(&output)?;
    replay::replay_run("run-1", root)?;
    let hash_b = bijux_infra::hash_file_sha256(&output)?;
