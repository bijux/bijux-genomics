...
`bijux-dna` owns the user-facing CLI process for the genomics workspace. It parses
operator commands, renders CLI output, and delegates planning, reporting, registry, and execution
contracts to stable API surfaces.

## What it must not do (boundaries)
This crate must not own domain science, planner policy, runner backends, stage execution, or hidden
side effects. Direct stage/domain dependencies are not allowed here; the CLI boundary is documented
in [docs/BOUNDARY.md](docs/BOUNDARY.md).
