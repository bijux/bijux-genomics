...
#[test]
fn bench_readiness_operational_benchmark_ready_writes_gate_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-operational-benchmark-ready"])
        .output()
        .expect("run cli");
    ...
    let payload: serde_json::Value = serde_json::from_slice(
        &std::fs::read(repo_root.join(rendered_path.trim())).expect("read operational gate"),
    )
    .expect("parse operational gate");
