...
#[test]
fn bench_readiness_micro_benchmark_execution_ready_writes_gate_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-micro-benchmark-execution-ready"])
        .output()
        .expect("run cli");
    ...
    let payload: serde_json::Value = serde_json::from_slice(
        &std::fs::read(repo_root.join(rendered_path.trim())).expect("read micro execution gate"),
    )
    .expect("parse micro execution gate");
