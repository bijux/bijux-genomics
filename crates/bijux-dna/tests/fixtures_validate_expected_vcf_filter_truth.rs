...
#[test]
fn fixtures_validate_expected_vcf_filter_truth_reports_governed_bundle() {
    let payload =
        run_cli_json(&["fixtures", "validate-expected", "--corpus", "vcf-filter-truth", "--json"]);

    assert_eq!(
        payload.get("schema_version").and_then(serde_json::Value::as_str),
        Some("bijux.bench.vcf_filter_truth.validation.v1")
    );
    assert_eq!(
        payload.get("fixture_id").and_then(serde_json::Value::as_str),
        Some("vcf-filter-truth")
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(&vec![serde_json::json!("vcf.damage_filter"), serde_json::json!("vcf.filter"),])
    );
    assert_eq!(
