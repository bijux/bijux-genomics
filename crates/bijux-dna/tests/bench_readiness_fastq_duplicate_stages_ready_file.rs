...
#[test]
fn bench_readiness_fastq_duplicate_stages_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-fastq-duplicate-stages-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/fastq/duplicate-stages-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read FASTQ duplicate stages readiness report");
    ...
    assert_eq!(report.get("active_row_count").and_then(serde_json::Value::as_u64), Some(3));
        ...
        .and_then(|rows| {
            rows.iter().find(|row| {
                row.get("stage_id").and_then(serde_json::Value::as_str)
                    ...
                    && row.get("tool_id").and_then(serde_json::Value::as_str) == Some("fastuniq")
            })
        })
        .expect("remove_duplicates fastuniq row");
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(
            "duplicate binding `fastq.remove_duplicates` / `fastuniq` keeps active scope, command, output, parser, expected-result, report, and schema proof for `fastq.remove_duplicates`",
        )
    );
    assert!(row.get("expected_outputs").and_then(serde_json::Value::as_array).is_some_and(
        |outputs| {
            outputs.iter().any(|value| value.as_str() == Some("dedup_reads_r1"))
    ...
    assert!(row.get("required_metric_fields").and_then(serde_json::Value::as_array).is_some_and(
        |fields| {
            fields.iter().any(|value| value.as_str() == Some("unique_reads"))
                && fields.iter().any(|value| value.as_str() == Some("output_reads"))
        }
    ));
}
