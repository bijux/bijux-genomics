...
#[test]
fn bench_local_fastq_micro_smoke_subset_writes_governed_summary_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "local", "run-fastq-micro-smoke-subset"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "runs/bench/micro/fastq/MICRO_FASTQ_SUMMARY.json");
    ...
    for row in rows {
        if let Some(support_path) =
            row.get("smoke_support_path").and_then(serde_json::Value::as_str)
        {
            assert!(
