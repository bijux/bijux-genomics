...
fn bench_readiness_all_domain_parser_fixture_coverage_writes_governed_tsv_file() {
    ...
    assert!(rows.iter().any(|row| {
        row == &"fastq\tfastq.trim_reads\ttrimmomatic\tcorpus-01-mini\tcorpus_only\tfastq.adapter.trim_reads\tfastq.parser.trim_reads\tfastq_trim_reads_v2\tparse_trim_reads_report\tbijux.fastq.trim_reads.report.v2\tfixture_case\tfastq.trim_reads.report_json\tfastq_parser_fixture_coverage\tcovered\tactive row `fastq.trim_reads` / `trimmomatic` is governed by FASTQ parser fixture case `fastq.trim_reads.report_json` using parser `parse_trim_reads_report` over canonical `fastp` report_json data"
    }));
    assert!(rows.iter().any(|row| {
