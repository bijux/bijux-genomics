...
#[test]
fn plan_validate_relatedness_segments_pipeline_reports_governed_profile() {
    ...
    let structure = profiles
        ...
        .expect("relatedness-segments profile");
        ...
        structure
            ...
            .is_some_and(|checks| {
                checks.iter().any(|value| {
                    value.as_str()
                        == Some("ibd insufficiency remains local to demography and does not block roh or qc outputs")
                })
            }),
        "plan validate must surface the governed relatedness and segment checks"
    );
}
