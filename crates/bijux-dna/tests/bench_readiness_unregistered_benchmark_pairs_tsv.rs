...
fn bench_readiness_unregistered_benchmark_pairs_writes_governed_tsv_columns() {
    ...
    let tsv_path = repo_root.join("benchmarks/readiness/unregistered-benchmark-pairs.tsv");
    ...
    assert!(
        !rows
            .iter()
            .any(|row| row.starts_with("bam\tbam.genotyping\t")),
        "TSV must not retain a registry-drift row for bam.genotyping once angsd is registered in production"
    );
    assert!(
