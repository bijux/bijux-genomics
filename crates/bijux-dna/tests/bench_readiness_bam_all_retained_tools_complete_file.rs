...
#[test]
fn bench_readiness_bam_all_retained_tools_complete_writes_governed_json_file() {
    let _cwd_guard = support::CWD_LOCK.lock().expect("cwd lock");
    let _env_guard = support::EnvGuard::new().expect("capture env");
    let _crate_root = support::crate_root("bijux-dna").expect("crate root");
    let sandbox =
        support::RepoSandbox::new("bam-retained-tools-completion-file-").expect("repo sandbox");
    ...
    let output = sandbox
        ...
        .args(["bench", "readiness", "render-bam-all-retained-tools-complete"])
        .output()
        .expect("run cli");
    ...
    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read BAM retained-tools completion report");
