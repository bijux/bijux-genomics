...
fn bench_readiness_undercovered_stages_writes_governed_tsv_columns() {
    ...
    let tsv_path = repo_root.join("benchmarks/readiness/undercovered-stages.tsv");
    ...
    assert_eq!(rows.len(), 0, "TSV must retain the governed undercovered-stage row count");
    assert!(
        !rows.iter().any(|row| row.starts_with("bam\tbam.overlap_correction\t")),
        "TSV must not retain an undercovered-stage row for bam.overlap_correction once the registry matches the admitted bamutil-only contract"
    );
    assert!(
        !rows.iter().any(|row| row.starts_with("bam\tbam.endogenous_content\t")),
        "TSV must not retain an undercovered-stage row for bam.endogenous_content"
    );
    assert!(
