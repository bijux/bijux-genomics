...
fn bench_readiness_vcf_damage_filter_ready_reports_complete_active_retained_callers() {
    let payload =
        run_cli_json(&["bench", "readiness", "render-vcf-damage-filter-ready", "--json"]);
    ...
    assert_eq!(
        row.get("parser_fixture_parser_id")
            .and_then(serde_json::Value::as_str),
        Some("parse_bcftools_damage_filter_metrics")
    );
    assert_eq!(
        row.get("parser_fixture_schema_id")
            .and_then(serde_json::Value::as_str),
    ...
    assert_eq!(
        row.get("smoke_damage_manifest_path")
            .and_then(serde_json::Value::as_str),
    ...
    assert_eq!(
        row.get("smoke_parseable").and_then(serde_json::Value::as_bool),
        Some(true)
    );
    assert_eq!(
        row.get("smoke_gt_present").and_then(serde_json::Value::as_bool),
        Some(true)
    );
    assert_eq!(
        row.get("smoke_gl_present").and_then(serde_json::Value::as_bool),
        Some(true)
    );
    assert_eq!(
        row.get("smoke_input_variants")
            .and_then(serde_json::Value::as_u64),
        Some(4)
    );
    assert_eq!(
        row.get("smoke_removed_variants")
            .and_then(serde_json::Value::as_u64),
        Some(2)
    );
    assert_eq!(
        row.get("smoke_retained_variants")
            .and_then(serde_json::Value::as_u64),
        Some(2)
    );
    assert_eq!(
        row.get("smoke_low_quality_filtered_variants")
            .and_then(serde_json::Value::as_u64),
        Some(1)
    );
    assert_eq!(
        row.get("smoke_damage_ratio_filtered_variants")
            .and_then(serde_json::Value::as_u64),
        Some(1)
    );
    assert_eq!(
        row.get("smoke_terminal_damage_filtered_variants")
            .and_then(serde_json::Value::as_u64),
        Some(1)
    );
    assert_eq!(
        row.get("smoke_damage_context_rule")
            .and_then(serde_json::Value::as_str),
        Some("remove_ct_ga_with_ratio_gt_0.35_or_terminal_signal_ge_0.50_or_pmd_lt_3.0")
    );
    assert_eq!(
        row.get("smoke_terminal_context_count")
            .and_then(serde_json::Value::as_u64),
        Some(1)
    );
    assert_eq!(
        row.get("smoke_sample_count").and_then(serde_json::Value::as_u64),
        Some(1)
    );
    assert_eq!(
