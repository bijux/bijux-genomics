...
#[test]
fn bench_readiness_corpus_asset_coverage_gate_file_writes_self_describing_report() {
    let output = run_cli(&["bench", "readiness", "render-corpus-asset-coverage-gate"]);
    assert!(
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/gate-corpus-assets-complete.json");

    let repo_root = support::repo_root().expect("repo root");
    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read corpus asset coverage gate report");
    ...
    assert_eq!(report.get("row_count").and_then(serde_json::Value::as_u64), Some(120));
    assert_eq!(report.get("gate_passed_row_count").and_then(serde_json::Value::as_u64), Some(120));
    ...
    assert_eq!(report.get("rows").and_then(serde_json::Value::as_array).map(Vec::len), Some(120));
}
