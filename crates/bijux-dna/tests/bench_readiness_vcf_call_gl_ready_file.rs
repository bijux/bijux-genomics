...
#[test]
fn bench_readiness_vcf_call_gl_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-vcf-call-gl-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/vcf/call-gl-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read VCF call_gl readiness report");
    ...
    assert!(
        row.get("raw_outputs")
            ...
            .is_some_and(|outputs| outputs.iter().any(|value| {
                value.as_str()
                    == Some(
                        "gl_sites_vcf=benchmarks/readiness/adapters/bcftools/vcf.call_gl/gl_sites_vcf.vcf.gz",
                    )
            }))
    );
    assert!(
        row.get("index_outputs")
            ...
            .is_some_and(|outputs| outputs.iter().any(|value| {
                value.as_str()
                    == Some(
                        "gl_sites_vcf_tbi=benchmarks/readiness/adapters/bcftools/vcf.call_gl/gl_sites_vcf.vcf.gz.tbi",
                    )
            }))
    );
    assert_eq!(
        ...
        Some(
            "active retained VCF GL caller `bcftools` keeps active scope, command, output, parser, expected-result, report, and likelihood-bearing smoke proof for `vcf.call_gl`",
        )
    );
}
