...
#[test]
fn fixtures_validate_expected_amplicon_truth_reports_governed_truth_bundle() {
    let payload =
        run_cli_json(&["fixtures", "validate-expected", "--corpus", "amplicon-truth", "--json"]);

    assert_eq!(
        payload.get("schema_version").and_then(serde_json::Value::as_str),
        Some("bijux.bench.amplicon_truth.validation.v1")
    );
    assert_eq!(
    ...
    assert_eq!(payload.get("validated_section_count").and_then(serde_json::Value::as_u64), Some(8));
    assert_eq!(payload.get("validated_row_count").and_then(serde_json::Value::as_u64), Some(21));
