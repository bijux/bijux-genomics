...
#[test]
fn plan_validate_amplicon_asv_otu_no_vcf_pipeline_reports_governed_profile() {
    ...
    let separation = profiles
        ...
        .expect("amplicon asv otu no-vcf profile");
        ...
        separation
            ...
            .is_some_and(|checks| {
                checks.iter().any(|value| {
                    value.as_str()
                        == Some("fastq.report_qc consumes ASV, OTU, and abundance evidence without mixing bam or vcf germline outputs")
                })
            }),
        "plan validate must surface the governed amplicon-only separation checks"
    );
}
