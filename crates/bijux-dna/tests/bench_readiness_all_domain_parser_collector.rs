...
fn bench_readiness_all_domain_parser_collector_reports_fake_and_real_smoke_rows() {
    ...
    assert_eq!(payload.get("row_count").and_then(serde_json::Value::as_u64), Some(129));
    ...
    assert_eq!(source_kind_counts.get("fake_run").and_then(serde_json::Value::as_u64), Some(125));
    ...
    let fake_result_ids = rows
        ...
        .collect::<BTreeSet<_>>();
    assert_eq!(fake_result_ids.len(), 125);
