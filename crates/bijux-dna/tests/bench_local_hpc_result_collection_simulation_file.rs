...
#[test]
fn bench_local_validate_hpc_result_collection_simulation_prints_validated_report_path() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, report_path) =
        render_path(&repo_root, "validate-hpc-result-collection-simulation-");
    ...
    let validate_output = run_cli(&[
        "bench",
        "local",
        "validate-hpc-result-collection-simulation",
        "--manifest",
        &report_arg,
    ]);
    assert!(
...
#[test]
fn bench_local_validate_hpc_result_collection_simulation_rejects_stale_report_file() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, report_path) =
        render_path(&repo_root, "validate-hpc-result-collection-simulation-stale-");
    let report_arg = report_path.to_string_lossy().into_owned();
    ...
    assert!(
        stderr.contains("result-collection simulation") && stderr.contains("drifted"),
