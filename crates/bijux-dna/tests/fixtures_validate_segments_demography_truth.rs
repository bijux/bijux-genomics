...
#[test]
fn fixtures_validate_segments_demography_truth_reports_governed_bundle() {
    let payload =
        run_cli_json(&["fixtures", "validate", "--corpus", "segments-demography-truth", "--json"]);

    assert_eq!(
        payload.get("schema_version").and_then(serde_json::Value::as_str),
        Some("bijux.bench.segments_demography_truth.validation.v1")
    );
    assert_eq!(
    ...
    assert_eq!(payload.get("validated_case_count").and_then(serde_json::Value::as_u64), Some(9));
    assert_eq!(
        payload.get("validated_stage_ids").and_then(serde_json::Value::as_array),
        Some(&vec![
            serde_json::json!("vcf.demography"),
            serde_json::json!("vcf.ibd"),
            serde_json::json!("vcf.roh"),
        ])
    );
    assert_eq!(
        payload.get("validated_tool_ids").and_then(serde_json::Value::as_array),
        Some(&vec![
            serde_json::json!("germline"),
            serde_json::json!("ibdhap"),
            serde_json::json!("ibdne"),
            serde_json::json!("ibdseq"),
            serde_json::json!("plink2"),
        ])
    );
