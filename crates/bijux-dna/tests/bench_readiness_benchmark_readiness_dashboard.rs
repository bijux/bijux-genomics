...
fn bench_readiness_benchmark_readiness_dashboard_tracks_governed_summary_counts() {
    ...
    assert_eq!(payload.get("expected_pair_count").and_then(serde_json::Value::as_u64), Some(120));
    assert_eq!(payload.get("ready_pair_count").and_then(serde_json::Value::as_u64), Some(120));
    assert_eq!(payload.get("blocked_pair_count").and_then(serde_json::Value::as_u64), Some(0));
    assert_eq!(
        payload.get("blocker_counts").and_then(serde_json::Value::as_object),
    ...
    assert_eq!(matrix.get("tool_count").and_then(serde_json::Value::as_u64), Some(65));
    assert_eq!(matrix.get("surface_status").and_then(serde_json::Value::as_str), Some("complete"));
    ...
    assert_eq!(parsers.get("excluded_pair_count").and_then(serde_json::Value::as_u64), Some(2));
    ...
    assert_eq!(reports.get("tool_section_count").and_then(serde_json::Value::as_u64), Some(65));
    ...
    assert!(blocked_pairs.is_empty(), "FASTQ + BAM readiness dashboard must be green");
}
