...
fn bench_readiness_all_domain_output_declarations_tracks_governed_rows() {
    ...
    assert_eq!(payload.get("complete_row_count").and_then(serde_json::Value::as_u64), Some(125));
    ...
    assert_eq!(status_counts.get("complete").and_then(serde_json::Value::as_u64), Some(125));
