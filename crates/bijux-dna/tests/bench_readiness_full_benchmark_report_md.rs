...
fn bench_readiness_full_benchmark_report_writes_markdown_and_json_outputs() {
    ...
    assert_eq!(rendered_path.trim(), "target/bench-readiness/FASTQ_BAM_VCF_BENCHMARK_REPORT.md");

    let repo_root = support::repo_root().expect("repo root");
    let markdown = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read report markdown");
    let json_path = repo_root.join("target/bench-readiness/FASTQ_BAM_VCF_BENCHMARK_REPORT.json");
    ...
    assert!(markdown.contains("- Report rows: 121"));
    assert!(markdown.contains("- Expected-result rows: 120"));
    ...
    assert_eq!(json_value.get("row_count").and_then(serde_json::Value::as_u64), Some(121));
}
