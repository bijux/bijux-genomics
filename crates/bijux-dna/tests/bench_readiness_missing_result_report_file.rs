...
fn bench_readiness_missing_result_report_writes_self_describing_report() {
    ...
    assert_eq!(rendered_path.trim(), "target/bench-readiness/missing-result-report-test.json");
    ...
    assert_eq!(
        report.get("passes_behavior_test"),
        Some(&serde_json::Value::Bool(true))
    );
    assert_eq!(
        report.get("expected_row_count").and_then(serde_json::Value::as_u64),
        Some(112)
    );
    assert_eq!(
        report
