...
fn bench_readiness_missing_result_report_writes_self_describing_report() {
    ...
    assert_eq!(report.get("expected_row_count").and_then(serde_json::Value::as_u64), Some(116));
    assert_eq!(
    ...
    assert_eq!(report.get("rows").and_then(serde_json::Value::as_array).map(Vec::len), Some(116));
}
