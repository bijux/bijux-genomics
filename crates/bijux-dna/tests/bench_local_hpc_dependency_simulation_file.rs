...
#[test]
fn bench_local_validate_hpc_dependency_simulation_prints_validated_report_path() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, report_path) = render_path(&repo_root, "validate-hpc-dependency-simulation-");
    ...
    let validate_output = run_cli(&[
        "bench",
        "local",
        "validate-hpc-dependency-simulation",
        "--manifest",
        &report_arg,
    ]);
    assert!(
...
#[test]
fn bench_local_validate_hpc_dependency_simulation_rejects_stale_report_file() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, report_path) =
        render_path(&repo_root, "validate-hpc-dependency-simulation-stale-");
    let report_arg = report_path.to_string_lossy().into_owned();
    ...
    let stale_report = fs::read_to_string(&report_path).expect("read report").replacen(
        "\"continued_sibling_job_id\": \"pipeline:relatedness-segments-vcf:vcf.roh\"",
        "\"continued_sibling_job_id\": \"pipeline:relatedness-segments-vcf:vcf.broken\"",
        1,
    );
    ...
    assert!(
        stderr.contains("simulation") && stderr.contains("drifted"),
        "stale report failure must identify simulation drift, got:\n{stderr}"
    );
}
