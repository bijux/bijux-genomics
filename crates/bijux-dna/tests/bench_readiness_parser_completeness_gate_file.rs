...
#[test]
fn bench_readiness_parser_completeness_gate_file_writes_self_describing_report() {
    let output = run_cli(&["bench", "readiness", "render-parser-completeness-gate"]);
    assert!(
    ...
    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read parser completeness gate report");
