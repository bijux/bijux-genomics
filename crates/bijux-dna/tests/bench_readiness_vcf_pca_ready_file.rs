...
#[test]
fn bench_readiness_vcf_pca_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-vcf-pca-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/vcf/pca-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read VCF PCA readiness report");
    ...
    assert_eq!(report.get("retained_row_count").and_then(serde_json::Value::as_u64), Some(2));
    ...
    let observed_tools = rows
        ...
        .collect::<BTreeSet<_>>();
    assert_eq!(observed_tools, BTreeSet::from(["eigensoft", "plink2"]));
    ...
    assert!(rows.iter().all(|row| {
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("vcf.pca")
            ...
            && row.get("smoke_variant_count").and_then(serde_json::Value::as_u64) == Some(2)
