...
#[test]
fn bench_local_validate_hpc_resume_simulation_prints_validated_report_path() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, report_path) = render_path(&repo_root, "validate-hpc-resume-simulation-");
    ...
    let validate_output =
        run_cli(&["bench", "local", "validate-hpc-resume-simulation", "--manifest", &report_arg]);
    assert!(
...
#[test]
fn bench_local_validate_hpc_resume_simulation_rejects_stale_report_file() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, report_path) = render_path(&repo_root, "validate-hpc-resume-simulation-stale-");
    let report_arg = report_path.to_string_lossy().into_owned();
    ...
    assert!(
        stderr.contains("resume simulation") && stderr.contains("drifted"),
