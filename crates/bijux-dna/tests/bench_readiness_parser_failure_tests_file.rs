...
#[test]
fn bench_readiness_parser_failure_tests_file_writes_self_describing_report() {
    let output = run_cli(&["bench", "readiness", "render-parser-failure-tests"]);
    assert!(
    ...
    assert_eq!(rendered_path.trim(), "target/bench-readiness/parser-failure-tests.json");

    let repo_root = support::repo_root().expect("repo root");
    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read parser failure tests report");
