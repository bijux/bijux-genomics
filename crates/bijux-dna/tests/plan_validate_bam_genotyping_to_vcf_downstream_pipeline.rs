...
#[test]
fn plan_validate_bam_genotyping_to_vcf_downstream_pipeline_reports_governed_profile() {
    ...
    let bridge = profiles
        ...
        .expect("bam-genotyping vcf-downstream profile");
    assert_eq!(bridge.get("check_count").and_then(serde_json::Value::as_u64), Some(8));
    assert!(
        bridge
            .get("checks")
            .and_then(serde_json::Value::as_array)
            .is_some_and(|checks| {
                checks.iter().any(|value| {
                    value.as_str()
                        == Some("vcf.filter consumes exact bam.genotyping vcf handoffs instead of a conceptual cohort-vcf placeholder")
                })
            }),
        "plan validate must surface the governed bam-to-vcf bridge checks"
    );
}
