...
fn bench_readiness_fastq_tool_serving_map_writes_governed_tsv_columns() {
    ...
    let tsv_path = repo_root.join("benchmarks/readiness/fastq-tool-serving-map.tsv");
    ...
    assert!(
        rows.iter().any(|row| {
            row == &"seqkit\tfastq.normalize_abundance\tgoverned_benchmark_cohort\trunnable\tbenchmark_normalized\tfixture:corpus-03-amplicon-mini"
        }),
    ...
    for tool_id in ["clumpify", "fastuniq"] {
        assert!(
            rows.iter().any(|row| {
                row == &format!(
                    "{tool_id}\tfastq.remove_duplicates\tgoverned_benchmark_cohort\trunnable\tbenchmark_normalized\tfixture:corpus-01-mini"
                )
            }),
