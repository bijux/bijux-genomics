...
#[test]
fn plan_validate_reference_panel_imputation_pipeline_writes_governed_report_file() {
    ...
    let output_path = repo_root
        .join("benchmarks/readiness/local-ready/pipeline-dag/reference-panel-imputation.json");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["pipeline", "validate", "--id", "reference-panel-imputation", "--strict"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(
        ...
        "benchmarks/readiness/local-ready/pipeline-dag/reference-panel-imputation.json"
    );
