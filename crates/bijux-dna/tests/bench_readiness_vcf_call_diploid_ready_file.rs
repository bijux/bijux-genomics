...
#[test]
fn bench_readiness_vcf_call_diploid_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-vcf-call-diploid-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/vcf/call-diploid-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read VCF call_diploid readiness report");
    ...
    assert!(row.get("report_metric_columns").and_then(serde_json::Value::as_array).is_some_and(
        |columns| {
            columns.iter().any(|value| value.as_str() == Some("ploidy"))
                && columns.iter().any(|value| value.as_str() == Some("called_genotypes"))
    ...
    assert!(
        row.get("raw_outputs")
            ...
            .is_some_and(|outputs| outputs.iter().any(|value| {
                value.as_str()
                    == Some(
                        "diploid_vcf=benchmarks/readiness/adapters/bcftools/vcf.call_diploid/diploid_vcf.vcf.gz",
                    )
            }))
    );
    assert!(
        row.get("index_outputs")
            ...
            .is_some_and(|outputs| outputs.iter().any(|value| {
                value.as_str()
                    == Some(
                        "diploid_vcf_tbi=benchmarks/readiness/adapters/bcftools/vcf.call_diploid/diploid_vcf.vcf.gz.tbi",
                    )
            }))
    );
    assert_eq!(
        ...
        Some(
            "retained VCF diploid caller `bcftools` keeps active scope, command, output, parser, expected-result, report, and diploid genotype proof for `vcf.call_diploid`",
        )
    );
}
