...
#[test]
fn plan_validate_popgen_structure_pipeline_reports_governed_profile() {
    let payload =
        run_cli_json(&["plan", "validate", "--id", "popgen-structure-vcf", "--strict", "--json"]);
    ...
    let structure = profiles
        .iter()
        .find(|profile| {
            profile.get("profile_id").and_then(serde_json::Value::as_str)
                == Some("population_structure_vcf")
        })
        .expect("population-structure profile");
    assert_eq!(structure.get("check_count").and_then(serde_json::Value::as_u64), Some(8));
    assert!(
        structure
            .get("checks")
            .and_then(serde_json::Value::as_array)
            .is_some_and(|checks| {
                checks.iter().any(|value| {
                    value.as_str()
                        == Some("vcf.population_structure consumes the pca and admixture metadata-join handoffs with mandatory labels")
                })
            }),
        "plan validate must surface the governed population-structure checks"
    );
}
