...
#[test]
fn plan_validate_relatedness_segments_pipeline_writes_governed_report_file() {
    ...
    let output_path = repo_root
        .join("benchmarks/readiness/local-ready/pipeline-dag/relatedness-segments-vcf.json");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["pipeline", "validate", "--id", "relatedness-segments-vcf", "--strict"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(
        ...
        "benchmarks/readiness/local-ready/pipeline-dag/relatedness-segments-vcf.json"
    );
