...
fn plan_validate_bam_genotyping_to_vcf_downstream_pipeline_writes_governed_report_file() {
    ...
    let output_path =
        repo_root.join("target/local-ready/pipeline-dag/bam-genotyping-to-vcf-downstream.json");
    ...
    assert_eq!(
        ...
        "target/local-ready/pipeline-dag/bam-genotyping-to-vcf-downstream.json"
    );
