...
#[test]
fn bench_local_adna_micro_pipeline_writes_governed_summary_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "local", "run-adna-micro-pipeline"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "runs/bench/micro/pipelines/adna/MICRO_ADNA_SUMMARY.json");

    let summary_path = repo_root.join(rendered_path.trim());
    assert!(summary_path.is_file(), "aDNA pipeline summary must exist");
    ...
    for row in rows {
        let status = row.get("status").and_then(serde_json::Value::as_str).expect("status");
        let outputs =
        ...
        if status == "succeeded" {
            ...
            assert_eq!(
                ...
                "skipped stages must not claim evidence"
            );
            assert!(outputs.is_empty(), "skipped stages must not declare outputs");
        }
    }
}
