...
#[test]
fn bench_local_validate_hpc_pipeline_node_array_prints_validated_script_path() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, script_path, _manifest_path) =
        render_paths(&repo_root, "validate-hpc-pipeline-node-array-");
    ...
    let validate_output =
        run_cli(&["bench", "local", "validate-hpc-pipeline-node-array", "--script", &script_arg]);
    assert!(
...
#[test]
fn bench_local_validate_hpc_pipeline_node_array_rejects_stale_manifest_file() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, script_path, manifest_path) =
        render_paths(&repo_root, "validate-hpc-pipeline-node-array-manifest-stale-");
    let script_arg = script_path.to_string_lossy().into_owned();
    ...
    let stale_manifest = fs::read_to_string(&manifest_path).expect("read manifest").replacen(
        ...
        "pipeline:core-germline-fastq-bam-vcf:broken",
        1,
    );
...
#[test]
fn bench_local_validate_hpc_pipeline_node_array_rejects_stale_script_file() {
    let repo_root = support::repo_root().expect("repo root");
    let (_temp_dir, script_path, _manifest_path) =
        render_paths(&repo_root, "validate-hpc-pipeline-node-array-script-stale-");
    let script_arg = script_path.to_string_lossy().into_owned();
    ...
    let stale_script = fs::read_to_string(&script_path).expect("read script").replacen(
        "#SBATCH --job-name=bijux-pipeline-node-array",
        "#SBATCH --job-name=bijux-pipeline-node-array-broken",
        1,
    );
