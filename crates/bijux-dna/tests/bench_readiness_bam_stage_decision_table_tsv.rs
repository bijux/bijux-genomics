...
fn bench_readiness_bam_stage_decision_table_writes_governed_tsv_columns() {
    ...
    let tsv_path = repo_root.join("benchmarks/readiness/bam-stage-decision-table.tsv");
    ...
    assert!(
        rows.iter().any(|row| {
            row == &"bam.authenticity\tbenchmark_ready\tauthenticct\tauthenticct\tsupported\trunnable\tparser_fixture_validated\tfixture:corpus-01-adna-damage-mini\tstage `bam.authenticity` is benchmark_ready via `authenticct` with a fixture-backed parser-validated BAM benchmark row"
        }),
    ...
    assert!(
        rows.iter().any(|row| {
            ...
            columns.len() == 9
                ...
                && columns[7] == "fixture:corpus-01-adna-bam-mini"
    ...
    assert!(
        rows.iter().any(|row| {
            row == &"bam.haplogroups\tbenchmark_ready\tyleaf\tyleaf\tsupported\trunnable\tparser_fixture_validated\tfixture:corpus-01-adna-bam-mini\tstage `bam.haplogroups` is benchmark_ready via `yleaf` with a fixture-backed parser-validated BAM benchmark row"
        }),
        "TSV must retain the governed benchmark-ready bam.haplogroups row"
    );
    assert!(
        rows.iter().any(|row| {
            row == &"bam.kinship\tbenchmark_ready\tking\tking\tsupported\trunnable\tparser_fixture_validated\tfixture:corpus-01-kinship-mini\tstage `bam.kinship` is benchmark_ready via `king` with a fixture-backed parser-validated BAM benchmark row"
        }),
    ...
    assert!(
        rows.iter().any(|row| {
            row == &"bam.genotyping\tbenchmark_ready\tangsd\tangsd\tsupported\trunnable\tparser_fixture_validated\tfixture:corpus-01-genotyping-mini\tstage `bam.genotyping` is benchmark_ready via `angsd` with a fixture-backed parser-validated BAM benchmark row"
        }),
