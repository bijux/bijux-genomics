...
#[test]
fn bench_readiness_vcf_population_structure_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-vcf-population-structure-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/vcf/population-structure-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read VCF population-structure readiness report");
    ...
    assert!(rows.iter().all(|row| {
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("vcf.population_structure")
            ...
            && row.get("smoke_sample_group_count").and_then(serde_json::Value::as_u64) == Some(4)
            && row.get("smoke_output_root").and_then(serde_json::Value::as_str).is_some_and(
                |value| value == "runs/bench/local-smoke/vcf.population_structure/plink2",
            )
            && row
                .get("smoke_population_structure_json_path")
