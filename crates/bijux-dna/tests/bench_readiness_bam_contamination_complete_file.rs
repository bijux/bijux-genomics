...
#[test]
fn bench_readiness_bam_contamination_complete_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-bam-contamination-complete"])
        .output()
        .expect("run cli");
    ...
    let payload = fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read BAM contamination completion JSON");
    let report: serde_json::Value =
        serde_json::from_str(&payload).expect("parse BAM contamination completion JSON");
    ...
    assert_eq!(
        report.get("local_smoke_sample_id").and_then(serde_json::Value::as_str),
