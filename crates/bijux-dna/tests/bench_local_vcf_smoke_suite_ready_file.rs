...
#[test]
fn bench_local_validate_vcf_smoke_suite_ready_writes_governed_json_file() {
    let output = run_cli(&["bench", "local", "validate-vcf-smoke-suite-ready"]);
    assert!(
    ...
    assert_eq!(parsed.get("checked_goal_count").and_then(serde_json::Value::as_u64), Some(19));
