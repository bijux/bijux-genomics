...
fn bench_readiness_full_benchmark_result_collector_merges_all_governed_surfaces() {
    ...
    assert_eq!(payload.get("row_count").and_then(serde_json::Value::as_u64), Some(583));
    assert_eq!(
    ...
    assert_eq!(payload.get("fake_run_row_count").and_then(serde_json::Value::as_u64), Some(121));
    assert_eq!(
    ...
    let surface_kind_counts = payload
        .get("surface_kind_counts")
    ...
    assert_eq!(surface_kind_counts.get("fake_run").and_then(serde_json::Value::as_u64), Some(121));
    assert_eq!(
    ...
    let result_status_counts = payload
        .get("result_status_counts")
        .and_then(serde_json::Value::as_object)
        .expect("result status counts");
    assert_eq!(result_status_counts.get("expected").and_then(serde_json::Value::as_u64), Some(121));
    assert_eq!(
        result_status_counts.get("succeeded").and_then(serde_json::Value::as_u64),
        Some(218)
    );
    assert_eq!(result_status_counts.get("failed").and_then(serde_json::Value::as_u64), Some(121));
    assert_eq!(result_status_counts.get("present").and_then(serde_json::Value::as_u64), Some(118));
    assert_eq!(
    ...
    assert_eq!(domain_counts.get("vcf").and_then(serde_json::Value::as_u64), Some(73));

    let rows = payload.get("rows").and_then(serde_json::Value::as_array).expect("rows array");
    assert_eq!(rows.len(), 583);

    let record_ids = rows
        .iter()
        .filter_map(|row| row.get("record_id").and_then(serde_json::Value::as_str))
        .collect::<BTreeSet<_>>();
    assert_eq!(record_ids.len(), 583);
