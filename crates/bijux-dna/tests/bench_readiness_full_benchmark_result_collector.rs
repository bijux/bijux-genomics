...
fn bench_readiness_full_benchmark_result_collector_merges_all_governed_surfaces() {
    ...
    assert_eq!(payload.get("row_count").and_then(serde_json::Value::as_u64), Some(599));
    assert_eq!(
    ...
    assert_eq!(payload.get("fake_run_row_count").and_then(serde_json::Value::as_u64), Some(125));
    assert_eq!(
    ...
    assert_eq!(surface_kind_counts.get("fake_run").and_then(serde_json::Value::as_u64), Some(125));
    assert_eq!(
    ...
    assert_eq!(result_status_counts.get("expected").and_then(serde_json::Value::as_u64), Some(125));
    assert_eq!(
        result_status_counts.get("succeeded").and_then(serde_json::Value::as_u64),
        Some(222)
    );
    assert_eq!(result_status_counts.get("failed").and_then(serde_json::Value::as_u64), Some(125));
    assert_eq!(result_status_counts.get("present").and_then(serde_json::Value::as_u64), Some(122));
    assert_eq!(
    ...
    assert_eq!(domain_counts.get("vcf").and_then(serde_json::Value::as_u64), Some(89));
