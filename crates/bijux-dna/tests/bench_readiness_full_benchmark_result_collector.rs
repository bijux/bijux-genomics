...
fn bench_readiness_full_benchmark_result_collector_merges_all_governed_surfaces() {
    ...
    assert_eq!(surface_kind_counts.get("fake_run").and_then(serde_json::Value::as_u64), Some(120));
    assert_eq!(
    ...
    assert_eq!(result_status_counts.get("expected").and_then(serde_json::Value::as_u64), Some(120));
    assert_eq!(
        result_status_counts.get("succeeded").and_then(serde_json::Value::as_u64),
        Some(217)
    );
    assert_eq!(result_status_counts.get("failed").and_then(serde_json::Value::as_u64), Some(120));
    assert_eq!(result_status_counts.get("present").and_then(serde_json::Value::as_u64), Some(117));
    assert_eq!(
