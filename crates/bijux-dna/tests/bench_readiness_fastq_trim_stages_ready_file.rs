...
#[test]
fn bench_readiness_fastq_trim_stages_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-fastq-trim-stages-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/fastq/trim-stages-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read FASTQ trim stages readiness report");
    ...
    assert_eq!(report.get("active_row_count").and_then(serde_json::Value::as_u64), Some(18));
    assert_eq!(report.get("complete_row_count").and_then(serde_json::Value::as_u64), Some(18));
    ...
    let row = report
        ...
        .expect("trim_reads adapterremoval row");
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(
            "trim binding `fastq.trim_reads` / `adapterremoval` keeps active scope, command, output, parser, expected-result, report, and schema proof for `fastq.trim_reads`",
        )
    );
    assert!(row.get("expected_outputs").and_then(serde_json::Value::as_array).is_some_and(
        |outputs| {
            outputs.iter().any(|value| value.as_str() == Some("trimmed_reads_r1"))
    ...
    assert!(row.get("required_metric_fields").and_then(serde_json::Value::as_array).is_some_and(
        |fields| {
            fields.iter().any(|value| value.as_str() == Some("reads_retained"))
                && fields.iter().any(|value| value.as_str() == Some("bases_removed"))
        }
    ));
}
