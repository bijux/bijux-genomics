...
#[test]
fn bench_local_vcf_call_smoke_reports_real_governed_outputs() {
    let payload = run_cli_json(&["bench", "local", "run-vcf-call-smoke", "--json"]);

    assert_eq!(
        payload.get("schema_version").and_then(serde_json::Value::as_str),
        Some("bijux.bench.local_vcf_call_smoke.v1")
    );
    assert_eq!(
        payload.get("command").and_then(serde_json::Value::as_str),
        Some("bijux-dna bench local run-vcf-call-smoke --tool-id bcftools")
    );
    assert_eq!(payload.get("stage_id").and_then(serde_json::Value::as_str), Some("vcf.call"));
    assert_eq!(
        payload.get("resolved_stage_id").and_then(serde_json::Value::as_str),
    ...
    assert_eq!(
        ...
        Some("runs/bench/local-smoke/vcf.call/bcftools/artifacts/reference/corpus_01_bam_reference.fasta")
    );
    assert_eq!(
        payload.get("output_root").and_then(serde_json::Value::as_str),
        Some("runs/bench/local-smoke/vcf.call/bcftools")
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some("runs/bench/local-smoke/vcf.call/bcftools/calls.vcf.gz.tbi")
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some("runs/bench/local-smoke/vcf.call/bcftools/stage-result.json")
    );
    assert_eq!(payload.get("exit_code").and_then(serde_json::Value::as_i64), Some(0));
    assert_eq!(payload.get("variant_count").and_then(serde_json::Value::as_u64), Some(3));
    assert_eq!(payload.get("snp_count").and_then(serde_json::Value::as_u64), Some(3));
    assert_eq!(payload.get("indel_count").and_then(serde_json::Value::as_u64), Some(0));
    ...
    assert_eq!(metrics.get("variant_count").and_then(serde_json::Value::as_u64), Some(3));
