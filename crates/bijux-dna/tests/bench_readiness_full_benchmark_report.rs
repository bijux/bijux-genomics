...
fn bench_readiness_full_benchmark_report_tracks_governed_report_sections() {
    ...
    assert_eq!(payload.get("present_row_count").and_then(serde_json::Value::as_u64), Some(118));
    assert_eq!(
    ...
    assert_eq!(payload.get("failure_row_count").and_then(serde_json::Value::as_u64), Some(121));
    ...
    assert_eq!(status_counts.get("present").and_then(serde_json::Value::as_u64), Some(118));
