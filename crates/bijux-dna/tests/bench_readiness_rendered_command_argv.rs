...
fn bench_readiness_render_command_argv_reports_governed_benchmark_ready_row_slice() {
    ...
    assert_eq!(
        ...
        Some("bijux.bench.readiness.rendered_command_argv.v1")
    );
    assert_eq!(
    ...
    assert_eq!(payload.get("rows").and_then(serde_json::Value::as_array).map(Vec::len), Some(120));
    assert!(payload
        .get("rows")
        ...
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("fastq.index_reference")
            ...
            && row.get("argv").and_then(serde_json::Value::as_array).is_some_and(|argv| {
                argv.first().and_then(serde_json::Value::as_str) == Some("sh")
                    && argv
                        .iter()
                        .any(|arg| arg.as_str().is_some_and(|item| item.contains("bowtie2-build")))
            })
    }));
    assert!(rows.iter().any(|row| {
