...
fn bench_local_render_all_domain_slurm_scripts_reports_governed_job_counts() {
    ...
    assert_eq!(payload.get("benchmark_job_count").and_then(serde_json::Value::as_u64), Some(141));
    assert_eq!(
    ...
    assert_eq!(domain_counts.get("fastq").and_then(serde_json::Value::as_u64), Some(100));
    assert_eq!(domain_counts.get("bam").and_then(serde_json::Value::as_u64), Some(80));
    assert_eq!(domain_counts.get("vcf").and_then(serde_json::Value::as_u64), Some(54));

    let benchmark_domain_counts = payload
        .get("benchmark_domain_counts")
        .and_then(serde_json::Value::as_object)
        .expect("benchmark domain counts");
    assert_eq!(benchmark_domain_counts.get("fastq").and_then(serde_json::Value::as_u64), Some(71));
    assert_eq!(benchmark_domain_counts.get("bam").and_then(serde_json::Value::as_u64), Some(49));
    assert_eq!(benchmark_domain_counts.get("vcf").and_then(serde_json::Value::as_u64), Some(21));
