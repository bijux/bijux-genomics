...
fn bench_local_vcf_call_pseudohaploid_smoke_writes_governed_files() {
    ...
    let repo_root = support::repo_root().expect("repo root");
    let output_vcf =
        repo_root.join("target/local-smoke/vcf.call_pseudohaploid/bcftools/pseudohaploid.vcf.gz");
    let output_tbi = repo_root
        .join("target/local-smoke/vcf.call_pseudohaploid/bcftools/pseudohaploid.vcf.gz.tbi");
    let metrics_path =
        repo_root.join("target/local-smoke/vcf.call_pseudohaploid/bcftools/metrics.json");
    let manifest_path =
        repo_root.join("target/local-smoke/vcf.call_pseudohaploid/bcftools/stage-result.json");
