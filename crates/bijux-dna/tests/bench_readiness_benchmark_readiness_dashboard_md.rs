...
fn bench_readiness_benchmark_readiness_dashboard_writes_markdown_and_json_outputs() {
    ...
    assert_eq!(rendered_path.trim(), "target/bench-readiness/FASTQ_BAM_BENCHMARK_READINESS.md");

    let repo_root = support::repo_root().expect("repo root");
    let markdown = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read dashboard markdown");
    let json_path = repo_root.join("target/bench-readiness/FASTQ_BAM_BENCHMARK_READINESS.json");
    ...
    assert!(markdown.contains("| stage_centric_report | target/bench-readiness/stage-centric-report.md | 51 stage_sections |"));
