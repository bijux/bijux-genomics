...
#[test]
fn bench_local_cross_domain_sample_consistency_writes_governed_json_file() {
    let output = run_cli(&["bench", "local", "validate-cross-domain-sample-consistency"]);
    assert!(
    ...
    let repo_root = support::repo_root().expect("repo root");
    let report_path =
        repo_root.join("benchmarks/readiness/science/cross-domain-sample-consistency.json");
    ...
    assert_eq!(
        ...
        Some("bijux.bench.cross_domain_sample_consistency.v1")
    );
    ...
    assert_eq!(
        payload.get("vcf_reference_id").and_then(serde_json::Value::as_str),
