...
#[test]
fn bench_local_vcf_call_smoke_writes_governed_files() {
    let output = run_cli(&["bench", "local", "run-vcf-call-smoke"]);
    assert!(
    ...
    assert_eq!(manifest.get("stage_id").and_then(serde_json::Value::as_str), Some("vcf.call"));
    assert_eq!(
