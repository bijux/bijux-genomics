...
#[test]
fn plan_validate_adna_pipeline_reports_governed_profile() {
        ...
        ancient_dna
            ...
            .is_some_and(|checks| {
                checks.iter().any(|value| {
                    value.as_str()
                        == Some("vcf.call_pseudohaploid consumes BAM damage and authenticity evidence with ancient-DNA reference contracts")
                })
            }),
        "plan validate must surface the governed ancient-DNA specialization checks"
    );
}
