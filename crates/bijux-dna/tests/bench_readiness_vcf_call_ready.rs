...
#[test]
fn bench_readiness_vcf_call_ready_reports_complete_retained_callers() {
    let payload = run_cli_json(&["bench", "readiness", "render-vcf-call-ready", "--json"]);
    ...
    assert_eq!(
        ...
        Some("benchmarks/readiness/vcf/call-ready.json")
    );
    ...
    assert_eq!(
        row.get("retained_scope_detail").and_then(serde_json::Value::as_str),
        Some("active")
    );
    assert_eq!(
        row.get("retained_scope_proof_path").and_then(serde_json::Value::as_str),
    ...
    assert_eq!(
        row.get("output_proof_path").and_then(serde_json::Value::as_str),
    ...
    assert_eq!(
        ...
        Some("benchmarks/tests/fixtures/bench/parsers/vcf/bcftools/vcf.call")
    );

    assert_eq!(row.get("expected_result_ready").and_then(serde_json::Value::as_bool), Some(true));
    assert_eq!(
        row.get("expected_result_proof_path").and_then(serde_json::Value::as_str),
    ...
    assert_eq!(row.get("smoke_variant_count").and_then(serde_json::Value::as_u64), Some(3));
    assert_eq!(row.get("smoke_snp_count").and_then(serde_json::Value::as_u64), Some(3));
    assert_eq!(row.get("smoke_indel_count").and_then(serde_json::Value::as_u64), Some(0));
