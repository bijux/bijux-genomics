...
fn run_cli_json() -> serde_json::Value {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-bam-contamination-complete", "--json"])
        .output()
        .expect("run cli");
...
#[test]
fn bench_readiness_bam_contamination_complete_reports_governed_rows() {
    let payload = run_cli_json();

    assert_eq!(
        payload.get("schema_version").and_then(serde_json::Value::as_str),
        Some("bijux.bench.readiness.bam_contamination_complete.v1")
    );
    assert_eq!(
        payload.get("output_path").and_then(serde_json::Value::as_str),
        Some("benchmarks/readiness/bam/stages/bam.contamination.complete.json")
    );
    ...
    let verifybamid2_row = rows
        .iter()
        .find(|row| row.get("tool_id").and_then(serde_json::Value::as_str) == Some("verifybamid2"))
        .expect("verifybamid2 row");
    assert_eq!(
        verifybamid2_row
            .get("ready_case_summary_schema_version")
            .and_then(serde_json::Value::as_str),
        Some("bijux.bam.contamination_evidence.v1")
    );
    assert_eq!(
        verifybamid2_row
            .get("insufficient_case_summary_schema_version")
            .and_then(serde_json::Value::as_str),
        Some("bijux.bam.contamination_evidence.v1")
    );
    assert_eq!(
        verifybamid2_row
            .get("ready_case_stage_metrics_schema_version")
            .and_then(serde_json::Value::as_str),
        Some("bijux.bam.contamination.stage_metrics.v1")
    );
    assert_eq!(
        verifybamid2_row
    ...
    assert_eq!(
        verifybamid2_row
            .get("ready_case_prerequisites_passed")
            .and_then(serde_json::Value::as_bool),
        Some(true)
    );
    assert_eq!(
        verifybamid2_row
            .get("insufficient_case_prerequisites_passed")
            .and_then(serde_json::Value::as_bool),
        Some(false)
    );
    assert!(verifybamid2_row
        .get("insufficient_case_refusal_codes")
