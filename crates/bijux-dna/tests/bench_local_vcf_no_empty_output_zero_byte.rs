...
fn bench_local_validate_vcf_no_empty_output_rejects_zero_byte_artifact() {
    ...
    let sandbox =
        support::RepoSandbox::new("vcf-no-empty-output-zero-byte-").expect("repo sandbox");
    ...
    let initial = run_cli(
        &sandbox,
        home.path(),
        &["bench", "local", "validate-vcf-no-empty-output", "--json"],
    );
    assert!(
    ...
    let failing = run_cli(
        &sandbox,
        home.path(),
        &["bench", "local", "validate-vcf-no-empty-output", "--skip-refresh", "--json"],
    );
    assert!(
