...
#[test]
fn plan_validate_adna_pipeline_writes_governed_report_file() {
    ...
    let output_path =
        repo_root.join("target/local-ready/pipeline-dag/adna-pseudohaploid-fastq-bam-vcf.json");
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["pipeline", "validate", "--id", "adna-pseudohaploid-fastq-bam-vcf", "--strict"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(
        ...
        "target/local-ready/pipeline-dag/adna-pseudohaploid-fastq-bam-vcf.json"
    );
