...
#[test]
fn fixtures_validate_fastq_taxonomy_truth_reports_governed_taxonomy_bundle() {
    let payload =
        run_cli_json(&["fixtures", "validate", "--corpus", "fastq-taxonomy-truth", "--json"]);

    assert_eq!(
        payload.get("schema_version").and_then(serde_json::Value::as_str),
        Some("bijux.bench.fastq_taxonomy_truth.validation.v1")
    );
    assert_eq!(
    ...
    assert_eq!(
        payload.get("validated_taxa_row_count").and_then(serde_json::Value::as_u64),
        Some(6)
    );
