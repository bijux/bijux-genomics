...
fn bench_readiness_vcf_expected_benchmark_results_tracks_governed_rows() {
    ...
    assert!(rows.iter().any(|row| {
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("vcf.admixture")
            ...
            && row.get("expected_outputs").and_then(serde_json::Value::as_array).is_some_and(
                |items| items.iter().any(|item| item.as_str() == Some("admixture_report")),
            )
            && row
                .get("expected_metrics")
                .and_then(serde_json::Value::as_array)
                .is_some_and(|items| items.iter().any(|item| item.as_str() == Some("selected_k")))
    ...
    assert!(rows.iter().any(|row| {
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("vcf.population_structure")
            ...
            && row.get("expected_outputs").and_then(serde_json::Value::as_array).is_some_and(
                |items| {
                    items.iter().any(|item| item.as_str() == Some("population_structure_report"))
                },
            )
            && row
                .get("expected_metrics")
    ...
    assert!(rows.iter().any(|row| {
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("vcf.impute")
            ...
            && row
                .get("expected_outputs")
                .and_then(serde_json::Value::as_array)
                .is_some_and(|items| items.iter().any(|item| item.as_str() == Some("imputed_vcf")))
            && row.get("expected_metrics").and_then(serde_json::Value::as_array).is_some_and(
                |items| items.iter().any(|item| item.as_str() == Some("masked_truth_match_count")),
            )
            && row.get("report_section").and_then(serde_json::Value::as_str) == Some("imputation")
    }));
    assert!(rows.iter().any(|row| {
    ...
    assert!(rows.iter().any(|row| {
        row.get("stage_id").and_then(serde_json::Value::as_str) == Some("vcf.roh")
            ...
            && row
                .get("comparable_metrics")
                .and_then(serde_json::Value::as_array)
                .is_some_and(|items| items.iter().any(|item| item.as_str() == Some("segment_count")))
            && row.get("report_section").and_then(serde_json::Value::as_str)
