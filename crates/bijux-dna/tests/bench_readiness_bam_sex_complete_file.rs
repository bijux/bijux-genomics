...
#[test]
fn bench_readiness_bam_sex_complete_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-bam-sex-complete"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/bam/stages/bam.sex.complete.json");

    let payload = fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read BAM sex completion JSON");
    let report: serde_json::Value =
        serde_json::from_str(&payload).expect("parse BAM sex completion JSON");
    ...
    assert_eq!(
        report.get("local_smoke_ready_sample_id").and_then(serde_json::Value::as_str),
        Some("adna_xy_autosome_coverage")
    );
    assert_eq!(
        report.get("local_smoke_insufficient_sample_id").and_then(serde_json::Value::as_str),
