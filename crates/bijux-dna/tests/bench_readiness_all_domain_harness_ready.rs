...
fn bench_readiness_all_domain_harness_ready_reports_governed_pass_state() {
    ...
    assert_eq!(payload.get("fake_run_result_count").and_then(serde_json::Value::as_u64), Some(126));
    assert_eq!(payload.get("fake_run_output_count").and_then(serde_json::Value::as_u64), Some(513));
    assert_eq!(
    ...
    assert_eq!(payload.get("completion_row_count").and_then(serde_json::Value::as_u64), Some(126));
    assert_eq!(
