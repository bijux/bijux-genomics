...
#[test]
fn bench_readiness_bam_haplogroups_complete_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-bam-haplogroups-complete"])
        .output()
        .expect("run cli");
    ...
    let payload = fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read BAM haplogroups completion JSON");
    let report: serde_json::Value =
        serde_json::from_str(&payload).expect("parse BAM haplogroups completion JSON");
    ...
    assert!(report.get("required_output_ids").and_then(serde_json::Value::as_array).is_some_and(
        |outputs| {
            outputs.iter().any(|output| output.as_str() == Some("haplogroups"))
