...
#[test]
fn bench_readiness_vcf_filter_ready_writes_governed_json_file() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_bijux-dna"))
        ...
        .args(["bench", "readiness", "render-vcf-filter-ready"])
        .output()
        .expect("run cli");
    ...
    assert_eq!(rendered_path.trim(), "benchmarks/readiness/vcf/filter-ready.json");

    let payload = std::fs::read_to_string(repo_root.join(rendered_path.trim()))
        .expect("read VCF filter readiness report");
    ...
    assert!(row.get("report_metric_columns").and_then(serde_json::Value::as_array).is_some_and(
        |columns| {
            columns.iter().any(|value| value.as_str() == Some("input_variants"))
                && columns.iter().any(|value| value.as_str() == Some("pass_variants"))
                && columns.iter().any(|value| value.as_str() == Some("failed_variants"))
                && columns.iter().any(|value| value.as_str() == Some("filter_ids"))
        }
    ));
    assert!(
        row.get("raw_outputs")
            ...
            .is_some_and(|outputs| outputs.iter().any(|value| {
                value.as_str()
                    == Some(
                        "filtered_vcf=benchmarks/readiness/adapters/bcftools/vcf.filter/filtered_vcf.vcf.gz",
                    )
            }))
    );
    assert!(
        row.get("index_outputs")
            ...
            .is_some_and(|outputs| outputs.iter().any(|value| {
                value.as_str()
                    == Some(
                        "filtered_vcf_tbi=benchmarks/readiness/adapters/bcftools/vcf.filter/filtered_vcf.vcf.gz.tbi",
                    )
            }))
    );
    assert_eq!(
        ...
        Some(
            "active retained VCF filter caller `bcftools` keeps active scope, command, output, parser, expected-result, report, and threshold-normalized smoke proof for `vcf.filter`",
        )
    );
}
