...
fn bench_readiness_vcf_active_stage_tool_matrix_reports_every_retained_binding() {
    ...
    assert_eq!(payload.get("active_row_count").and_then(serde_json::Value::as_u64), Some(15));
    assert_eq!(payload.get("complete_row_count").and_then(serde_json::Value::as_u64), Some(0));
    assert_eq!(payload.get("removed_row_count").and_then(serde_json::Value::as_u64), Some(32));

    let scope_state_counts = payload
        .get("scope_state_counts")
        .and_then(serde_json::Value::as_object)
        .expect("scope state counts");
    assert_eq!(scope_state_counts.get("active").and_then(serde_json::Value::as_u64), Some(15));
    assert_eq!(
    ...
    assert_eq!(scope_detail_counts.get("active").and_then(serde_json::Value::as_u64), Some(15));
    assert_eq!(
