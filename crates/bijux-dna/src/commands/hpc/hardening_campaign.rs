...
fn goal_specific_checks(
    ...
    match goal_id {
        ...
        "G256" => vec![
            format!("image_dossier_rows_present={}", !rows.is_empty()),
            format!(
                "image_dossier_stage_count={}",
                rows.iter().map(|row| row.stage_id.clone()).collect::<BTreeSet<_>>().len()
            ),
            format!(
                "image_dossier_trim_summary_stats_bound={}",
                rows.iter().any(|row| row.stage_id == "fastq.trim_reads")
                    && rows.iter().any(|row| row.stage_id == "bam.mapping_summary")
                    && rows.iter().any(|row| row.stage_id == "vcf.stats")
            ),
            format!(
                "image_dossier_non_ready_rows={}",
                rows.iter().filter(|row| row.readiness_class != "ready").count()
            ),
            format!(
                "image_dossier_drift_findings={}",
                findings
                    .iter()
                    .filter(|finding| {
        ...
        "G277" => vec![
            format!("local_archive_rows_present={}", !rows.is_empty()),
            format!(
                "local_archive_stage_count={}",
                rows.iter().map(|row| row.stage_id.clone()).collect::<BTreeSet<_>>().len()
            ),
            format!(
                "local_archive_profile_coverage_postprocess_bound={}",
                rows.iter().any(|row| row.stage_id == "fastq.profile_reads")
                    && rows.iter().any(|row| row.stage_id == "bam.coverage")
                    && rows.iter().any(|row| row.stage_id == "vcf.postprocess")
            ),
            format!(
                "local_archive_encrypted_findings={}",
                findings
                    .iter()
                    .filter(|finding| finding.result_scope.starts_with("encrypted-"))
                    .count()
...
mod tests {
    ...
    fn goal_256_emits_image_dossier_checks() {
        ...
        assert!(entries[0]
            .goal_checks
            .iter()
            .any(|check| check == "image_dossier_trim_summary_stats_bound=true"));
        assert!(entries[0]
            .goal_checks
            .iter()
    ...
    fn goal_277_emits_local_archive_checks() {
        ...
        assert!(entries[0]
            .goal_checks
            .iter()
            .any(|check| check == "local_archive_profile_coverage_postprocess_bound=true"));
        assert!(entries[0]
            .goal_checks
            .iter()
