...
fn ensure_fake_failure_row_alignment(
    ...
    if expected_rows.len() != 120 || command_rows.len() != 120 || output_rows.len() != 120 {
        return Err(anyhow!(
            "all-domain fake-failure runner requires exactly 120 expected-result, command, and output rows"
        ));
    }
...
fn ensure_all_domain_fake_failure_contract(report: &AllDomainFakeFailuresManifest) -> Result<()> {
    if report.result_count != 120 || report.failures.len() != 120 {
        return Err(anyhow!(
            "all-domain fake-failure runner must cover exactly 120 governed benchmark-ready results"
        ));
    }
