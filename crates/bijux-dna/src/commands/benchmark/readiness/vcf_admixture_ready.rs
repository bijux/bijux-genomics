...
use crate::commands::benchmark::local_vcf_admixture_smoke::{
    run_local_vcf_admixture_smoke, LocalVcfAdmixtureSmokeReport, LocalVcfAdmixtureSmokeRow,
};
use crate::commands::cli::parse;
use crate::commands::cli::render;

pub(crate) const DEFAULT_VCF_ADMIXTURE_READY_PATH: &str =
...
const REQUIRED_POPULATION_COUNT: u64 = 2;
const REQUIRED_SAMPLE_IDS: [&str; 4] = ["sample_a", "sample_b", "sample_c", "sample_d"];
const REQUIRED_CLUSTER_HEADERS: [&str; 2] = ["cluster_1", "cluster_2"];
...
const VCF_ADMIXTURE_READY_LOCK_PATH: &str =
    "artifacts/bench-readiness/vcf-admixture-ready/render.lock";

#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfAdmixtureReadyRow {
    ...
    pub(crate) smoke_admixture_tsv_path: String,
    pub(crate) smoke_admixture_json_path: String,
    pub(crate) smoke_source_q_matrix_path: String,
    pub(crate) smoke_source_k_selection_path: String,
    ...
    pub(crate) smoke_selected_k: usize,
    pub(crate) smoke_status: String,
    pub(crate) smoke_sample_count: u64,
    pub(crate) smoke_population_count: u64,
    pub(crate) smoke_cluster_headers: Vec<String>,
    pub(crate) smoke_rows: Vec<LocalVcfAdmixtureSmokeRow>,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfAdmixtureReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<VcfAdmixtureReadyRow>,
    pub(crate) violations: Vec<VcfAdmixtureReadyRow>,
}

pub(crate) fn run_render_vcf_admixture_ready(
    args: &parse::BenchReadinessRenderVcfAdmixtureReadyArgs,
) -> Result<()> {
...
pub(crate) fn render_vcf_admixture_ready(
    repo_root: &Path,
    output_path: PathBuf,
    ...
    .with_context(|| {
        format!(
            "acquire VCF admixture readiness lock {}",
            repo_root.join(VCF_ADMIXTURE_READY_LOCK_PATH).display()
        )
    })?;
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_vcf_admixture_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "vcf.admixture active retained callers must stay complete across active-scope, command, output, parser, report, and ancestry-sum smoke proof"
        ));
    }
    Ok(report)
}

fn build_vcf_admixture_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<VcfAdmixtureReadyReport> {
    let (command_report, bindings) =
        collect_vcf_stage_readiness_bindings(repo_root, VCF_ADMIXTURE_STAGE_ID)?;
    ...
    let report = VcfAdmixtureReadyReport {
        schema_version: VCF_ADMIXTURE_READY_SCHEMA_VERSION,
    ...
    ensure_vcf_admixture_ready_contract(&report)?;
    Ok(report)
}

fn build_vcf_admixture_ready_row(
    command_report: &VcfRenderedCommandsReport,
    binding: VcfStageReadinessBinding,
    smoke_report: Option<&LocalVcfAdmixtureSmokeReport>,
) -> VcfAdmixtureReadyRow {
    ...
    let output_ready = binding.output_row.as_ref().is_some_and(|row| {
        row.status == VcfAdapterOutputCoverageStatus::Complete
            && row.benchmark_status == "benchmark_ready"
            && contains_artifact_id(&row.normalized_metrics, "admixture_report")
            && contains_artifact_id(&row.raw_outputs, "admixture_proxy_eigenvec")
            && contains_artifact_id(&row.raw_outputs, "admixture_proxy_eigenval")
    ...
    let expected_result_ready = binding.expected_row.as_ref().is_some_and(|row| {
        row.report_section == "population_structure"
            && row.expected_outputs.iter().any(|value| value == "admixture_report")
    ...
    let smoke_ready = smoke_report.is_some_and(|report| {
        !report.output_root.trim().is_empty()
            ...
            && !report.admixture_tsv_path.trim().is_empty()
            && !report.admixture_json_path.trim().is_empty()
            && !report.source_q_matrix_path.trim().is_empty()
            && !report.source_k_selection_path.trim().is_empty()
            && !report.source_logs_path.trim().is_empty()
            && !report.stage_result_manifest_path.trim().is_empty()
            && !report.execution_mode.trim().is_empty()
            && report.status == REQUIRED_STATUS
            && report.selected_k == REQUIRED_SELECTED_K
    ...
    if !smoke_ready {
        missing_surfaces.push("local_vcf_admixture_smoke".to_string());
    }
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "`{}` / `{}` stays active with governed adapter, parser, report, and ancestry-sum smoke proof",
    ...
    VcfAdmixtureReadyRow {
        ...
        smoke_cluster_headers: smoke_report
            .map(|report| report.cluster_headers.clone())
            .unwrap_or_default(),
...
fn ensure_vcf_admixture_ready_contract(report: &VcfAdmixtureReadyReport) -> Result<()> {
    ...
    {
        return Err(anyhow!(
            "vcf.admixture readiness must retain exactly one complete active governed row"
        ));
    }
    let row = report.rows.first().ok_or_else(|| {
        anyhow!("vcf.admixture readiness must publish the active governed admixture row")
    })?;
    if row.stage_id != VCF_ADMIXTURE_STAGE_ID
        || row.tool_id != "plink2"
    ...
    {
        return Err(anyhow!(
            "vcf.admixture readiness row drifted from the governed active admixture contract"
        ));
    }
    if row.report_section_id != "population_structure"
        || row.summary_table_id != "population_structure_metrics"
        || row.smoke_selected_k != REQUIRED_SELECTED_K
        || row.smoke_sample_count != REQUIRED_SAMPLE_COUNT
        || row.smoke_population_count != REQUIRED_POPULATION_COUNT
    {
        return Err(anyhow!(
            "vcf.admixture readiness row is missing required report or smoke invariants"
        ));
    }
    Ok(())
}
