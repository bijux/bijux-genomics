...
pub(crate) const DEFAULT_BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_PATH: &str =
    "benchmarks/readiness/bam/contamination-sex-haplogroups-ready.json";
const BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_SCHEMA_VERSION: &str =
    "bijux.bench.readiness.bam_contamination_sex_haplogroups_ready.v1";
...
const EXPECTED_FIXTURE_ID: &str = "corpus-01-adna-bam-mini";
...
const CONTAMINATION_REQUIRED_OUTPUT_IDS: [&str; 3] =
    ["contamination_report", "summary", "stage_metrics"];
const SEX_REQUIRED_OUTPUT_IDS: [&str; 3] = ["sex_report", "summary", "stage_metrics"];
const HAPLOGROUPS_REQUIRED_OUTPUT_IDS: [&str; 3] = ["haplogroups", "summary", "stage_metrics"];

const CONTAMINATION_REQUIRED_LOCAL_PROOF_FIELDS: [&str; 4] =
    ["sample_id", "scope", "reference_panels", "emit_confidence_caveats"];
const SEX_REQUIRED_LOCAL_PROOF_FIELDS: [&str; 11] = [
    "method",
    "chromosome_system",
    "minimum_y_sites",
    "x_coverage",
...
const HAPLOGROUPS_REQUIRED_LOCAL_PROOF_FIELDS: [&str; 4] =
    ["sample_id", "reference_panel_id", "coverage_gate", "reference_build"];

const CONTAMINATION_REQUIRED_LOCAL_PROOF_OUTPUT_IDS: [&str; 3] =
    ["contamination_report", "summary", "stage_metrics"];
const SEX_REQUIRED_LOCAL_PROOF_OUTPUT_IDS: [&str; 3] =
    ["sex_report", "sex_summary", "stage_metrics"];
const HAPLOGROUPS_REQUIRED_LOCAL_PROOF_OUTPUT_IDS: [&str; 3] =
    ["haplogroups", "summary", "stage_metrics"];

const CONTAMINATION_REQUIRED_LOCAL_PROOF_COMMAND_FRAGMENTS: [&str; 4] = [
    "adna_contamination_panel_screen.sam.bai",
    "adna_bam_reference.fasta",
    "adna_contamination_panel.dat",
    "benchmarks/readiness/local-ready/bam.contamination/contamination.summary.json",
];
const HAPLOGROUPS_REQUIRED_LOCAL_PROOF_COMMAND_FRAGMENTS: [&str; 5] = [
    "adna_y_haplogroup_panel.sam",
    "adna_y_haplogroup_panel.sam.bai",
    "adna_y_haplogroup_panel.tsv",
    "--reference_genome hg38",
    "benchmarks/readiness/local-ready/bam.haplogroups/haplogroups",
];
...
fn bam_binding_specs() -> Vec<BamBindingSpec> {
    ...
    let mut specs = vec![
        BamBindingSpec {
            ...
            tool_id: "contammix",
            required_output_ids: &CONTAMINATION_REQUIRED_OUTPUT_IDS,
            normalized_metrics_output_id: "contamination_report",
            schema_extension_id: "bam_contamination_normalized_v1",
            expected_local_proof_kind: LOCAL_PROOF_KIND_READY_PLAN,
            expected_local_proof_tool_id: "verifybamid2",
            required_local_proof_fields: &CONTAMINATION_REQUIRED_LOCAL_PROOF_FIELDS,
            required_local_proof_output_ids: &CONTAMINATION_REQUIRED_LOCAL_PROOF_OUTPUT_IDS,
            required_local_proof_command_fragments:
                &CONTAMINATION_REQUIRED_LOCAL_PROOF_COMMAND_FRAGMENTS,
            report_section_id: "ancient_signal",
            summary_table_id: "damage_authenticity",
            report_anchor_tool_id: "schmutzi",
        },
        BamBindingSpec {
            stage_id: "bam.contamination",
            tool_id: "schmutzi",
        ...
        BamBindingSpec {
            stage_id: "bam.sex",
            tool_id: "angsd",
            required_output_ids: &SEX_REQUIRED_OUTPUT_IDS,
            normalized_metrics_output_id: "sex_report",
            schema_extension_id: "bam_sex_normalized_v1",
            expected_local_proof_kind: LOCAL_PROOF_KIND_SMOKE,
            expected_local_proof_tool_id: "rxy",
            required_local_proof_fields: &SEX_REQUIRED_LOCAL_PROOF_FIELDS,
            required_local_proof_output_ids: &SEX_REQUIRED_LOCAL_PROOF_OUTPUT_IDS,
            required_local_proof_command_fragments: &[],
            report_section_id: "sample_identity",
            summary_table_id: "identity_inference",
            report_anchor_tool_id: "rxy",
        },
        BamBindingSpec {
            stage_id: "bam.sex",
            tool_id: "rxy",
    ...
    specs.push(BamBindingSpec {
        ...
        tool_id: "yleaf",
        required_output_ids: &HAPLOGROUPS_REQUIRED_OUTPUT_IDS,
        normalized_metrics_output_id: "haplogroups",
        schema_extension_id: "bam_haplogroups_normalized_v1",
        expected_local_proof_kind: LOCAL_PROOF_KIND_READY_PLAN,
        expected_local_proof_tool_id: "yleaf",
        required_local_proof_fields: &HAPLOGROUPS_REQUIRED_LOCAL_PROOF_FIELDS,
        required_local_proof_output_ids: &HAPLOGROUPS_REQUIRED_LOCAL_PROOF_OUTPUT_IDS,
        required_local_proof_command_fragments: &HAPLOGROUPS_REQUIRED_LOCAL_PROOF_COMMAND_FRAGMENTS,
        report_section_id: "sample_identity",
        summary_table_id: "identity_inference",
        report_anchor_tool_id: "yleaf",
    });
    specs
}
...
pub(crate) struct BamContaminationSexHaplogroupsReadyRow {
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct BamContaminationSexHaplogroupsReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<BamContaminationSexHaplogroupsReadyRow>,
    pub(crate) violations: Vec<BamContaminationSexHaplogroupsReadyRow>,
}

pub(crate) fn run_render_bam_contamination_sex_haplogroups_ready(
    args: &parse::BenchReadinessRenderBamContaminationSexHaplogroupsReadyArgs,
) -> Result<()> {
    let repo_root = std::env::current_dir().context("resolve current directory")?;
    let report = render_bam_contamination_sex_haplogroups_ready(
        &repo_root,
        args.output
            .clone()
            .unwrap_or_else(|| PathBuf::from(DEFAULT_BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_PATH)),
    )?;
    if args.json {
...
pub(crate) fn render_bam_contamination_sex_haplogroups_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<BamContaminationSexHaplogroupsReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).with_context(|| format!("create {}", parent.display()))?;
    }
    let report = build_bam_contamination_sex_haplogroups_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "bam contamination, sex, and haplogroups readiness must keep active scope, command, output, parser, expected-result, report, schema, and local proof"
        ));
    }
    Ok(report)
}

fn build_bam_contamination_sex_haplogroups_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<BamContaminationSexHaplogroupsReadyReport> {
    ...
    ensure_expected_bindings(
        "BAM contamination/sex/haplogroups active rows",
    ...
    let command_rows = command_report
        ...
        .collect::<Vec<_>>();
    ensure_expected_bindings(
        "BAM contamination/sex/haplogroups command rows",
    ...
    let output_rows = output_report
        ...
        .collect::<Vec<_>>();
    ensure_expected_bindings(
        "BAM contamination/sex/haplogroups output-contract rows",
    ...
    let parser_rows = parser_report
        ...
        .collect::<Vec<_>>();
    ensure_expected_bindings(
        "BAM contamination/sex/haplogroups parser rows",
    ...
    let expected_rows = expected_report
        ...
        .collect::<Vec<_>>();
    ensure_expected_bindings(
        "BAM contamination/sex/haplogroups expected-result rows",
    ...
    ensure_expected_stages(
        "BAM contamination/sex/haplogroups report-map rows",
        report_map_rows.iter().map(|row| row.stage_id.as_str()),
    )?;
    ensure_expected_stages(
        "BAM contamination/sex/haplogroups schema rows",
    ...
    for spec in bam_binding_specs() {
        ...
        let active_row = active_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing BAM contamination/sex/haplogroups active row for `{}` / `{}`",
                spec.stage_id,
                spec.tool_id
            )
        })?;
        let command_row = command_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing BAM contamination/sex/haplogroups command row for `{}` / `{}`",
                spec.stage_id,
                spec.tool_id
            )
        })?;
        let output_row = output_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing BAM contamination/sex/haplogroups output-contract row for `{}` / `{}`",
                spec.stage_id,
                spec.tool_id
            )
        })?;
        let parser_row = parser_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing BAM contamination/sex/haplogroups parser row for `{}` / `{}`",
                spec.stage_id,
                spec.tool_id
            )
        })?;
        let expected_row = expected_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing BAM contamination/sex/haplogroups expected-result row for `{}` / `{}`",
                spec.stage_id,
                spec.tool_id
            )
        })?;
        let report_map_row = report_map_by_stage.get(spec.stage_id).ok_or_else(|| {
            anyhow!(
                "missing BAM contamination/sex/haplogroups report-map row for `{}`",
                spec.stage_id
            )
        })?;
        let schema_contract = schema_contracts.get(spec.stage_id).ok_or_else(|| {
            anyhow!("missing BAM contamination/sex/haplogroups schema row for `{}`", spec.stage_id)
        })?;
        let local_proof = local_proofs_by_stage.get(spec.stage_id).ok_or_else(|| {
            anyhow!("missing BAM contamination/sex/haplogroups local proof for `{}`", spec.stage_id)
        })?;

        rows.push(build_bam_contamination_sex_haplogroups_ready_row(
            active_row,
            command_row,
            output_row,
            parser_row,
    ...
    Ok(BamContaminationSexHaplogroupsReadyReport {
        schema_version: BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_SCHEMA_VERSION,
...
fn build_bam_contamination_sex_haplogroups_ready_row(
    ...
    spec: BamBindingSpec,
) -> BamContaminationSexHaplogroupsReadyRow {
...
fn collect_local_proofs(repo_root: &Path) -> Result<BTreeMap<String, LocalProof>> {
    let contamination_plan_path = bijux_dna_api::v1::api::bam::write_local_contamination_plan()?;
    let contamination_plan: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(&contamination_plan_path)
            .with_context(|| format!("read {}", contamination_plan_path.display()))?,
    )
    .with_context(|| format!("parse {}", contamination_plan_path.display()))?;

    let sex_report_path = bijux_dna_api::v1::api::bam::write_local_sex_smoke_report()?;
    let sex_payload: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(&sex_report_path)
            .with_context(|| format!("read {}", sex_report_path.display()))?,
    )
    .with_context(|| format!("parse {}", sex_report_path.display()))?;
    let sex_report_json_path =
        repo_root.join(required_json_path(&sex_payload, "sex_report", &sex_report_path)?);
    let sex_summary_path =
        repo_root.join(required_json_path(&sex_payload, "sex_summary", &sex_report_path)?);
    let sex_stage_metrics_path =
        repo_root.join(required_json_path(&sex_payload, "stage_metrics", &sex_report_path)?);
    let sex_report_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(&sex_report_json_path)
            .with_context(|| format!("read {}", sex_report_json_path.display()))?,
    )
    .with_context(|| format!("parse {}", sex_report_json_path.display()))?;
    let sex_summary_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(&sex_summary_path)
            .with_context(|| format!("read {}", sex_summary_path.display()))?,
    )
    .with_context(|| format!("parse {}", sex_summary_path.display()))?;
    let sex_stage_metrics_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(&sex_stage_metrics_path)
            .with_context(|| format!("read {}", sex_stage_metrics_path.display()))?,
    )
    .with_context(|| format!("parse {}", sex_stage_metrics_path.display()))?;
    ...
    proofs.insert(
        ...
        LocalProof {
            ...
            proof_path: path_relative_to_repo(repo_root, &contamination_plan_path),
            tool_id: required_string(&contamination_plan, "tool_id", &contamination_plan_path)?,
            sample_id: required_nested_string(
                &contamination_plan,
                &["params", "sample_id"],
                &contamination_plan_path,
            )?,
            ...
            observed_fields: collect_json_fields(std::slice::from_ref(&contamination_plan)),
    ...
    proofs.insert(
        ...
        LocalProof {
            ...
            proof_path: path_relative_to_repo(repo_root, &sex_report_path),
            tool_id: required_string(&sex_payload, "method", &sex_report_path)?,
            sample_id: required_string(&sex_payload, "sample_id", &sex_report_path)?,
            artifact_paths: [
                path_relative_to_repo(repo_root, &sex_report_path),
                ensure_repo_relative_file(
                    repo_root,
                    &required_json_path(&sex_payload, "sex_report", &sex_report_path)?,
                )?,
                ensure_repo_relative_file(
                    repo_root,
                    &required_json_path(&sex_payload, "sex_summary", &sex_report_path)?,
                )?,
                ensure_repo_relative_file(
                    repo_root,
                    &required_json_path(&sex_payload, "stage_metrics", &sex_report_path)?,
                )?,
            ]
            .into_iter()
            .collect(),
            observed_fields: collect_json_fields(&[
                ...
                sex_report_json,
                sex_summary_json,
                sex_stage_metrics_json,
            ]),
            declared_output_ids: SEX_REQUIRED_LOCAL_PROOF_OUTPUT_IDS
                .iter()
                .map(|value| (*value).to_string())
                .collect(),
            command_shell: None,
    ...
    #[cfg(feature = "bam_downstream")]
    {
        let haplogroups_plan_path = bijux_dna_api::v1::api::bam::write_local_haplogroups_plan()?;
        let haplogroups_plan: serde_json::Value = serde_json::from_str(
            &fs::read_to_string(&haplogroups_plan_path)
                .with_context(|| format!("read {}", haplogroups_plan_path.display()))?,
        )
        .with_context(|| format!("parse {}", haplogroups_plan_path.display()))?;
        proofs.insert(
            "bam.haplogroups".to_string(),
            LocalProof {
                proof_kind: LOCAL_PROOF_KIND_READY_PLAN.to_string(),
                proof_path: path_relative_to_repo(repo_root, &haplogroups_plan_path),
                tool_id: required_string(&haplogroups_plan, "tool_id", &haplogroups_plan_path)?,
                sample_id: required_nested_string(
                    &haplogroups_plan,
                    &["params", "sample_id"],
                    &haplogroups_plan_path,
                )?,
                artifact_paths: collect_plan_artifact_paths(
                    repo_root,
                    &haplogroups_plan,
                ...
                observed_fields: collect_json_fields(std::slice::from_ref(&haplogroups_plan)),
                declared_output_ids: collect_plan_output_ids(
                    &haplogroups_plan,
                    &haplogroups_plan_path,
                )?,
    ...
    T: BindingRow,
{
    ...
    for row in rows {
        ...
        if by_binding.insert(key.clone(), row).is_some() {
            return Err(anyhow!(
                "duplicate BAM contamination/sex/haplogroups binding `{}` / `{}`",
                key.stage_id,
                key.tool_id
            ));
        }
...
mod tests {
    ...
    use super::{
        render_bam_contamination_sex_haplogroups_ready,
        BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_SCHEMA_VERSION,
    ...
    #[test]
    fn render_bam_contamination_sex_haplogroups_ready_reports_governed_rows() {
        let root = repo_root();
        let report = render_bam_contamination_sex_haplogroups_ready(
            &root,
            PathBuf::from(DEFAULT_BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_PATH),
        )
        .expect("render BAM contamination/sex/haplogroups readiness");
        ...
        assert_eq!(report.schema_version, BAM_CONTAMINATION_SEX_HAPLOGROUPS_READY_SCHEMA_VERSION);
        ...
        assert_eq!(report.stage_count, expected_stage_count);
        assert_eq!(report.violation_count, 0);
        assert!(report.ok);
        assert_eq!(report.rows.len(), expected_row_count);
        ...
        let contamination_row = report
            .rows
            .iter()
            .find(|row| row.stage_id == "bam.contamination" && row.tool_id == "verifybamid2")
            .expect("bam.contamination verifybamid2 row");
        assert_eq!(contamination_row.expected_normalized_metrics_output_id, "contamination_report");
        assert_eq!(contamination_row.report_section_id, "ancient_signal");
        assert_eq!(contamination_row.summary_table_id, "damage_authenticity");
        assert_eq!(contamination_row.expected_local_proof_kind, "local_ready_plan");
        assert_eq!(contamination_row.local_proof_tool_id, "verifybamid2");
        assert!(contamination_row
            .required_local_proof_fields
            .iter()
            .any(|field| field == "reference_panels"));
        assert!(contamination_row
            .local_proof_declared_output_ids
            .iter()
            .any(|output| output == "contamination_estimate"));
        ...
        assert_eq!(sex_row.expected_normalized_metrics_output_id, "sex_report");
        assert_eq!(sex_row.report_section_id, "sample_identity");
        assert_eq!(sex_row.summary_table_id, "identity_inference");
        assert_eq!(sex_row.expected_local_proof_kind, "local_smoke");
        assert_eq!(sex_row.local_proof_tool_id, "rxy");
        assert!(sex_row
            .required_local_proof_fields
            .iter()
            .any(|field| field == "autosomal_coverage"));
        assert!(sex_row
            .local_proof_artifact_paths
            .iter()
            .any(|path| path == "runs/bench/local-smoke/bam.sex/sex.json"));

        #[cfg(feature = "bam_downstream")]
        {
            let haplogroups_row = report
                .rows
                .iter()
                .find(|row| row.stage_id == "bam.haplogroups" && row.tool_id == "yleaf")
                .expect("bam.haplogroups yleaf row");
            assert_eq!(haplogroups_row.expected_normalized_metrics_output_id, "haplogroups");
            assert_eq!(haplogroups_row.report_section_id, "sample_identity");
            assert_eq!(haplogroups_row.summary_table_id, "identity_inference");
            assert_eq!(haplogroups_row.expected_local_proof_kind, "local_ready_plan");
            assert_eq!(haplogroups_row.local_proof_tool_id, "yleaf");
            assert!(haplogroups_row
                .required_local_proof_fields
                .iter()
                .any(|field| field == "reference_panel_id"));
            assert!(haplogroups_row
                .local_proof_declared_output_ids
                .iter()
                .any(|output| output == "haplogroup_report"));
        }
    }
}
