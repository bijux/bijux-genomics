...
use crate::commands::benchmark::local_vcf_call_pseudohaploid_smoke::{
    run_local_vcf_call_pseudohaploid_smoke, LocalVcfCallPseudohaploidSmokeReport,
};
use crate::commands::cli::parse;
use crate::commands::cli::render;

pub(crate) const DEFAULT_VCF_CALL_PSEUDOHAPLOID_READY_PATH: &str =
    "benchmarks/readiness/vcf/call-pseudohaploid-ready.json";
const VCF_CALL_PSEUDOHAPLOID_READY_SCHEMA_VERSION: &str =
    "bijux.bench.readiness.vcf_call_pseudohaploid_ready.v1";
const VCF_CALL_PSEUDOHAPLOID_STAGE_ID: &str = "vcf.call_pseudohaploid";
...
const REQUIRED_SAMPLING_POLICY: &str = "deterministic_first_allele_projection";
const REQUIRED_SEED_USAGE: &str = "control_seed_for_replay_proof";
const REQUIRED_RANDOM_SEED: u64 = 73;
const COVERAGE_STATUS_COMPLETE: &str = "complete";
const COVERAGE_STATUS_INCOMPLETE: &str = "incomplete";
const NO_VALUE: &str = "none";

#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfCallPseudohaploidReadyRow {
    ...
    pub(crate) smoke_haploid_compatible: bool,
    pub(crate) smoke_gt_present: bool,
    pub(crate) smoke_gl_present: bool,
    pub(crate) smoke_target_sites: u64,
    pub(crate) smoke_covered_sites: u64,
    pub(crate) smoke_called_sites: u64,
    pub(crate) smoke_missing_sites: u64,
    pub(crate) smoke_sampling_policy: String,
    pub(crate) smoke_seed_usage: String,
    pub(crate) smoke_random_seed: u64,
    pub(crate) smoke_sample_count: u64,
    pub(crate) smoke_raw_replay_match: bool,
    pub(crate) smoke_deterministic_replay_match: bool,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfCallPseudohaploidReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<VcfCallPseudohaploidReadyRow>,
    pub(crate) violations: Vec<VcfCallPseudohaploidReadyRow>,
}

pub(crate) fn run_render_vcf_call_pseudohaploid_ready(
    args: &parse::BenchReadinessRenderVcfCallPseudohaploidReadyArgs,
) -> Result<()> {
    let repo_root = std::env::current_dir().context("resolve current directory")?;
    let report = render_vcf_call_pseudohaploid_ready(
        &repo_root,
        args.output
            .clone()
            .unwrap_or_else(|| PathBuf::from(DEFAULT_VCF_CALL_PSEUDOHAPLOID_READY_PATH)),
    )?;
    if args.json {
...
pub(crate) fn render_vcf_call_pseudohaploid_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<VcfCallPseudohaploidReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_vcf_call_pseudohaploid_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "vcf.call_pseudohaploid retained callers must stay complete across active-scope, command, output, parser, report, and pseudohaploid smoke proof"
        ));
    }
    Ok(report)
}

fn build_vcf_call_pseudohaploid_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<VcfCallPseudohaploidReadyReport> {
    ...
    let report = VcfCallPseudohaploidReadyReport {
        schema_version: VCF_CALL_PSEUDOHAPLOID_READY_SCHEMA_VERSION,
    ...
    ensure_vcf_call_pseudohaploid_ready_contract(&report)?;
    Ok(report)
}

fn build_vcf_call_pseudohaploid_ready_row(
    command_report: &VcfRenderedCommandsReport,
    binding: VcfStageReadinessBinding,
    smoke_report: Option<&LocalVcfCallPseudohaploidSmokeReport>,
) -> VcfCallPseudohaploidReadyRow {
    ...
    let output_ready = binding.output_row.as_ref().is_some_and(|row| {
        row.status == VcfAdapterOutputCoverageStatus::Complete
            && row.benchmark_status == "benchmark_ready"
            && contains_artifact_id(&row.raw_outputs, "pseudohaploid_vcf")
            && !row.normalized_metrics.is_empty()
            && !row.manifest.trim().is_empty()
            && contains_artifact_id(&row.index_outputs, "pseudohaploid_vcf_tbi")
    });
    if !output_ready {
    ...
    let expected_result_ready = binding.expected_row.as_ref().is_some_and(|row| {
        row.report_section == "variant_calling"
            && row.expected_outputs.iter().any(|value| value == "pseudohaploid_vcf")
    ...
    let smoke_ready = smoke_report.is_some_and(|report| {
        report.parseable
            ...
            && report.called_sites + report.missing_sites == report.target_sites
            && report.covered_sites >= report.called_sites
            && report.sampling_policy == REQUIRED_SAMPLING_POLICY
            && report.seed_usage == REQUIRED_SEED_USAGE
            && report.random_seed == REQUIRED_RANDOM_SEED
            && report.deterministic_replay_match
    });
    if !smoke_ready {
        missing_surfaces.push("local_vcf_call_pseudohaploid_smoke".to_string());
    }
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "retained VCF pseudohaploid caller `{}` keeps active scope, command, output, parser, expected-result, report, and pseudohaploid smoke proof for `vcf.call_pseudohaploid`",
            binding.retained_row.tool_id
        )
    } else {
        format!(
            "retained VCF pseudohaploid caller `{}` is missing: {}",
            binding.retained_row.tool_id,
    ...
    VcfCallPseudohaploidReadyRow {
        ...
        smoke_haploid_compatible: smoke_report.is_some_and(|report| report.haploid_compatible),
        ...
        smoke_target_sites: smoke_report.map_or(0, |report| report.target_sites),
        smoke_covered_sites: smoke_report.map_or(0, |report| report.covered_sites),
        smoke_called_sites: smoke_report.map_or(0, |report| report.called_sites),
        smoke_missing_sites: smoke_report.map_or(0, |report| report.missing_sites),
        smoke_sampling_policy: smoke_report
            .map_or_else(no_value_string, |report| report.sampling_policy.to_string()),
        smoke_seed_usage: smoke_report
            .map_or_else(no_value_string, |report| report.seed_usage.to_string()),
        smoke_random_seed: smoke_report.map_or(0, |report| report.random_seed),
        smoke_sample_count: smoke_report.map_or(0, |report| report.sample_count),
        smoke_raw_replay_match: smoke_report.is_some_and(|report| report.raw_replay_match),
        smoke_deterministic_replay_match: smoke_report
            .is_some_and(|report| report.deterministic_replay_match),
        required_metric_names,
        missing_surfaces,
        coverage_status,
        reason,
...
fn ensure_vcf_call_pseudohaploid_ready_contract(
    report: &VcfCallPseudohaploidReadyReport,
) -> Result<()> {
    if report.retained_row_count != report.rows.len() {
        return Err(anyhow!(
            "VCF call_pseudohaploid readiness must keep exactly one row per retained `vcf.call_pseudohaploid` binding"
        ));
    }
    ...
    if report.checked_surface_count != 8 {
        return Err(anyhow!(
            "VCF call_pseudohaploid readiness must record exactly 8 checked surfaces"
        ));
    }
    let unique_results =
    ...
    if unique_results != report.rows.len() {
        return Err(anyhow!(
            "VCF call_pseudohaploid readiness must keep one unique result_id per retained caller row"
        ));
    }
    for row in &report.rows {
        if row.stage_id != VCF_CALL_PSEUDOHAPLOID_STAGE_ID {
            return Err(anyhow!(
                "VCF call_pseudohaploid readiness row `{}` drifted away from the `vcf.call_pseudohaploid` stage",
                row.stage_id
            ));
        }
        if row.coverage_status == COVERAGE_STATUS_COMPLETE && !row.missing_surfaces.is_empty() {
            return Err(anyhow!(
                "VCF call_pseudohaploid readiness row `{}` / `{}` cannot be complete while listing missing surfaces",
                row.stage_id,
                row.tool_id
            ));
        }
