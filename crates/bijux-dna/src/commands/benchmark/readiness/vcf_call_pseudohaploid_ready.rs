...
fn build_vcf_call_pseudohaploid_ready_row(
    ...
    VcfCallPseudohaploidReadyRow {
        ...
        smoke_target_sites: smoke_report.map(|report| report.target_sites).unwrap_or(0),
        smoke_covered_sites: smoke_report.map(|report| report.covered_sites).unwrap_or(0),
        smoke_called_sites: smoke_report.map(|report| report.called_sites).unwrap_or(0),
        smoke_missing_sites: smoke_report.map(|report| report.missing_sites).unwrap_or(0),
        smoke_sampling_policy: smoke_report
            .map(|report| report.sampling_policy.to_string())
            .unwrap_or_else(no_value_string),
        smoke_seed_usage: smoke_report
            .map(|report| report.seed_usage.to_string())
            .unwrap_or_else(no_value_string),
        smoke_random_seed: smoke_report.map(|report| report.random_seed).unwrap_or(0),
