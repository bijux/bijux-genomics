...
use crate::commands::benchmark::local_vcf_stats_smoke::{
    run_local_vcf_stats_smoke, LocalVcfStatsSmokeReport,
};
use crate::commands::cli::parse;
use crate::commands::cli::render;

pub(crate) const DEFAULT_VCF_STATS_READY_PATH: &str = "benchmarks/readiness/vcf/stats-ready.json";
const VCF_STATS_READY_SCHEMA_VERSION: &str = "bijux.bench.readiness.vcf_stats_ready.v1";
const VCF_STATS_STAGE_ID: &str = "vcf.stats";
...
const REQUIRED_VARIANT_COUNT: u64 = 4;
const REQUIRED_SNP_COUNT: u64 = 3;
const REQUIRED_INDEL_COUNT: u64 = 1;
const REQUIRED_TRANSITION_COUNT: u64 = 2;
const REQUIRED_TRANSVERSION_COUNT: u64 = 1;
const REQUIRED_TI_TV: f64 = 2.0;
const REQUIRED_SAMPLE_COUNT: u64 = 2;
const COVERAGE_STATUS_COMPLETE: &str = "complete";
const COVERAGE_STATUS_INCOMPLETE: &str = "incomplete";
const NO_VALUE: &str = "none";

#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfStatsReadyRow {
    ...
    pub(crate) smoke_stats_json_path: String,
    pub(crate) smoke_bcftools_stats_path: String,
    ...
    pub(crate) smoke_transition_count: u64,
    pub(crate) smoke_transversion_count: u64,
    pub(crate) smoke_ti_tv: f64,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfStatsReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<VcfStatsReadyRow>,
    pub(crate) violations: Vec<VcfStatsReadyRow>,
}

pub(crate) fn run_render_vcf_stats_ready(
    args: &parse::BenchReadinessRenderVcfStatsReadyArgs,
) -> Result<()> {
    let repo_root = std::env::current_dir().context("resolve current directory")?;
    let report = render_vcf_stats_ready(
        &repo_root,
        args.output.clone().unwrap_or_else(|| PathBuf::from(DEFAULT_VCF_STATS_READY_PATH)),
    )?;
    if args.json {
...
pub(crate) fn render_vcf_stats_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<VcfStatsReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_vcf_stats_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "vcf.stats active retained callers must stay complete across active-scope, command, output, parser, report, and normalized stats smoke proof"
        ));
    }
    Ok(report)
}

fn build_vcf_stats_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<VcfStatsReadyReport> {
    let (command_report, bindings) =
        collect_vcf_stage_readiness_bindings(repo_root, VCF_STATS_STAGE_ID)?;
    ...
    for binding in active_bindings {
        let smoke_report = run_local_vcf_stats_smoke(repo_root, &binding.retained_row.tool_id).ok();
        rows.push(build_vcf_stats_ready_row(&command_report, binding, smoke_report.as_ref()));
    }
    ...
    let report = VcfStatsReadyReport {
        schema_version: VCF_STATS_READY_SCHEMA_VERSION,
    ...
    ensure_vcf_stats_ready_contract(&report)?;
    Ok(report)
}

fn build_vcf_stats_ready_row(
    command_report: &VcfRenderedCommandsReport,
    binding: VcfStageReadinessBinding,
    smoke_report: Option<&LocalVcfStatsSmokeReport>,
) -> VcfStatsReadyRow {
    ...
    let output_ready = binding.output_row.as_ref().is_some_and(|row| {
        row.status == VcfAdapterOutputCoverageStatus::Complete
            && row.benchmark_status == "benchmark_ready"
            && contains_artifact_id(&row.raw_outputs, "bcftools_stats_txt")
            && contains_artifact_id(&row.normalized_metrics, "stats_json")
    ...
    let expected_result_ready = binding.expected_row.as_ref().is_some_and(|row| {
        row.report_section == "quality_control"
            && row.expected_outputs.iter().any(|value| value == "stats_json")
    ...
    let smoke_ready = smoke_report.is_some_and(|report| {
        !report.input_vcf_path.trim().is_empty()
            ...
            && !report.stats_json_path.trim().is_empty()
            && !report.bcftools_stats_path.trim().is_empty()
            ...
            && report.snp_count == REQUIRED_SNP_COUNT
            && report.indel_count == REQUIRED_INDEL_COUNT
            && report.transition_count == REQUIRED_TRANSITION_COUNT
            && report.transversion_count == REQUIRED_TRANSVERSION_COUNT
            && report.ti_tv == REQUIRED_TI_TV
            && report.sample_count == REQUIRED_SAMPLE_COUNT
    });
    if !smoke_ready {
        missing_surfaces.push("local_vcf_stats_smoke".to_string());
    }
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "active retained VCF stats caller `{}` keeps active scope, command, output, parser, expected-result, report, and normalized stats smoke proof for `vcf.stats`",
            binding.retained_row.tool_id
        )
    } else {
        format!(
            "active retained VCF stats caller `{}` is missing: {}",
            binding.retained_row.tool_id,
    ...
    VcfStatsReadyRow {
        ...
        smoke_stats_json_path: smoke_report
            .map_or_else(no_value_string, |report| report.stats_json_path.clone()),
        smoke_bcftools_stats_path: smoke_report
            .map_or_else(no_value_string, |report| report.bcftools_stats_path.clone()),
        ...
        smoke_snp_count: smoke_report.map_or(0, |report| report.snp_count),
        smoke_indel_count: smoke_report.map_or(0, |report| report.indel_count),
        smoke_transition_count: smoke_report.map_or(0, |report| report.transition_count),
        smoke_transversion_count: smoke_report.map_or(0, |report| report.transversion_count),
        smoke_ti_tv: smoke_report.map_or(0.0, |report| report.ti_tv),
...
fn ensure_vcf_stats_ready_contract(report: &VcfStatsReadyReport) -> Result<()> {
    if report.retained_row_count != report.rows.len() {
        return Err(anyhow!(
            "VCF stats readiness must keep exactly one row per active retained `vcf.stats` binding"
        ));
    }
    if report.rows.is_empty() {
        return Err(anyhow!(
            "VCF stats readiness must keep at least one active retained caller row"
        ));
    }
    if report.checked_surface_count != 8 {
        return Err(anyhow!("VCF stats readiness must record exactly 8 checked surfaces"));
    }
    let unique_results =
        report.rows.iter().map(|row| row.result_id.as_str()).collect::<BTreeSet<_>>().len();
    if unique_results != report.rows.len() {
        return Err(anyhow!(
            "VCF stats readiness must keep one unique result_id per active retained caller row"
        ));
    }
    for row in &report.rows {
        if row.stage_id != VCF_STATS_STAGE_ID {
            return Err(anyhow!(
                "VCF stats readiness row `{}` drifted away from the `vcf.stats` stage",
                row.stage_id
            ));
        }
        if row.coverage_status == COVERAGE_STATUS_COMPLETE && !row.missing_surfaces.is_empty() {
            return Err(anyhow!(
                "VCF stats readiness row `{}` / `{}` cannot be complete while listing missing surfaces",
                row.stage_id,
                row.tool_id
            ));
        }
