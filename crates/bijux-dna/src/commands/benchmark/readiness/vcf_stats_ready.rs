...
fn build_vcf_stats_ready_row(
    ...
    VcfStatsReadyRow {
        ...
        smoke_stats_json_path: smoke_report
            .map(|report| report.stats_json_path.clone())
            .unwrap_or_else(no_value_string),
        smoke_bcftools_stats_path: smoke_report
            .map(|report| report.bcftools_stats_path.clone())
            .unwrap_or_else(no_value_string),
        ...
        smoke_transition_count: smoke_report.map(|report| report.transition_count).unwrap_or(0),
        smoke_transversion_count: smoke_report.map(|report| report.transversion_count).unwrap_or(0),
        smoke_ti_tv: smoke_report.map(|report| report.ti_tv).unwrap_or(0.0),
