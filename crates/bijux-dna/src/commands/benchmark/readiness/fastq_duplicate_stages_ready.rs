...
const FASTQ_DUPLICATE_STAGES_READY_SCHEMA_VERSION: &str =
    "bijux.bench.readiness.fastq_duplicate_stages_ready.v1";
...
const DETECT_DUPLICATES_PREMERGE_TOOL_IDS: [&str; 1] = ["bijux_dna"];
const REMOVE_DUPLICATES_TOOL_IDS: [&str; 2] = ["clumpify", "fastuniq"];

const DETECT_DUPLICATES_PREMERGE_REQUIRED_OUTPUT_IDS: [&str; 1] = ["duplicate_signal_report"];
const REMOVE_DUPLICATES_REQUIRED_OUTPUT_IDS: [&str; 1] = ["dedup_reads_r1"];

const DETECT_DUPLICATES_PREMERGE_REQUIRED_METRIC_FIELDS: [&str; 4] =
    ["duplicate_count", "duplicate_fraction", "inspected_pair_count", "report_json"];
const REMOVE_DUPLICATES_REQUIRED_METRIC_FIELDS: [&str; 5] =
    ["unique_reads", "output_reads", "duplicates_removed", "dedup_rate", "report_json"];

#[derive(Debug, Clone, Copy)]
struct DuplicateStageSpec {
    stage_id: &'static str,
    ...
    required_report_section_id: &'static str,
    required_summary_table_id: &'static str,
}

const DUPLICATE_STAGE_SPECS: [DuplicateStageSpec; 2] = [
    DuplicateStageSpec {
        stage_id: "fastq.detect_duplicates_premerge",
        expected_tool_ids: &DETECT_DUPLICATES_PREMERGE_TOOL_IDS,
        required_output_ids: &DETECT_DUPLICATES_PREMERGE_REQUIRED_OUTPUT_IDS,
        normalized_metrics_output_id: "duplicate_signal_report",
        required_metric_fields: &DETECT_DUPLICATES_PREMERGE_REQUIRED_METRIC_FIELDS,
        required_report_section_id: "quality_profiling",
        required_summary_table_id: "premerge_complexity",
    },
    DuplicateStageSpec {
        stage_id: "fastq.remove_duplicates",
        expected_tool_ids: &REMOVE_DUPLICATES_TOOL_IDS,
        required_output_ids: &REMOVE_DUPLICATES_REQUIRED_OUTPUT_IDS,
        normalized_metrics_output_id: "report_json",
        required_metric_fields: &REMOVE_DUPLICATES_REQUIRED_METRIC_FIELDS,
        required_report_section_id: "read_cleanup",
        required_summary_table_id: "cleanup_retention",
    },
];

#[derive(Debug, Clone, Serialize)]
pub(crate) struct FastqDuplicateStagesReadyRow {
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct FastqDuplicateStagesReadyReport {
    ...
    pub(crate) report_targets_by_stage: BTreeMap<String, Vec<String>>,
    pub(crate) coverage_status_counts: BTreeMap<String, usize>,
    pub(crate) violation_count: usize,
    pub(crate) ok: bool,
    pub(crate) rows: Vec<FastqDuplicateStagesReadyRow>,
    pub(crate) violations: Vec<FastqDuplicateStagesReadyRow>,
}
...
pub(crate) fn render_fastq_duplicate_stages_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<FastqDuplicateStagesReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_fastq_duplicate_stages_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "fastq duplicate stages must keep active scope, command, output, parser, expected-result, report, and schema proof"
        ));
    }
    Ok(report)
}

fn build_fastq_duplicate_stages_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<FastqDuplicateStagesReadyReport> {
    ...
    let schema_contracts = collect_duplicate_stage_schema_contracts()?;

    let active_rows = active_matrix
        .rows
        .into_iter()
        .filter(|row| duplicate_stage_spec(&row.stage_id).is_some())
        .collect::<Vec<_>>();
    ensure_duplicate_stage_tool_sets(
        "FASTQ duplicate-stage active rows",
    ...
    let output_rows = output_contract
        .rows
        .into_iter()
        .filter(|row| duplicate_stage_binding_admitted(&row.stage_id, &row.tool_id))
        .collect::<Vec<_>>();
    ensure_duplicate_stage_tool_sets(
        "FASTQ duplicate-stage output-contract rows",
    ...
    let parser_rows = parser_coverage
        ...
        .collect::<Vec<_>>();
    ensure_duplicate_stage_tool_sets(
        "FASTQ duplicate-stage parser rows",
    ...
    let expected_rows = expected_results
        ...
        .into_iter()
        .filter(|row| {
            row.domain == "fastq" && duplicate_stage_binding_admitted(&row.stage_id, &row.tool_id)
        })
        .collect::<Vec<_>>();
    ensure_duplicate_stage_tool_sets(
        "FASTQ duplicate-stage expected-result rows",
    ...
    let command_rows = command_coverage
        ...
        .collect::<Vec<_>>();
    ensure_duplicate_stage_tool_sets(
        "FASTQ duplicate-stage rendered-command rows",
    ...
    for active_row in active_rows {
        ...
        let spec = duplicate_stage_spec(&active_row.stage_id).expect("duplicate stage spec");
        let output_row = output_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing duplicate-stage output-contract row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let parser_row = parser_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing duplicate-stage parser row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let expected_row = expected_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing duplicate-stage expected-result row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let command_row = command_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing duplicate-stage rendered-command row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let report_map_row = report_map_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing duplicate-stage report-map row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let schema_contract = schema_contracts.get(&active_row.stage_id).ok_or_else(|| {
            anyhow!("missing duplicate-stage schema contract for `{}`", active_row.stage_id)
        })?;

        rows.push(build_fastq_duplicate_stages_ready_row(
            active_row,
            output_row,
            parser_row,
            expected_row,
    ...
    Ok(FastqDuplicateStagesReadyReport {
        schema_version: FASTQ_DUPLICATE_STAGES_READY_SCHEMA_VERSION,
        output_path: path_relative_to_repo(repo_root, output_path),
        active_row_count: rows.len(),
        complete_row_count,
        incomplete_row_count,
        checked_surface_count: CHECKED_SURFACE_COUNT,
        stage_count: DUPLICATE_STAGE_SPECS.len(),
        expected_tool_ids_by_stage: DUPLICATE_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
            ...
            .collect(),
        required_output_ids_by_stage: DUPLICATE_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
                    spec.stage_id.to_string(),
                    spec.required_output_ids
                        .iter()
            ...
            .collect(),
        required_metric_fields_by_stage: DUPLICATE_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
                    spec.stage_id.to_string(),
            ...
            .collect(),
        report_targets_by_stage: DUPLICATE_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
                    spec.stage_id.to_string(),
                    vec![
                        spec.required_report_section_id.to_string(),
                        spec.required_summary_table_id.to_string(),
                    ],
                )
            })
            .collect(),
...
fn build_fastq_duplicate_stages_ready_row(
    ...
    schema_contract: &DuplicateStageSchemaContract,
    spec: DuplicateStageSpec,
    schema_proof_path: &str,
) -> FastqDuplicateStagesReadyRow {
    ...
    let output_ready = output_row.output_contract_status
        ...
        && spec.required_output_ids.iter().all(|output_id| {
            output_row.raw_output_artifact_ids.iter().any(|candidate| candidate == output_id)
    ...
    let expected_result_ready = expected_row.sample_scope == "sample-set"
        && expected_row.normalized_metrics_output_id.as_deref()
            == Some(spec.normalized_metrics_output_id)
        && spec.required_output_ids.iter().all(|output_id| {
            expected_row.expected_output_artifact_ids.iter().any(|candidate| candidate == output_id)
        })
        && !expected_row.stage_result_manifest_path.is_empty()
        && !expected_row.stdout_path.is_empty()
        && !expected_row.stderr_path.is_empty();
    let report_ready = report_map_row.report_section_id == spec.required_report_section_id
        && report_map_row.summary_table_id == spec.required_summary_table_id;
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "duplicate binding `{}` / `{}` keeps active scope, command, output, parser, expected-result, report, and schema proof for `{}`",
            active_row.stage_id, active_row.tool_id, active_row.stage_id
        )
    } else {
        format!(
            "duplicate binding `{}` / `{}` is missing readiness proof for {}",
            active_row.stage_id,
            active_row.tool_id,
...
fn collect_duplicate_stage_schema_contracts(
) -> Result<BTreeMap<String, DuplicateStageSchemaContract>> {
    ...
    for spec in DUPLICATE_STAGE_SPECS {
    ...
    T: BindingRow,
{
    ...
    for row in rows {
        ...
        if by_binding.insert(key.clone(), row).is_some() {
            return Err(anyhow!(
                "duplicate duplicate-stage binding `{}` / `{}`",
                key.stage_id,
                key.tool_id
            ));
        }
...
fn ensure_duplicate_stage_tool_sets<'a>(
    label: &str,
    ...
    for spec in DUPLICATE_STAGE_SPECS {
        ...
        if actual != expected {
            return Err(anyhow!(
                "{} for `{}` must match the governed duplicate tool set: expected [{}], found [{}]",
                label,
                spec.stage_id,
...
fn ensure_required_stage_rows<'a>(
    ...
    let expected =
        DUPLICATE_STAGE_SPECS.iter().map(|spec| spec.stage_id.to_string()).collect::<BTreeSet<_>>();
...
fn duplicate_stage_spec(stage_id: &str) -> Option<DuplicateStageSpec> {
    DUPLICATE_STAGE_SPECS.iter().copied().find(|spec| spec.stage_id == stage_id)
}

fn duplicate_stage_binding_admitted(stage_id: &str, tool_id: &str) -> bool {
    duplicate_stage_spec(stage_id).is_some_and(|spec| spec.expected_tool_ids.contains(&tool_id))
}
...
mod tests {
    ...
    fn render_fastq_duplicate_stages_ready_reports_complete_duplicate_bindings() {
        ...
        .expect("render FASTQ duplicate stages readiness");

        assert_eq!(report.schema_version, FASTQ_DUPLICATE_STAGES_READY_SCHEMA_VERSION);
        assert_eq!(report.output_path, DEFAULT_FASTQ_DUPLICATE_STAGES_READY_PATH);
        ...
        assert!(report.ok);
        assert_eq!(report.coverage_status_counts.get("complete"), Some(&3));

        let detect_row = report
            .rows
            .iter()
            .find(|row| {
                row.stage_id == "fastq.detect_duplicates_premerge" && row.tool_id == "bijux_dna"
            })
            .expect("detect_duplicates_premerge row");
        assert!(detect_row.active_scope_ready);
        assert!(detect_row.command_ready);
        assert!(detect_row.output_ready);
        assert!(detect_row.parser_ready);
        assert!(detect_row.expected_result_ready);
        assert!(detect_row.report_ready);
        assert!(detect_row.schema_ready);
        assert_eq!(detect_row.report_section_id, "quality_profiling");
        assert_eq!(detect_row.summary_table_id, "premerge_complexity");
        assert_eq!(
            detect_row.normalized_metrics_output_id.as_deref(),
            Some("duplicate_signal_report")
        );
        assert!(detect_row
            .schema_required_fields
            .iter()
            .any(|field| field == "inspected_pair_count"));
    }
}
