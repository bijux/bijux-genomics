...
pub(crate) const DEFAULT_FASTQ_REPORT_MAP_PATH: &str =
    "benchmarks/readiness/fastq/fastq-report-map.tsv";
...
pub(crate) struct FastqReportStageMetadata {
    pub(crate) canonical_stage_rank: usize,
    pub(crate) readiness_kind: String,
    pub(crate) stage_kind: String,
    pub(crate) criticality: String,
...
pub(crate) fn collect_fastq_report_map_rows(repo_root: &Path) -> Result<Vec<FastqReportMapRow>> {
    ...
    for row in &expected_rows {
        ...
        rows.push(FastqReportMapRow {
            ...
            metric_classes: metadata.metric_classes.clone(),
            mutates_fastq: metadata.mutates_fastq,
            produces_reports_only: metadata.produces_reports_only,
            ...
            reason: report_reason(
                ...
                support_status,
                metadata.produces_reports_only,
            ),
        });
    }
...
pub(crate) fn collect_fastq_report_stage_metadata(
    repo_root: &Path,
) -> Result<BTreeMap<String, FastqReportStageMetadata>> {
...
fn ensure_fastq_report_map_contract(
    ...
    for row in rows {
        ...
        {
            return Err(anyhow!(
                "FASTQ report map row `{}` / `{}` is missing required report binding fields",
                row.stage_id,
                row.tool_id
            ));
        }
...
fn require_row_mapping(
    ...
    let row = rows
        ...
        .ok_or_else(|| anyhow!("FASTQ report map is missing `{stage_id}` / `{tool_id}`"))?;
    ...
    {
        return Err(anyhow!(
            "FASTQ report map row `{stage_id}` / `{tool_id}` drifted from its governed reporting contract"
        ));
    }
    Ok(())
}
...
fn report_reason(
    ...
    format!(
        "benchmark result binding `{stage_id}` / `{tool_id}` is a FASTQ {stage_mode} row with governed support status `{support_status}`; benchmark reporting places it in section `{report_section_id}` and summary table `{summary_table_id}` to track {report_focus}"
    )
}
...
fn admission_support_status<'a>(
    ...
    admissions
        ...
        .ok_or_else(|| anyhow!("FASTQ report map stage is missing support status for `{tool_id}`"))
}
...
mod tests {
    ...
    #[test]
    fn fastq_report_map_tracks_governed_expected_result_bindings() {
        ...
        let screen_taxonomy = report
            .rows
            .iter()
            .find(|row| row.stage_id == "fastq.screen_taxonomy" && row.tool_id == "kraken2")
            .expect("screen taxonomy kraken2 row");
        assert_eq!(screen_taxonomy.support_status, "supported");
        ...
        assert_eq!(trim_reads.summary_table_id, "cleanup_retention");
    }

    #[test]
    fn fastq_report_stage_metadata_keeps_report_only_stage_anchor_contract() {
        let repo_root = repo_root();
        let metadata =
            collect_fastq_report_stage_metadata(&repo_root).expect("collect FASTQ stage metadata");
        let report_qc = metadata.get("fastq.report_qc").expect("report qc metadata");
