...
use crate::commands::benchmark::local_vcf_gl_propagation_smoke::{
    run_local_vcf_gl_propagation_smoke, LocalVcfGlPropagationSmokeReport,
};
use crate::commands::cli::parse;
use crate::commands::cli::render;

pub(crate) const DEFAULT_VCF_GL_PROPAGATION_READY_PATH: &str =
    "benchmarks/readiness/vcf/gl-propagation-ready.json";
const VCF_GL_PROPAGATION_READY_SCHEMA_VERSION: &str =
    "bijux.bench.readiness.vcf_gl_propagation_ready.v1";
const VCF_GL_PROPAGATION_STAGE_ID: &str = "vcf.gl_propagation";
...
pub(crate) struct VcfGlPropagationReadyRow {
    ...
    pub(crate) smoke_output_bcf_path: String,
    pub(crate) smoke_output_bcf_csi_path: String,
    pub(crate) smoke_report_path: String,
    ...
    pub(crate) smoke_input_likelihood_fields: Vec<String>,
    pub(crate) smoke_output_likelihood_fields: Vec<String>,
    pub(crate) smoke_lost_fields: Vec<String>,
    pub(crate) smoke_site_count_before: u64,
    pub(crate) smoke_site_count_after: u64,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfGlPropagationReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<VcfGlPropagationReadyRow>,
    pub(crate) violations: Vec<VcfGlPropagationReadyRow>,
}

pub(crate) fn run_render_vcf_gl_propagation_ready(
    args: &parse::BenchReadinessRenderVcfGlPropagationReadyArgs,
) -> Result<()> {
    let repo_root = std::env::current_dir().context("resolve current directory")?;
    let report = render_vcf_gl_propagation_ready(
        &repo_root,
        args.output.clone().unwrap_or_else(|| PathBuf::from(DEFAULT_VCF_GL_PROPAGATION_READY_PATH)),
    )?;
    if args.json {
...
pub(crate) fn render_vcf_gl_propagation_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<VcfGlPropagationReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_vcf_gl_propagation_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "vcf.gl_propagation active retained callers must stay complete across active-scope, command, output, parser, report, and likelihood-propagation smoke proof"
        ));
    }
    Ok(report)
}

fn build_vcf_gl_propagation_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<VcfGlPropagationReadyReport> {
    let (command_report, bindings) =
        collect_vcf_stage_readiness_bindings(repo_root, VCF_GL_PROPAGATION_STAGE_ID)?;
    ...
    for binding in active_bindings {
        let smoke_report =
            run_local_vcf_gl_propagation_smoke(repo_root, &binding.retained_row.tool_id).ok();
        rows.push(build_vcf_gl_propagation_ready_row(
            &command_report,
            binding,
            smoke_report.as_ref(),
        ));
    ...
    let report = VcfGlPropagationReadyReport {
        schema_version: VCF_GL_PROPAGATION_READY_SCHEMA_VERSION,
    ...
    ensure_vcf_gl_propagation_ready_contract(&report)?;
    Ok(report)
}

fn build_vcf_gl_propagation_ready_row(
    command_report: &VcfRenderedCommandsReport,
    binding: VcfStageReadinessBinding,
    smoke_report: Option<&LocalVcfGlPropagationSmokeReport>,
) -> VcfGlPropagationReadyRow {
    ...
    let output_ready = binding.output_row.as_ref().is_some_and(|row| {
        row.status == VcfAdapterOutputCoverageStatus::Complete
            && row.benchmark_status == "benchmark_ready"
            && contains_artifact_id(&row.raw_outputs, "gl_propagated_vcf")
            && !row.normalized_metrics.is_empty()
            && !row.manifest.trim().is_empty()
            && contains_artifact_id(&row.index_outputs, "gl_propagated_vcf_tbi")
    });
    if !output_ready {
    ...
    let expected_result_ready = binding.expected_row.as_ref().is_some_and(|row| {
        row.report_section == "likelihood_postprocess"
            && row.expected_outputs.iter().any(|value| value == "gl_propagated_vcf")
    ...
    let smoke_ready = smoke_report.is_some_and(|report| {
        report.parseable
            ...
            && !report.output_bcf_path.trim().is_empty()
            && !report.output_bcf_csi_path.trim().is_empty()
            && !report.report_path.trim().is_empty()
            && !report.metrics_path.trim().is_empty()
            && !report.stage_result_manifest_path.trim().is_empty()
            && report.lost_fields.is_empty()
            && likelihood_field_sets_match(
                &report.input_likelihood_fields,
                &report.output_likelihood_fields,
            )
            && likelihood_field_sets_match(
                &report.input_likelihood_fields,
                &required_likelihood_fields(),
            )
            && report.site_count_before > 0
            && report.site_count_before == report.site_count_after
            && report.sample_count > 0
    });
    if !smoke_ready {
        missing_surfaces.push("local_vcf_gl_propagation_smoke".to_string());
    }
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "active retained VCF GL propagation caller `{}` keeps active scope, command, output, parser, expected-result, report, and likelihood-preserving smoke proof for `vcf.gl_propagation`",
            binding.retained_row.tool_id
        )
    } else {
        format!(
            "active retained VCF GL propagation caller `{}` is missing: {}",
            binding.retained_row.tool_id,
    ...
    VcfGlPropagationReadyRow {
        ...
        smoke_output_bcf_path: smoke_report
            .map_or_else(no_value_string, |report| report.output_bcf_path.clone()),
        smoke_output_bcf_csi_path: smoke_report
            .map_or_else(no_value_string, |report| report.output_bcf_csi_path.clone()),
        smoke_report_path: smoke_report
            .map_or_else(no_value_string, |report| report.report_path.clone()),
        ...
        smoke_input_likelihood_fields: smoke_report
            .map(|report| report.input_likelihood_fields.clone())
            .unwrap_or_default(),
        smoke_output_likelihood_fields: smoke_report
            .map(|report| report.output_likelihood_fields.clone())
            .unwrap_or_default(),
        smoke_lost_fields: smoke_report
            .map(|report| report.lost_fields.clone())
            .unwrap_or_default(),
        smoke_site_count_before: smoke_report.map_or(0, |report| report.site_count_before),
        smoke_site_count_after: smoke_report.map_or(0, |report| report.site_count_after),
...
fn ensure_vcf_gl_propagation_ready_contract(report: &VcfGlPropagationReadyReport) -> Result<()> {
    if report.retained_row_count != report.rows.len() {
        return Err(anyhow!(
            "VCF gl_propagation readiness must keep exactly one row per active retained `vcf.gl_propagation` binding"
        ));
    }
    if report.rows.is_empty() {
        return Err(anyhow!(
            "VCF gl_propagation readiness must keep at least one active retained caller row"
        ));
    }
    if report.checked_surface_count != 8 {
        return Err(anyhow!("VCF gl_propagation readiness must record exactly 8 checked surfaces"));
    }
    let unique_results =
        report.rows.iter().map(|row| row.result_id.as_str()).collect::<BTreeSet<_>>().len();
    if unique_results != report.rows.len() {
        return Err(anyhow!(
            "VCF gl_propagation readiness must keep one unique result_id per active retained caller row"
        ));
    }
    for row in &report.rows {
        if row.stage_id != VCF_GL_PROPAGATION_STAGE_ID {
            return Err(anyhow!(
                "VCF gl_propagation readiness row `{}` drifted away from the `vcf.gl_propagation` stage",
                row.stage_id
            ));
        }
        if row.coverage_status == COVERAGE_STATUS_COMPLETE && !row.missing_surfaces.is_empty() {
            return Err(anyhow!(
                "VCF gl_propagation readiness row `{}` / `{}` cannot be complete while listing missing surfaces",
                row.stage_id,
                row.tool_id
            ));
        }
        if row.coverage_status == COVERAGE_STATUS_COMPLETE && !row.smoke_lost_fields.is_empty() {
            return Err(anyhow!(
                "VCF gl_propagation readiness row `{}` / `{}` cannot be complete while losing likelihood fields",
                row.stage_id,
                row.tool_id
            ));
        }
...
fn required_likelihood_fields() -> Vec<String> {
    REQUIRED_LIKELIHOOD_FIELDS.iter().map(|value| (*value).to_string()).collect()
}

fn likelihood_field_sets_match(left: &[String], right: &[String]) -> bool {
