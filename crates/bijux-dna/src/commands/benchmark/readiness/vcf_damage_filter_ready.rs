...
use crate::commands::benchmark::local_vcf_damage_filter_smoke::{
    run_local_vcf_damage_filter_smoke, LocalVcfDamageFilterSmokeReport,
};
use crate::commands::cli::parse;
use crate::commands::cli::render;

pub(crate) const DEFAULT_VCF_DAMAGE_FILTER_READY_PATH: &str =
    "benchmarks/readiness/vcf/damage-filter-ready.json";
const VCF_DAMAGE_FILTER_READY_SCHEMA_VERSION: &str =
    "bijux.bench.readiness.vcf_damage_filter_ready.v1";
const VCF_DAMAGE_FILTER_STAGE_ID: &str = "vcf.damage_filter";
...
const REQUIRED_DAMAGE_CONTEXT_RULE: &str =
...
pub(crate) struct VcfDamageFilterReadyRow {
    ...
    pub(crate) smoke_summary_path: String,
    pub(crate) smoke_counts_path: String,
    pub(crate) smoke_warnings_path: String,
    pub(crate) smoke_damage_manifest_path: String,
    ...
    pub(crate) smoke_removed_variants: u64,
    pub(crate) smoke_retained_variants: u64,
    pub(crate) smoke_low_quality_filtered_variants: u64,
    pub(crate) smoke_damage_ratio_filtered_variants: u64,
    pub(crate) smoke_terminal_damage_filtered_variants: u64,
    pub(crate) smoke_damage_context_rule: String,
    pub(crate) smoke_terminal_context_count: u64,
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct VcfDamageFilterReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<VcfDamageFilterReadyRow>,
    pub(crate) violations: Vec<VcfDamageFilterReadyRow>,
}

pub(crate) fn run_render_vcf_damage_filter_ready(
    args: &parse::BenchReadinessRenderVcfDamageFilterReadyArgs,
) -> Result<()> {
    let repo_root = std::env::current_dir().context("resolve current directory")?;
    let report = render_vcf_damage_filter_ready(
        &repo_root,
        args.output.clone().unwrap_or_else(|| PathBuf::from(DEFAULT_VCF_DAMAGE_FILTER_READY_PATH)),
    )?;
    if args.json {
...
pub(crate) fn render_vcf_damage_filter_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<VcfDamageFilterReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_vcf_damage_filter_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "vcf.damage_filter active retained callers must stay complete across active-scope, command, output, parser, report, and damage-context smoke proof"
        ));
    }
    Ok(report)
}

fn build_vcf_damage_filter_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<VcfDamageFilterReadyReport> {
    let (command_report, bindings) =
        collect_vcf_stage_readiness_bindings(repo_root, VCF_DAMAGE_FILTER_STAGE_ID)?;
    ...
    for binding in active_bindings {
        let smoke_report =
            run_local_vcf_damage_filter_smoke(repo_root, &binding.retained_row.tool_id).ok();
        rows.push(build_vcf_damage_filter_ready_row(
            &command_report,
            binding,
            smoke_report.as_ref(),
        ));
    ...
    let report = VcfDamageFilterReadyReport {
        schema_version: VCF_DAMAGE_FILTER_READY_SCHEMA_VERSION,
    ...
    ensure_vcf_damage_filter_ready_contract(&report)?;
    Ok(report)
}

fn build_vcf_damage_filter_ready_row(
    command_report: &VcfRenderedCommandsReport,
    binding: VcfStageReadinessBinding,
    smoke_report: Option<&LocalVcfDamageFilterSmokeReport>,
) -> VcfDamageFilterReadyRow {
    ...
    let output_ready = binding.output_row.as_ref().is_some_and(|row| {
        row.status == VcfAdapterOutputCoverageStatus::Complete
            && row.benchmark_status == "benchmark_ready"
            && contains_artifact_id(&row.raw_outputs, "damage_filtered_vcf")
            && !row.normalized_metrics.is_empty()
            && !row.manifest.trim().is_empty()
            && contains_artifact_id(&row.index_outputs, "damage_filtered_vcf_tbi")
    });
    if !output_ready {
    ...
    let expected_result_ready = binding.expected_row.as_ref().is_some_and(|row| {
        row.report_section == "damage_aware_filtering"
            && row.expected_outputs.iter().any(|value| value == "damage_filtered_vcf")
    ...
    let report_ready = binding.report_row.as_ref().is_some_and(|row| {
        row.section_id == "damage_aware_filtering"
            && row.summary_table == "damage_filtering_metrics"
            && required_metric_names
                .iter()
    ...
    let smoke_ready = smoke_report.is_some_and(|report| {
        report.parseable
            ...
            && !report.summary_path.trim().is_empty()
            && !report.counts_path.trim().is_empty()
            && !report.warnings_path.trim().is_empty()
            && !report.damage_manifest_path.trim().is_empty()
            && !report.stage_result_manifest_path.trim().is_empty()
            && report.input_variants > 0
            && report.removed_variants > 0
            && report.retained_variants > 0
            && report.input_variants == report.removed_variants + report.retained_variants
            && report.low_quality_filtered_variants > 0
            && report.damage_ratio_filtered_variants > 0
            && report.terminal_damage_filtered_variants > 0
            && report.damage_context_rule == REQUIRED_DAMAGE_CONTEXT_RULE
            && report.terminal_context_count > 0
            && report.sample_count > 0
    });
    if !smoke_ready {
        missing_surfaces.push("local_vcf_damage_filter_smoke".to_string());
    }
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "active retained VCF damage filter `{}` keeps active scope, command, output, parser, expected-result, report, and damage-context smoke proof for `vcf.damage_filter`",
            binding.retained_row.tool_id
        )
    } else {
        format!(
            "active retained VCF damage filter `{}` is missing: {}",
            binding.retained_row.tool_id,
    ...
    VcfDamageFilterReadyRow {
        ...
        smoke_summary_path: smoke_report
            .map_or_else(no_value_string, |report| report.summary_path.clone()),
        smoke_counts_path: smoke_report
            .map_or_else(no_value_string, |report| report.counts_path.clone()),
        smoke_warnings_path: smoke_report
            .map_or_else(no_value_string, |report| report.warnings_path.clone()),
        smoke_damage_manifest_path: smoke_report
            .map_or_else(no_value_string, |report| report.damage_manifest_path.clone()),
        ...
        smoke_removed_variants: smoke_report.map_or(0, |report| report.removed_variants),
        smoke_retained_variants: smoke_report.map_or(0, |report| report.retained_variants),
        smoke_low_quality_filtered_variants: smoke_report
            .map_or(0, |report| report.low_quality_filtered_variants),
        smoke_damage_ratio_filtered_variants: smoke_report
            .map_or(0, |report| report.damage_ratio_filtered_variants),
        smoke_terminal_damage_filtered_variants: smoke_report
            .map_or(0, |report| report.terminal_damage_filtered_variants),
        smoke_damage_context_rule: smoke_report
            .map_or_else(no_value_string, |report| report.damage_context_rule.clone()),
        smoke_terminal_context_count: smoke_report
            .map_or(0, |report| report.terminal_context_count),
...
fn ensure_vcf_damage_filter_ready_contract(report: &VcfDamageFilterReadyReport) -> Result<()> {
    if report.retained_row_count != report.rows.len() {
        return Err(anyhow!(
            "VCF damage_filter readiness must keep exactly one row per active retained `vcf.damage_filter` binding"
        ));
    }
    if report.rows.is_empty() {
        return Err(anyhow!(
            "VCF damage_filter readiness must keep at least one active retained caller row"
        ));
    }
    if report.checked_surface_count != 8 {
        return Err(anyhow!("VCF damage_filter readiness must record exactly 8 checked surfaces"));
    }
    let unique_results =
        report.rows.iter().map(|row| row.result_id.as_str()).collect::<BTreeSet<_>>().len();
    if unique_results != report.rows.len() {
        return Err(anyhow!(
            "VCF damage_filter readiness must keep one unique result_id per active retained caller row"
        ));
    }
    for row in &report.rows {
        if row.stage_id != VCF_DAMAGE_FILTER_STAGE_ID {
            return Err(anyhow!(
                "VCF damage_filter readiness row `{}` drifted away from the `vcf.damage_filter` stage",
                row.stage_id
            ));
        }
        if row.coverage_status == COVERAGE_STATUS_COMPLETE && !row.missing_surfaces.is_empty() {
            return Err(anyhow!(
                "VCF damage_filter readiness row `{}` / `{}` cannot be complete while listing missing surfaces",
                row.stage_id,
                row.tool_id
            ));
        }
