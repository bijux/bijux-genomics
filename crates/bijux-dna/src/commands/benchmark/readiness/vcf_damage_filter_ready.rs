...
fn build_vcf_damage_filter_ready_row(
    ...
    VcfDamageFilterReadyRow {
        ...
        smoke_removed_variants: smoke_report.map(|report| report.removed_variants).unwrap_or(0),
        smoke_retained_variants: smoke_report.map(|report| report.retained_variants).unwrap_or(0),
        smoke_low_quality_filtered_variants: smoke_report
            .map(|report| report.low_quality_filtered_variants)
            .unwrap_or(0),
        smoke_damage_ratio_filtered_variants: smoke_report
            .map(|report| report.damage_ratio_filtered_variants)
            .unwrap_or(0),
        smoke_terminal_damage_filtered_variants: smoke_report
            .map(|report| report.terminal_damage_filtered_variants)
            .unwrap_or(0),
        smoke_damage_context_rule: smoke_report
            .map(|report| report.damage_context_rule.clone())
            .unwrap_or_else(no_value_string),
        smoke_terminal_context_count: smoke_report
            .map(|report| report.terminal_context_count)
            .unwrap_or(0),
