...
pub(crate) const DEFAULT_FASTQ_FILTER_STAGES_READY_PATH: &str =
    "benchmarks/readiness/fastq/filter-stages-ready.json";
const FASTQ_FILTER_STAGES_READY_SCHEMA_VERSION: &str =
    "bijux.bench.readiness.fastq_filter_stages_ready.v1";
...
const FILTER_READS_REQUIRED_METRIC_FIELDS: [&str; 9] = [
    "filtered_reads_r1",
    "reads_retained",
    "reads_dropped",
    "reads_removed",
...
const FILTER_LOW_COMPLEXITY_REQUIRED_METRIC_FIELDS: [&str; 5] = [
    "filtered_fastq_r1",
    "reads_retained",
    "reads_dropped",
    "reads_removed",
...
const FILTER_STAGE_SPECS: [FilterStageSpec; 2] = [
    FilterStageSpec {
        stage_id: "fastq.filter_reads",
        expected_tool_ids: &FILTER_READS_TOOL_IDS,
        required_output_ids: &FILTER_READS_REQUIRED_OUTPUT_IDS,
        normalized_metrics_output_id: "report_json",
        required_metric_fields: &FILTER_READS_REQUIRED_METRIC_FIELDS,
    },
    FilterStageSpec {
        stage_id: "fastq.filter_low_complexity",
        expected_tool_ids: &FILTER_LOW_COMPLEXITY_TOOL_IDS,
        required_output_ids: &FILTER_LOW_COMPLEXITY_REQUIRED_OUTPUT_IDS,
        normalized_metrics_output_id: "filter_report_json",
        required_metric_fields: &FILTER_LOW_COMPLEXITY_REQUIRED_METRIC_FIELDS,
    },
];

#[derive(Debug, Clone, Serialize)]
pub(crate) struct FastqFilterStagesReadyRow {
...
#[derive(Debug, Clone, Serialize)]
pub(crate) struct FastqFilterStagesReadyReport {
    ...
    pub(crate) ok: bool,
    pub(crate) rows: Vec<FastqFilterStagesReadyRow>,
    pub(crate) violations: Vec<FastqFilterStagesReadyRow>,
}
...
#[derive(Debug, Clone)]
struct FilterStageSchemaContract {
    extension_id: String,
    required_fields: Vec<String>,
}

pub(crate) fn run_render_fastq_filter_stages_ready(
    args: &parse::BenchReadinessRenderFastqFilterStagesReadyArgs,
) -> Result<()> {
    let repo_root = std::env::current_dir().context("resolve current directory")?;
    let report = render_fastq_filter_stages_ready(
        &repo_root,
        args.output
            .clone()
            .unwrap_or_else(|| PathBuf::from(DEFAULT_FASTQ_FILTER_STAGES_READY_PATH)),
    )?;
    if args.json {
...
pub(crate) fn render_fastq_filter_stages_ready(
    repo_root: &Path,
    output_path: PathBuf,
) -> Result<FastqFilterStagesReadyReport> {
    let output_path = repo_relative_path(repo_root, &output_path);
    let report = build_fastq_filter_stages_ready_report(repo_root, &output_path)?;
    bijux_dna_infra::atomic_write_json(&output_path, &report)?;
    if !report.ok {
        return Err(anyhow!(
            "fastq filter stages must keep active scope, command, output, parser, expected-result, report, and schema proof"
        ));
    }
    Ok(report)
}

fn build_fastq_filter_stages_ready_report(
    repo_root: &Path,
    output_path: &Path,
) -> Result<FastqFilterStagesReadyReport> {
    ...
    let schema_contracts = collect_filter_stage_schema_contracts()?;
    ...
    ensure_filter_stage_tool_sets(
        "FASTQ filter-stage active rows",
    ...
    let output_rows = output_contract
        .rows
        .into_iter()
        .filter(|row| filter_stage_binding_admitted(&row.stage_id, &row.tool_id))
        .collect::<Vec<_>>();
    ensure_filter_stage_tool_sets(
        "FASTQ filter-stage output-contract rows",
    ...
    let parser_rows = parser_coverage
        ...
        .collect::<Vec<_>>();
    ensure_filter_stage_tool_sets(
        "FASTQ filter-stage parser rows",
    ...
    let expected_rows = expected_results
        ...
        .into_iter()
        .filter(|row| {
            row.domain == "fastq" && filter_stage_binding_admitted(&row.stage_id, &row.tool_id)
        })
        .collect::<Vec<_>>();
    ensure_filter_stage_tool_sets(
        "FASTQ filter-stage expected-result rows",
    ...
    let command_rows = command_coverage
        ...
        .collect::<Vec<_>>();
    ensure_filter_stage_tool_sets(
        "FASTQ filter-stage rendered-command rows",
    ...
    for active_row in active_rows {
        ...
        let spec = filter_stage_spec(&active_row.stage_id).expect("filter stage spec");
        let output_row = output_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing filter-stage output-contract row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let parser_row = parser_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing filter-stage parser row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let expected_row = expected_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing filter-stage expected-result row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        let command_row = command_by_binding.get(&key).ok_or_else(|| {
            anyhow!(
                "missing filter-stage rendered-command row for `{}` / `{}`",
                active_row.stage_id,
                active_row.tool_id
            )
        })?;
        ...
        rows.push(build_fastq_filter_stages_ready_row(
            active_row,
            output_row,
            parser_row,
            expected_row,
    ...
    Ok(FastqFilterStagesReadyReport {
        schema_version: FASTQ_FILTER_STAGES_READY_SCHEMA_VERSION,
        output_path: path_relative_to_repo(repo_root, output_path),
        active_row_count: rows.len(),
        complete_row_count,
        incomplete_row_count,
        checked_surface_count: CHECKED_SURFACE_COUNT,
        stage_count: FILTER_STAGE_SPECS.len(),
        expected_tool_ids_by_stage: FILTER_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
            ...
            .collect(),
        required_output_ids_by_stage: FILTER_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
                    spec.stage_id.to_string(),
            ...
            .collect(),
        required_metric_fields_by_stage: FILTER_STAGE_SPECS
            .iter()
            .map(|spec| {
                (
                    spec.stage_id.to_string(),
...
fn build_fastq_filter_stages_ready_row(
    ...
    schema_contract: &FilterStageSchemaContract,
    spec: FilterStageSpec,
    schema_proof_path: &str,
) -> FastqFilterStagesReadyRow {
    ...
    let reason = if missing_surfaces.is_empty() {
        format!(
            "filter binding `{}` / `{}` keeps active scope, command, output, parser, expected-result, report, and schema proof for `{}`",
            active_row.stage_id, active_row.tool_id, active_row.stage_id
        )
    } else {
        format!(
            "filter binding `{}` / `{}` is missing readiness proof for {}",
            active_row.stage_id,
            active_row.tool_id,
...
fn collect_filter_stage_schema_contracts() -> Result<BTreeMap<String, FilterStageSchemaContract>> {
    ...
    for spec in FILTER_STAGE_SPECS {
        ...
        contracts.insert(
            spec.stage_id.to_string(),
            FilterStageSchemaContract { extension_id: extension_id.to_string(), required_fields },
        );
    }
    ...
    T: BindingRow,
{
    ...
    for row in rows {
        ...
        if by_binding.insert(key.clone(), row).is_some() {
            return Err(anyhow!(
                "duplicate filter-stage binding `{}` / `{}`",
                key.stage_id,
                key.tool_id
            ));
        }
...
fn ensure_filter_stage_tool_sets<'a>(
    label: &str,
    ...
    for spec in FILTER_STAGE_SPECS {
        ...
        if actual != expected {
            return Err(anyhow!(
                "{} for `{}` must match the governed filter tool set: expected [{}], found [{}]",
                label,
                spec.stage_id,
...
fn ensure_required_stage_rows<'a>(
    ...
    let expected =
        FILTER_STAGE_SPECS.iter().map(|spec| spec.stage_id.to_string()).collect::<BTreeSet<_>>();
...
fn filter_stage_spec(stage_id: &str) -> Option<FilterStageSpec> {
    FILTER_STAGE_SPECS.iter().copied().find(|spec| spec.stage_id == stage_id)
}
...
mod tests {
    ...
    use super::{
        render_fastq_filter_stages_ready, DEFAULT_FASTQ_FILTER_STAGES_READY_PATH,
        FASTQ_FILTER_STAGES_READY_SCHEMA_VERSION,
    };
    ...
    #[test]
    fn render_fastq_filter_stages_ready_reports_complete_filter_bindings() {
        let root = repo_root();
        let report = render_fastq_filter_stages_ready(
            &root,
            PathBuf::from(DEFAULT_FASTQ_FILTER_STAGES_READY_PATH),
        )
        .expect("render FASTQ filter stages readiness");
        ...
        let row = report
            .rows
            .iter()
            .find(|row| row.stage_id == "fastq.filter_low_complexity" && row.tool_id == "bbduk")
        ...
        assert_eq!(row.normalized_metrics_output_id.as_deref(), Some("filter_report_json"));
