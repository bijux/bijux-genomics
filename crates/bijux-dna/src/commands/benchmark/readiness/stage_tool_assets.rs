...
const STAGE_TOOL_ASSETS_SCOPE: &str = "benchmark_ready_command_assets";
...
fn render_stage_tool_assets_toml(config: &StageToolAssetsConfig) -> Result<String> {
    ...
    Ok(format!(
        "# schema_version = 1\n\
         ...
         # purpose = Governed asset declarations for benchmark-ready FASTQ and BAM stage-tool commands.\n\
    ...
    fn render_stage_tool_assets_reports_governed_asset_rows() {
        ...
        assert!(report.rows.iter().any(|row| {
            row.stage_id == "fastq.deplete_host"
                && row.tool_id == "bowtie2"
                && row.asset_role == "reference_index"
