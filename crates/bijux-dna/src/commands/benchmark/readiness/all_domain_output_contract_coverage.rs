...
mod tests {
    ...
    fn render_all_domain_output_contract_coverage_reports_complete_active_rows() {
        ...
        assert_eq!(report.output_declaration_binding_count, 126);
        assert_eq!(report.source_proof_binding_count, 126);
        assert_eq!(report.covered_row_count, 126);
        assert_eq!(report.missing_row_count, 0);
        assert_eq!(report.coverage_percent, 100.0);
        assert_eq!(report.raw_output_declared_row_count, 126);
        assert_eq!(report.normalized_metrics_declared_row_count, 126);
        assert_eq!(report.logs_declared_row_count, 126);
        assert_eq!(report.manifest_declared_row_count, 126);
        ...
        assert_eq!(report.proof_source_counts.get(PROOF_SOURCE_VCF), Some(&14));
        ...
        assert_eq!(report.coverage_status_counts.get(COVERAGE_STATUS_COVERED), Some(&126));
