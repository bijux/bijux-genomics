...
mod tests {
    ...
    fn render_all_domain_output_contract_coverage_reports_complete_active_rows() {
        ...
        assert_eq!(report.output_declaration_binding_count, 125);
        assert_eq!(report.source_proof_binding_count, 125);
        assert_eq!(report.covered_row_count, 125);
        assert_eq!(report.missing_row_count, 0);
        assert_eq!(report.coverage_percent, 100.0);
        assert_eq!(report.raw_output_declared_row_count, 125);
        assert_eq!(report.normalized_metrics_declared_row_count, 125);
        assert_eq!(report.logs_declared_row_count, 125);
        assert_eq!(report.manifest_declared_row_count, 125);
        ...
        assert_eq!(report.proof_source_counts.get(PROOF_SOURCE_VCF), Some(&13));
        assert_eq!(report.index_required_row_count, 8);
        assert_eq!(report.index_declared_row_count, 8);
        assert_eq!(report.index_not_applicable_row_count, 117);
        assert_eq!(report.index_coverage_counts.get(INDEX_STATUS_COVERED), Some(&8));
        assert_eq!(report.index_coverage_counts.get(INDEX_STATUS_NOT_APPLICABLE), Some(&117));
        assert_eq!(report.coverage_status_counts.get(COVERAGE_STATUS_COVERED), Some(&125));
