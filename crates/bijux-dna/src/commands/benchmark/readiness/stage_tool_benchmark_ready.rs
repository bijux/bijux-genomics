...
mod tests {
    ...
    fn stage_tool_benchmark_ready_gate_tracks_ready_slice_and_excluded_pairs() {
        ...
        assert_eq!(report.benchmark_ready_pair_count, 112);
        assert_eq!(report.excluded_pair_count, 11);
        assert_eq!(report.failing_pair_count, 0);
        assert_eq!(report.generated_job_pair_count, 112);
        assert_eq!(report.expected_result_pair_count, 112);
        assert_eq!(report.benchmark_ready_stage_count, 47);
