...
const CANONICAL_RESULT_COUNT: usize = 126;
const ESSENTIAL_PIPELINE_NODE_COUNT: usize = 93;
const EXPECTED_ROW_COUNT: usize = 126;
const FAKE_RUN_ROW_COUNT: usize = 126;
const FAKE_FAILURE_ROW_COUNT: usize = 126;
const MISSING_AUDIT_ROW_COUNT: usize = 126;
const REAL_SMOKE_ROW_COUNT: usize = 4;
const INSUFFICIENT_DATA_ROW_COUNT: usize = 1;
const UNSUPPORTED_PAIR_ROW_COUNT: usize = 1;
const TOTAL_ROW_COUNT: usize = 603;
...
fn ensure_full_benchmark_result_collector_contract(
    ...
    if count_status(&report.result_status_counts, FullBenchmarkResultStatus::Expected) != 126
        || count_status(&report.result_status_counts, FullBenchmarkResultStatus::Succeeded) != 223
        || count_status(&report.result_status_counts, FullBenchmarkResultStatus::Failed) != 126
        || count_status(&report.result_status_counts, FullBenchmarkResultStatus::Present) != 123
    ...
    if report.domain_counts.get("fastq").copied() != Some(282)
        ...
        || report.domain_counts.get("vcf").copied() != Some(93)
    {
        return Err(anyhow!(
