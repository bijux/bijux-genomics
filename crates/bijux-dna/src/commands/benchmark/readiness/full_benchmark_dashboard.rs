...
pub(crate) const DEFAULT_FULL_BENCHMARK_DASHBOARD_MARKDOWN_PATH: &str =
    "benchmarks/readiness/FASTQ_BAM_VCF_BENCHMARK_DASHBOARD.md";
pub(crate) const DEFAULT_FULL_BENCHMARK_DASHBOARD_JSON_PATH: &str =
    "benchmarks/readiness/FASTQ_BAM_VCF_BENCHMARK_DASHBOARD.json";
...
fn ensure_full_benchmark_dashboard_contract(
    ...
    if report.total_tools != expected_results.tool_count || report.total_tools != 67 {
        return Err(anyhow!(
    ...
    if report.total_expected_jobs != expected_results.row_count || report.total_expected_jobs != 126
    {
        return Err(anyhow!(
            "full benchmark dashboard total expected jobs drifted from the governed expected-result slice"
        ));
    }
    if report.ready_jobs != full_report.present_row_count || report.ready_jobs != 123 {
        return Err(anyhow!(
