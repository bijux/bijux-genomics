...
fn build_vcf_filter_ready_row(
    ...
    VcfFilterReadyRow {
        ...
        smoke_filter_breakdown_path: smoke_report
            .map(|report| report.filter_breakdown_path.clone())
            .unwrap_or_else(no_value_string),
        smoke_filter_breakdown_tsv_path: smoke_report
            .map(|report| report.filter_breakdown_tsv_path.clone())
            .unwrap_or_else(no_value_string),
        smoke_filter_explain_path: smoke_report
            .map(|report| report.filter_explain_path.clone())
            .unwrap_or_else(no_value_string),
        ...
        smoke_pass_variants: smoke_report.map(|report| report.pass_variants).unwrap_or(0),
        smoke_failed_variants: smoke_report.map(|report| report.failed_variants).unwrap_or(0),
        smoke_filter_ids: smoke_report.map(|report| report.filter_ids.clone()).unwrap_or_default(),
        smoke_depth_threshold: smoke_report.map(|report| report.depth_threshold).unwrap_or(0.0),
        smoke_quality_threshold: smoke_report.map(|report| report.quality_threshold).unwrap_or(0.0),
        smoke_missingness_threshold: smoke_report
            .map(|report| report.missingness_threshold)
            .unwrap_or(0.0),
