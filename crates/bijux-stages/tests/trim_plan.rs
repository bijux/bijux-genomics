...
fn trim_output_names_are_defined_for_known_tools() {
    assert_eq!(
        bijux_stages::fastq::trim::trim_output_name("fastp"),
        Some("fastp.fastq.gz")
    );
    assert_eq!(
        bijux_stages::fastq::trim::trim_output_name("trimmomatic"),
        Some("trimmomatic.fastq.gz")
    );
    assert_eq!(bijux_stages::fastq::trim::trim_output_name("unknown"), None);
}
...
fn plan_trim_rejects_unknown_tool() {
    match bijux_stages::fastq::trim::plan(
        "mystery",
        std::path::Path::new("reads.fastq.gz"),
        std::path::Path::new("out"),
    ) {
