...
mod tests {
    ...
    fn bayeshammer_reconstruction_preserves_paired_record_count() {
        ...
        fs::create_dir_all(&bin_dir).expect("create fake tool dir");
        ...
        .expect("write fake bayeshammer");
        let mut permissions = fs::metadata(&fake_bayeshammer)
        ...
        let status = Command::new(&plan.command.template[0])
            ...
            .status()
            .expect("execute fake bayeshammer plan");
    ...
    fn read_gzip_text(path: &Path) -> String {
        ...
        reader
            ...
            .unwrap_or_else(|err| panic!("read gzip {}: {err}", path.display()));
        decoded
    }
