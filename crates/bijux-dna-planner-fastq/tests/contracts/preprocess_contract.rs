...
fn preprocess_policy_records_adapter_handoff_when_detection_stage_is_present() {
    ...
    assert_eq!(
        adapter_inference["source_stage_id"],
        serde_json::json!("fastq.detect_adapters")
    );
    assert_eq!(
        adapter_inference["source_tool_id"],
    ...
    assert_eq!(
        adapter_inference["consumer_binding"]["tool_id"],
        serde_json::json!("fastp")
    );
}
