...
fn local_detect_duplicates_premerge_smoke_plans_use_governed_toy_fixtures() -> Result<()> {
    ...
    let plans = bijux_dna_planner_fastq::stage_api::local_detect_duplicates_premerge_smoke_plans(
        &repo_root,
    )?;
    assert_eq!(
        plans.len(),
        2,
        "governed local-smoke config must keep duplicate-hit and clear coverage"
    );

    let duplicate_hit =
        plans.iter().find(|case| case.sample_id == "duplicate-hit-pe").unwrap_or_else(|| {
            panic!("duplicate-hit-pe case missing from local duplicate smoke plans")
        });
    ...
    let duplicate_clear =
        plans.iter().find(|case| case.sample_id == "duplicate-clear-pe").unwrap_or_else(|| {
            panic!("duplicate-clear-pe case missing from local duplicate smoke plans")
        });
    assert_eq!(duplicate_clear.r1, PathBuf::from("assets/toy/core-v1/fastq/reads_1.fastq"));
    assert_eq!(duplicate_clear.r2, Some(PathBuf::from("assets/toy/core-v1/fastq/reads_2.fastq")));
    assert_eq!(
    ...
    assert_eq!(
        duplicate_clear.plan.effective_params["paired_mode"],
