...
fn local_deplete_host_plan_uses_governed_repo_inputs() -> Result<()> {
    ...
    assert_eq!(plan.out_dir, PathBuf::from("target/local-ready/fastq.deplete_host"));
    ...
    assert_eq!(
        retained_reads.path,
        PathBuf::from("target/local-ready/fastq.deplete_host/host_depleted.fastq.gz")
    );
    ...
    assert_eq!(
        removed_reads.path,
        PathBuf::from("target/local-ready/fastq.deplete_host/removed_host.fastq.gz")
    );
    ...
    assert_eq!(
        report_json.path,
        PathBuf::from("target/local-ready/fastq.deplete_host/host_depletion_report.json")
    );
