...
fn local_trim_terminal_damage_smoke_plans_use_governed_toy_fixture() -> Result<()> {
    ...
    assert_eq!(
        case.plan.out_dir,
        PathBuf::from("target/local-smoke/fastq.trim_terminal_damage/ancient-se/cutadapt")
    );
    assert_eq!(case.plan.resources.threads, 1);
    assert_eq!(
        case.plan.params["report_json"],
        serde_json::json!(
            "target/local-smoke/fastq.trim_terminal_damage/ancient-se/cutadapt/trim_terminal_damage_report.json"
        )
    );
