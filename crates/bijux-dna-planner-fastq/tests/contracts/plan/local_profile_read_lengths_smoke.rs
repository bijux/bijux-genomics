...
#[test]
fn local_profile_read_lengths_smoke_plans_use_governed_toy_fixtures() -> Result<()> {
    ...
    let single_end = plans
        ...
        .unwrap_or_else(|| panic!("toy-se case missing from local read-length smoke plans"));
    assert_eq!(single_end.plan.stage_id.as_str(), "fastq.profile_read_lengths");
    ...
    let paired_end = plans
        ...
        .unwrap_or_else(|| panic!("toy-pe case missing from local read-length smoke plans"));
