...
- `benchmarks/readiness/bam/stages/` holds stage-owned BAM completion gates when a single stage needs an explicit governed completion artifact beyond broader BAM slice reports, including the owned `bam.overlap_correction`, `bam.endogenous_content`, and `bam.damage` completion JSON artifacts.
