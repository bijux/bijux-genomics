...
## Contracts
- Contract configs are schema-validated and owner-mapped in CI.
- Production tool versions are pinned; floating versions are rejected.
- Isolated runs must keep writable build/temp paths under `ISO_ROOT`.
- Generated contract docs must be regenerated from source-of-truth inputs.
...
- Report and manifest contracts are validated by policy/contract tests.
