...
| bam.qc_pre | structural intake | report | supported | Emit governed pre-filter QC summaries with a reporting companion before downstream interpretation. |
...
| bam.complexity | qc expansion | report | supported | Estimate library complexity through the governed complexity-curve and saturation contract. |
...
| bam.overlap_correction | cleanup | mutation | supported | Correct overlapping pairs with governed corrected-BAM and before-and-after audit artifacts. |
...
| bam.bias_mitigation | policy mediation | mutation | supported | Record governed bias-mitigation actions alongside preserved damage-evidence artifacts. |
