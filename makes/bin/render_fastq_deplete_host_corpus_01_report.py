...
def parse_args() -> argparse.Namespace:
    return parse_corpus_report_args(
        description="Render corpus-01 fastq.deplete_host benchmark summary.",
        docs_root="docs/benchmark/fastq.deplete_host/corpus-01",
    )
...
def validate_run_manifest_contract(run_manifest: dict) -> None:
    ...
    if run_manifest.get("dry_run"):
        raise SystemExit(
            "deplete-host benchmark report rendering requires an executed run, not --dry-run output"
        )
    if run_manifest.get("sample_limit") is not None:
        raise SystemExit(
            "deplete-host benchmark report rendering requires the full corpus run, not --sample-limit output"
        )
    if run_manifest.get("stage_id") != DEPLETE_HOST_BENCHMARK_CONTRACT.stage_id:
        raise SystemExit(
            "deplete-host benchmark report drift: "
            f"expected stage_id {DEPLETE_HOST_BENCHMARK_CONTRACT.stage_id}, "
            f"found {run_manifest.get('stage_id')}"
        )
    if run_manifest.get("scenario_id") != DEPLETE_HOST_BENCHMARK_CONTRACT.scenario_id:
        raise SystemExit(
            "deplete-host benchmark report drift: "
            f"expected scenario_id {DEPLETE_HOST_BENCHMARK_CONTRACT.scenario_id}, "
    ...
    if not run_manifest.get("reference_index"):
        raise SystemExit(
            "deplete-host benchmark report drift: run manifest must record reference_index"
        )
...
def validate_row_contract(
    *,
    ...
    for row in sample_rows:
        ...
        if float(row["host_identity_threshold"]) != float(
            run_manifest["host_identity_threshold"]
        ):
            raise SystemExit(
                "deplete-host benchmark report drift: "
                f"expected host_identity_threshold {run_manifest['host_identity_threshold']}, "
                f"found {row['host_identity_threshold']} for {row['sample_id']}/{row['tool']}"
            )
        if bool(row["retain_unmapped_only"]) != bool(
            run_manifest["retain_unmapped_only"]
        ):
            raise SystemExit(
                "deplete-host benchmark report drift: "
                f"expected retain_unmapped_only {run_manifest['retain_unmapped_only']!r}, "
                f"found {row['retain_unmapped_only']!r} for {row['sample_id']}/{row['tool']}"
            )
...
def render_markdown(summary: dict) -> str:
    lines: list[str] = []
    lines.append("# `fastq.deplete_host` on `corpus-01`")
    lines.append("")
    ...
    lines.append(f"- retain_unmapped_only: `{summary['retain_unmapped_only']}`")
    lines.append("")
    ...
    lines.append(
        f"- Highest mean host fraction removed: `{summary['headline']['strongest_depletion_tool']}` at `{summary['headline']['strongest_depletion_fraction']:.3f}`."
    )
    lines.append(
    ...
    for row in summary["tool_summary"]:
        lines.append(
            f"| `{row['tool']}` | {row['records']} | {row['pass_rate']:.1%} | {row['median_runtime_s']:.3f} | {row['median_read_retention']:.3f} | {row['median_base_retention']:.3f} | {row['mean_host_fraction_removed']:.3f} | {row['mean_reads_removed']:.1f} |"
        )
    lines.append("")
    lines.append("## Notes")
    lines.append("")
...
def main() -> int:
    ...
    run_root = (
        ...
        if args.run_root
        else preferred_report_run_root(corpus_root, DEPLETE_HOST_BENCHMARK_CONTRACT.stage_id)
    )
    ...
    for run in run_manifest["runs"]:
        ...
        for record in records:
            ...
            row = {
                ...
                "host_fraction_removed": float(
                    normalize_metric(record, "host_fraction_removed") or 0.0
                ),
                ...
                "host_identity_threshold": normalize_parameter(
                    record, "host_identity_threshold"
                )
                or run_manifest["host_identity_threshold"],
                "retain_unmapped_only": normalize_parameter(
                    record, "retain_unmapped_only"
                )
                if normalize_parameter(record, "retain_unmapped_only") is not None
                else run_manifest["retain_unmapped_only"],
    ...
    for tool, rows in sorted(tool_rows.items()):
        ...
        host_removed = [float(row["host_fraction_removed"]) for row in rows]
        ...
        tool_summary.append(
            {
                ...
                "mean_host_fraction_removed": safe_mean(host_removed),
    ...
    summary = {
        ...
        "stage_id": DEPLETE_HOST_BENCHMARK_CONTRACT.stage_id,
        "scenario_id": DEPLETE_HOST_BENCHMARK_CONTRACT.scenario_id,
        ...
        "retain_unmapped_only": run_manifest["retain_unmapped_only"],
        "headline": {
