...
def tool_runtime_summary(rows: list[dict]) -> list[dict]:
    ...
    for tool in sorted(by_tool):
        ...
        reads_with_umi = [float(row["reads_with_umi"]) for row in tool_rows]
        reads_with_umi_fraction = [
            float(row["reads_with_umi_fraction"]) for row in tool_rows
        ]
...
def summarize_cohort_rows(
    *,
    ...
    read_retention = [float(row["read_retention"]) for row in cohort_rows]
    reads_with_umi_fraction = [
        float(row["reads_with_umi_fraction"]) for row in cohort_rows
    ]
    return {
        "tool": tool,
        "dimension": dimension,
