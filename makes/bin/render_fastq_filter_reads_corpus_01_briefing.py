...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--docs-root",
        default="docs/benchmark/fastq.filter_reads/corpus-01",
    )
...
def tool_runtime_summary(rows: list[dict]) -> list[dict]:
    ...
    for tool in sorted(by_tool):
        ...
        reads_dropped = [float(row["reads_dropped"]) for row in tool_rows]
        removed_low_complexity = [
            float(row["reads_removed_low_complexity"]) for row in tool_rows
        ]
        removed_by_n = [float(row["reads_removed_by_n"]) for row in tool_rows]
...
def summarize_cohort_rows(
    *,
    ...
    reads_dropped = [float(row["reads_dropped"]) for row in cohort_rows]
