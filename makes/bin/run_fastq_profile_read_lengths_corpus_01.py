...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fastq.profile_read_lengths benchmarks for corpus-01."
    )
    parser.add_argument(
        "--repo-root",
        default=".",
    ...
    parser.add_argument(
        "--tools",
        default="",
        help="Comma-separated read-length profiler ids. Defaults to the governed benchmark cohort.",
    )
...
def report_path(out_root: Path, sample_id: str) -> Path:
    return out_root / "bench" / "profile_read_lengths" / sample_id / "report.json"
...
def main() -> int:
    ...
    requested_tools = (
        ...
        if args.tools
        else PROFILE_READ_LENGTHS_BENCHMARK_CONTRACT.tools
    )
