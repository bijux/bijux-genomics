...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-root",
        default="",
        help="Benchmark output root. Defaults to <corpus-root-parent>/results/<corpus-dir>/fastq.profile_overrepresented_sequences/lunarc.",
    )
    parser.add_argument(
        "--platform",
...
def build_command(
    ...
    command = [
        ...
        "bench",
        "fastq",
        "profile-overrepresented-sequences",
        "--sample-id",
        sample["sample_id"],
        "--r1",
        str(sample["r1"]),
...
def main() -> int:
    ...
    out_root = (
        ...
        if args.out_root
            ...
            corpus_root, PROFILE_OVERREPRESENTED_BENCHMARK_CONTRACT.stage_id
        )
    )
    ...
    requested_tools = (
        ...
        if args.tools
        else PROFILE_OVERREPRESENTED_BENCHMARK_CONTRACT.tools
    )
    ...
    payload = {
        "schema_version": "bijux.fastq.profile_overrepresented_sequences.corpus_run.v1",
