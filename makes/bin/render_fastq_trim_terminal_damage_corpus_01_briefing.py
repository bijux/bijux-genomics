...
def parse_args() -> argparse.Namespace:
    return parse_corpus_briefing_args(
        ...
        docs_root="docs/benchmark/fastq.trim_terminal_damage/corpus-01",
    )
...
def tool_runtime_summary(rows: list[dict]) -> list[dict]:
    return summarize_tool_runtime_rows(
        ...
        metric_specs=[
            ...
            BriefingMetricSpec('asymmetry_reduction', 'mean_asymmetry_reduction', 'mean', skip_blank=True),
...
def sample_runtime_outliers(rows: list[dict]) -> list[dict]:
    return summarize_sample_outlier_rows(
        ...
        selectors=[
            ...
            BriefingOutlierSpec('asymmetry_reduction', 'max', 'strongest_damage_tool', 'strongest_asymmetry_reduction', parser='float', skip_blank=True),
        ],
...
def render_markdown(
    ...
    lines.append("")
    lines.append(
        f"- `{len(runtime_rows)}` governed terminal-damage backends were benchmarked across `{summary['samples_total']}` samples (`{zero_exit}/{len(rows)}` zero-exit tool-sample observations)."
    )
    lines.append(
