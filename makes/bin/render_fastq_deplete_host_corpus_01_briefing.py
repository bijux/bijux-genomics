...
def parse_args() -> argparse.Namespace:
    return parse_corpus_briefing_args(
        description="Render an enriched benchmark briefing from corpus-01 deplete-host artifacts.",
...
def tool_runtime_summary(rows: list[dict]) -> list[dict]:
    return summarize_tool_runtime_rows(
        ...
        metric_specs=[
            ...
            BriefingMetricSpec('host_fraction_removed', 'mean_host_fraction_removed', 'mean'),
...
def summarize_cohort_rows(
    *,
    ...
    host_removed = [float(row["host_fraction_removed"]) for row in cohort_rows]
    return {
        "tool": tool,
        "dimension": dimension,
        "cohort": cohort,
...
def sample_runtime_outliers(rows: list[dict]) -> list[dict]:
    ...
    for sample_id, sample_rows in by_sample.items():
        ...
        strongest = max(
            sample_rows,
            key=lambda row: float(row["host_fraction_removed"]),
        )
        output.append(
            {
                "sample_id": sample_id,
                ...
                "strongest_host_fraction_removed": float(
                    strongest["host_fraction_removed"]
                ),
            }
        )
...
def render_markdown(
    ...
    strongest = max(runtime_rows, key=lambda row: row["mean_host_fraction_removed"])
    zero_exit = sum(1 for row in rows if row["exit_code"] == "0")

    lines: list[str] = []
    lines.append("# `fastq.deplete_host` benchmark on `corpus-01`")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"- `{len(runtime_rows)}` governed host-depletion backends were benchmarked across `{summary['samples_total']}` samples (`{zero_exit}/{len(rows)}` zero-exit tool-sample observations)."
    )
    lines.append(
        f"- Fastest median runtime: `{fastest['tool']}` at `{fmt_runtime(fastest['median_runtime_s'])}` seconds."
    )
    lines.append(
        f"- Highest mean host fraction removed: `{strongest['tool']}` at `{fmt_metric(strongest['mean_host_fraction_removed'])}`."
    )
    lines.append("")
    ...
    for row in runtime_rows:
        lines.append(
            f"| `{row['tool']}` | {fmt_runtime(row['median_runtime_s'])} | {fmt_runtime(row['p90_runtime_s'])} | {fmt_metric(row['median_read_retention'])} | {fmt_metric(row['median_base_retention'])} | {fmt_metric(row['mean_host_fraction_removed'])} | {fmt_metric(row['mean_reads_removed'])} | x{row['slowdown_vs_fastest_median']:.2f} |"
        )
    lines.append("")
    ...
    for row in cohort_rows:
        lines.append(
            f"| `{row['tool']}` | `{row['cohort']}` | {fmt_runtime(row['mean_runtime_s'])} | {fmt_runtime(row['median_runtime_s'])} | {fmt_metric(row['median_read_retention'])} | {fmt_metric(row['median_base_retention'])} | {fmt_metric(row['mean_host_fraction_removed'])} | {row['samples']} |"
        )
    lines.append("")
    ...
    for row in outliers[:5]:
        lines.append(
            f"| `{row['sample_id']}` | `{row['accession']}` | `{row['era']}` | `{row['layout']}` | `{row['size_band']}` | {fmt_runtime(row['total_stage_runtime_s'])} | `{row['slowest_tool']}` | {fmt_runtime(row['slowest_runtime_s'])} | `{row['strongest_depletion_tool']}` | {fmt_metric(row['strongest_host_fraction_removed'])} |"
        )
    lines.append("")
