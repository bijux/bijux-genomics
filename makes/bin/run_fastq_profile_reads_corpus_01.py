...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fastq.profile_reads benchmarks for corpus-01."
    )
    parser.add_argument(
        "--repo-root",
        default=".",
    ...
    parser.add_argument(
        "--out-root",
        default="",
        help="Benchmark output root. Defaults to <corpus-root-parent>/results/<corpus-dir>/fastq.profile_reads/lunarc.",
    )
    parser.add_argument(
        "--platform",
    ...
    parser.add_argument(
        "--tools",
        default="",
        help="Comma-separated read-profile tool ids. Defaults to the governed benchmark cohort.",
    )
...
def report_path(out_root: Path, sample_id: str) -> Path:
    return out_root / "bench" / "profile_reads" / sample_id / "report.json"
...
def main() -> int:
    ...
    out_root = (
        ...
        if args.out_root
        else default_results_stage_root(corpus_root, PROFILE_READS_BENCHMARK_CONTRACT.stage_id)
    )
    ...
    requested_tools = (
        ...
        if args.tools
        else PROFILE_READS_BENCHMARK_CONTRACT.tools
    )
    tools = require_canonical_tool_roster(
        repo_root,
        PROFILE_READS_BENCHMARK_CONTRACT.stage_id,
        requested_tools,
        scenario_id=PROFILE_READS_BENCHMARK_CONTRACT.scenario_id,
    )
    runs: list[SampleRun] = []
    failures = 0
