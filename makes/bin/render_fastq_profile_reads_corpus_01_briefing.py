...
def validate_summary_contract(summary: dict) -> None:
    if summary.get("stage_id") != "fastq.profile_reads":
        raise SystemExit(
            "profile-reads briefing drift: "
            f"expected stage_id fastq.profile_reads, found {summary.get('stage_id')}"
        )
    if summary.get("scenario_id") != "profile_reads_fairness":
        raise SystemExit(
            "profile-reads briefing drift: "
            f"expected scenario_id profile_reads_fairness, found {summary.get('scenario_id')}"
        )
...
def validate_rows_contract(summary: dict, rows: list[dict]) -> None:
    if not rows:
        raise SystemExit("profile-reads briefing drift: sample_results.csv must not be empty")
...
def summarize_cohort_rows(
    *,
    ...
    mean_q = [float(row["mean_q"]) for row in cohort_rows]
    gc_percent = [float(row["gc_percent"]) for row in cohort_rows]
    read_length = [float(row["mean_read_length"]) for row in cohort_rows]
    histogram_bins = [float(row["histogram_bin_count"]) for row in cohort_rows]
    return {
        "tool": tool,
        "dimension": dimension,
        "cohort": cohort,
