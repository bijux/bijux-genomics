...
def retired_execution_python_paths() -> list[Path]:
    ...
    paths = [ROOT / "makes" / "bin" / name for name in names]
    paths.append(ROOT / "makes" / "bin" / "benchmark_fastq_corpus" / "runner_compat.py")
    return paths
...
class CorpusBenchmarkSupportTests(unittest.TestCase):
    ...
    def test_remediation_queue_builder_is_deleted(self) -> None:
        self.assertFalse(
            (ROOT / "makes" / "bin" / "build_corpus_01_benchmark_remediation_queue.py").exists()
        )
...
class BenchmarkMakefileTests(unittest.TestCase):
    ...
    def test_fastq_benchmark_execution_no_longer_ships_python_runner_wrappers(self) -> None:
        self.assertEqual(
            [path.name for path in retired_execution_python_paths() if path.exists()],
            [],
        )
    ...
    def test_benchmark_tooling_uses_neutral_dossier_filename(self) -> None:
        ...
        self.assertFalse((BIN_DIR / "audit_corpus_01_fastq_benchmark_docs.py").exists())
    ...
    def test_corpus_execution_runner_wrappers_are_not_shipped_in_makes_bin(self) -> None:
        self.assertEqual(
            sorted(path.name for path in retired_execution_python_paths() if path.exists()),
            [],
        )
