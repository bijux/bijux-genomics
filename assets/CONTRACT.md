...
1. Run the relevant generator command (for example `cargo run -p bijux-dna-dev -- assets run refresh-reference`, `cargo run -p bijux-dna-dev -- assets run refresh-toy`, or `cargo run -p bijux-dna-dev -- assets run refresh-golden`).
2. Update package checksums and generation metadata.
3. Run asset policy checks (`cargo run -q -p bijux-dna-dev -- checks run check-assets-contracts`).
