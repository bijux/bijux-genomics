...
| total_tools | 67 | `benchmarks/readiness/expected-benchmark-results-all-domains.tsv` | `tool_count` | unique tools across canonical all-domain expected benchmark jobs |
| total_expected_jobs | 126 | `benchmarks/readiness/expected-benchmark-results-all-domains.tsv` | `row_count` | canonical FASTQ, BAM, and VCF expected benchmark bindings |
| ready_jobs | 123 | `benchmarks/readiness/FASTQ_BAM_VCF_BENCHMARK_REPORT.json` | `present_row_count` | expected benchmark jobs with present governed result rows |
