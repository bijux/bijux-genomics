...
| total_tools | 68 | `benchmarks/readiness/expected-benchmark-results-all-domains.tsv` | `tool_count` | unique tools across canonical all-domain expected benchmark jobs |
| total_expected_jobs | 128 | `benchmarks/readiness/expected-benchmark-results-all-domains.tsv` | `row_count` | canonical FASTQ, BAM, and VCF expected benchmark bindings |
| ready_jobs | 125 | `benchmarks/readiness/FASTQ_BAM_VCF_BENCHMARK_REPORT.json` | `present_row_count` | expected benchmark jobs with present governed result rows |
