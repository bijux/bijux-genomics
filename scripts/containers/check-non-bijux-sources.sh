...
for line in text.splitlines():
    ...
    if not m:
        continue
    tool_id, def_path, upstream, license_field, checksum = m.groups()
    rows[tool_id] = (def_path, upstream, license_field, checksum)
...
for tool_id in defs:
    if tool_id not in rows:
        ...
        continue
    def_path, upstream, license_field, checksum = rows[tool_id]
