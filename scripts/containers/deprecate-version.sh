...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: scripts/containers/deprecate-version.sh --tool <id> --version <semver> --rationale <text> --removal-after <YYYY-MM-DD> [--compatibility-mode allowed|blocked]
EOF
      exit 0
      ;;
...
[[ -n "$tool" && -n "$version" && -n "$rationale" && -n "$removal_after" ]] || { echo "missing required args" >&2; exit 2; }
[[ "$compatibility_mode" == "allowed" || "$compatibility_mode" == "blocked" ]] || { echo "--compatibility-mode must be allowed|blocked" >&2; exit 2; }

python3 - "$ROOT_DIR" "$tool" "$version" "$today" "$removal_after" "$rationale" "$compatibility_mode" <<'PY'
...
tool, version, deprecated_since, removal_after, rationale, mode = sys.argv[2:8]
