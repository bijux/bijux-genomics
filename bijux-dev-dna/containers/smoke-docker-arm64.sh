...
get_registry_field() {
  field="$1"
  tool="$2"
  value=$("$ROOT_DIR/bijux-dev-dna/containers/registry-tools.sh" show-tool "$tool" 2>/dev/null \
...
RUNTIME_TOOLS=$("$ROOT_DIR/bijux-dev-dna/containers/registry-tools.sh" tools-by-runtime docker)
