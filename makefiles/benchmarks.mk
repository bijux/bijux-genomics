...
benchmark-trim: ## Benchmark adapter/quality trimming tools
	...
	OUT_DIR="."; \
	if [ -n "$(FASTQ_ROOT_OVERRIDE)" ]; then FASTQ_ROOT="$(FASTQ_ROOT_OVERRIDE)"; else FASTQ_ROOT="lab/corpus/fastq"; fi; \
	if [ -z "$(FASTQ_ROOT_OVERRIDE)" ] && [ -d lab/corpus/fastq/canonical ]; then FASTQ_ROOT="lab/corpus/fastq/canonical"; fi; \
	ROOTS="$$FASTQ_ROOT"; \
	ROOTS=$$(echo $$ROOTS | tr "," " "); \
	FILES=""; \
	...
	FILES=$$(echo $$FILES | tr " " "\n" | sort | uniq); \
	if [ -z "$$FILES" ]; then \
		echo "no FASTQ files found in lab/corpus/fastq"; \
		exit 1; \
	fi; \
...
benchmark-merge: ## Benchmark read merging tools (paired-end)
	...
	FILES=$$(echo $$FILES | tr " " "\n" | sort | uniq); \
	if [ -z "$$FILES" ]; then \
		echo "no paired FASTQ files found in lab/corpus/fastq"; \
		exit 1; \
	fi; \
	for r1 in $$FILES; do \
